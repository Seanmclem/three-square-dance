...
**Version 4.60.3** — last updated 2026-08-05
...
- Preview is an `<img>` re-rendered per gesture: pointer drag = orbit (yaw/pitch), wheel = zoom; Zoom + Light sliders each paired with a numeric input (v4.60.3 `NumField` — local draft while typing so "0.5" isn't clamped at "0", live-commits clamped to the slider's min/max, snaps display on blur/Enter); Reset view returns to `DEFAULT_STAGE`
