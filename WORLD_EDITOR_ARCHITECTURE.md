...
- **railing meshes**: built when `hasRailing: true` — posts are `BoxGeometry`, top-rail bars come from `railBarGeo(len, taperStart, taperEnd)` (v4.23.6/7): spans exactly ±len/2 (callers bake the `+barT` miter extension into `len` — the path branch does so for every segment), square ends = `BoxGeometry`, but a **free end of a sloped run** keeps its tip in place and eases the corner (`ExtrudeGeometry` side profile: end face raked `0.15·barT` meeting a 45° bottom-corner chamfer `0.4·barT`). Config from `stair.railing` (`StairRailingDef`): `topRail`/`balusters` toggles, `height`, `stepInterval` (a post every N steps, top step always included), `barThickness`, `postThickness`, `sideInset`, `overhang`. Absent → defaults (0.9 / every step / 0.1 / 0.06 / 0.1 / 0.15). Two code paths:
