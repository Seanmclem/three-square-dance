...
**Version 4.23.4** — last updated 2026-07-09
...
- v4.23.4 — **Stair rail-end returns (drooping ends).** Rail free ends jutted straight out (the flat `overhang` stub clipped into walls/landings). `StairRailingDef` += `overhangAngle?` (degrees, 0/absent = current flat overhang; >0 curls each free rail end **down** by that angle over `overhang` length — like a real handrail return). StairBuilder gains `addRailSeg` (renders a bar between two absolute rail-height points) + `addEndReturn` (drops a short segment from a rail endpoint along its run's horizontal direction, drooping by the angle). Applied at both ends of the legacy plain-stair top rail (when angled, the rail runs anchor→anchor with drooping ends instead of a centered flat overhang) and at the first/last point of every Phase-29 rail polyline (returns get no post). Default 0 → byte-identical to before. Panel: `END DROP°` field (0–90) in the RAILING section. Verified in-browser at 60° on a 4-flight switchback — ends curl down cleanly (10 return bars added), no longer protruding.
...
material, per-face winding corrected for the mirrored "left"-turn frame);
...
- **railing meshes**: built from `BoxGeometry` parts when `hasRailing: true`, config from `stair.railing` (`StairRailingDef`): `topRail`/`balusters` toggles, `height`, `stepInterval` (a post every N steps, top step always included), `barThickness`, `postThickness`, `sideInset`, `overhang`. Absent → defaults (0.9 / every step / 0.1 / 0.06 / 0.1 / 0.15). Two code paths:
  - **Plain stair** (`isPlainStair`): the pre-Phase-29 railing verbatim — per side, a sloped top rail plus vertical balusters anchored at the **centre of each step's tread** (local x = 0, outer edge), rail spanning first→last tread centre + a symmetric `overhang` each end, built via a slope-aligned orthonormal basis (`makeBasis`): rail follows the diagonal, posts are world-vertical. Legacy stairs render pixel-identical.
  - **Landing/switchback** (Phase 29): `computeRailPaths(stair, layout)` returns **side-tagged** (`RailSide`, inner/outer) walking-surface-height polylines — an **inner path** around the void (up flight k's void side → level across the landing's void edge → straight up flight k+1 → … → free end after the top landing's crossing; the tread-anchor line passes through `(u=run, y=topY)` exactly, so slope→level corners join without a gooseneck) and **outer rails** gated by `landingPerimeter` (v4.23.1, default **false**): OFF = per-flight sloped segments terminating at each landing boundary; ON = one continuous path wrapping every landing's three outer perimeter edges, the last leg lying on flight k+1's outer rail line, terminating at the top landing's open exit edge. flights=1 + landing = balcony: side rails stop at the landing edge, or (perimeter ON) wrap its 3 exposed edges with a lateral jog when `landing.width > width`; the entry edge stays open. `emitRailPath` renders each segment as an oriented box `handrailH` above the path (+`barThickness` length overlap closes interior miters) and posts from the pure module's per-side deduped list — corner posts at every vertex, per-step anchors on sloped runs, `stepInterval × stepDepth` spacing on level runs — gated per side by `balustersInner`/`balustersOuter` (both default to `balusters`; inner = turn/void side, same mapping in the plain-stair branch). Rails have no colliders (unchanged).
...
**Colliders:** `ColliderBuilder.registerStairSteps(stair)` → consumes the same `computeStairLayout` output: one box collider per step **per flight**, plus one cuboid per landing slab (frame-axis-aligned, rotated by the stair's `-angle` like the step boxes). Plain stairs emit byte-identical descriptors to pre-Phase-29.
