...
**Version 4.32.0** — last updated 2026-07-16
...
**Add ceiling (cap closed loop)** — Phase 38. Same closed-loop gating as the floor fill. Creates a node-backed polygon `PlatformDef` (reusing the run's node IDs, so it follows wall-node drags) at `position.y = wall.elevation + wall.height` — slab bottom flush with the wall top, lid resting ON the walls — with `thickness 0.2`. Retexture the underside via the platform's BOTTOM material section (`bottomMaterial`).
