...
- **`#` toggle (Phase 51)** swaps the pill strip between **category mode**
  (exclusive, one shelf) and **tag mode** (multi-select, ANDed). Tags are
  ordered by frequency, active ones pinned to the front of the strip, overflow
  in the shared `More ▾` popout (which stays open across clicks in tag mode and
  shows per-tag counts). The two filters compose — the toggle shows the active
  tag count (`#2`), and in tag mode an active category renders as a clearable
  `Category ✕` pill so it can't hide behind an empty grid
...
- `#` toggle swaps the same strip to **tag chips** (multi-select, ANDed, frequency-ordered) — see the AssetBrowser.tsx section above
