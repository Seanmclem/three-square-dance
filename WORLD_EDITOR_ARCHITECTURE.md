...
**Version 4.60.1** — last updated 2026-08-05
...
existing `materialList`/`assets` props), plus two preview-overlay checkboxes
(v4.60.1): **perf counter** and **crosshair**. Their state lives in App
(`showPerfCounter`/`showCrosshair`, persisted to localStorage
`editorShowPerf`/`editorShowCrosshair` the way `editorQuality` is — `'0'` =
off, absent = on) and threads App → PropertiesPanel → ToolView; the perf one
gates `<FpsCounter>` in App's preview overlay stack, the crosshair one is the
`showCrosshair` prop on PreviewHUD.
