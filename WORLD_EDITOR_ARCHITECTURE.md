...
**Version 4.36.0** — last updated 2026-07-21
...
**Current shape (v4.36.0):** a 64px column with a 10-entry `TOOLS` list (Select, Floor, Wall, Platform, Stair, Shape, Groups, Spawn, Trigger, Light — several are menu-first variant groups), then a bottom cluster: **ASSETS** flyout button (Models / Materials / Decals / Sounds / Skybox — `ASSET_ENTRIES`; "tool" rows call `onToolSelect("object"|"decal")` and rely on App's tool→panel coupling, "panel" rows call `onPanelToggle`), the amber **SCRIPTS** panel button, and the Play split-button row. The Object and Decal tools live *only* in the ASSETS flyout (moved from the tool list in Phase 42, along with the removal of the separate MATS/SOUNDS/SKYBOX buttons). The flyout anchors `bottom: 0` (near-viewport-bottom button) and shares the variant-popover close plumbing via the `openMenu` sentinel `"assets-menu"`. ASSETS button active-state: any of its five panels/tools active; its icon swaps to the active entry's.
