...
**Version 4.76.8** — last updated 2026-08-18
...
- v4.84.0 — **Phase 74: level preloading: the menu fully loads the level Start opens; a playing level downloads the next one (user: "what could the menu do to pre-load ... level-1, and level one do to preload for level-2?", then "do your What I'd build").** New `src/runtime/ScenePreloader.ts`, two depths. **Menu:** once the menu is up, `warm()` fetches the scene the likelier button opens (the save's scene when a runtime save exists, else `entryScene`), walks it with the export's own `collectAssetRefs`, and downloads AND decodes everything into AssetManager's caches; the other candidate scene is download-only. Nothing is being played on the menu, so main-thread GLTF parsing costs nothing the player can feel. **Playing:** `sceneEntered()` reads the level's `load_scene` targets (new `AssetRefs.scenes`, collected in `collectActions`, ignored by the export) and, 3 s after the level is up, downloads, never decodes, what those levels need that is not already in memory, at `fetch` low priority, 2 at a time, streaming and dropping the bytes, so gameplay frames are never touched and the transition parses from the HTTP cache instead of waiting on the network. The engine still has no notion of "level 2": the `load_scene` links ARE the order, which also covers branches and hubs. **AssetManager gained in-flight sharing (`_once`)** so pressing Start mid-preload joins the running loads instead of duplicating them, plus `preloadModel` / `fileUrls`. `RuntimeApp` now keeps the four fire-and-forget registry inits as promises because the preloader cannot map ids to files before they resolve. **Measured (exported `platfrom-obby`, static server with 80 ms per-request latency):** Start to playing **4530 ms before, 275 ms after**; at the title screen 76 requests instead of 12 (all 39 models, textures, 11 sounds, 4 icons), and Start adds exactly one request (the scene JSON). 3 s into level 1 it fetched `level_2.json` + only level 2's 3 new models and 3 new sounds (14 shared assets skipped), and those then served `deliveryType: cache`, 0 bytes transferred. **Race (1.5 s latency, Start clicked with 31 of 39 models loaded):** game loaded, zero assets requested twice. **Real transition (`__runtime.router.go("level_2")` on the dev shell origin):** level 2 rendered, only its 3 new models were requested, the 7 shared ones came from memory. On the dev shell the workspace is `no-store`, so the download tier detects that on its first response and turns itself off (a bug found in test: it kept counting skipped downloads as prefetched; fixed). Runtime chunk 19.6 KB to 26.9 KB; the editor chunk is unchanged. Not done on purpose: no GPU pre-upload / shader pre-compile (first frame of a level still pays that behind the fade), per-instance override-enabled map slots are not warmed, and the in-memory caches still never shrink across levels.
- v4.83.3 — **Unplaced prefabs no longer ship their assets in game exports (user: "every prefab in the game.json library has its assets included, even if no instance is placed anywhere? that should be fixed").** `src/export/assetRefs.ts` walked every `game.prefabs[].template` on top of the scenes. That was pure over-collection: a placed instance is real, stamped entities in the scene JSON (already walked by `collectZone`), the runtime has zero prefab machinery (no `.prefabs` read anywhere in the `runtime-*`/`WorldLoader-*` chunks, grep-verified), and `spawn_object` only re-shows entities that already exist in the scene. So the template walk could only ever ADD assets nothing in the game uses. Removed the walk plus the now-orphaned `collectPrefabTemplate` helper and `PrefabTemplateEntity` import. Measured on `platfrom-obby` (11 library prefabs): exactly two files dropped, `pipe_90.gltf` + `pipe_straight.gltf`, the members of the never-placed "Pipe base and corner" prefab; 109 → 107 files, `missing` still empty, and the re-exported bundle plays from `python3 -m http.server` with zero failed asset requests. `game.json` itself still ships verbatim, so the (unused) template JSON is still in the bundle; only the asset files are pruned.
- v4.83.2 — **Game exports ship only the chunks the runtime can reach (user: "every export should not carry about 1 MB of unused editor code, and not grow as I add utilities").** `desktop/export.ts` used to copy EVERY depth-1 file of `dist/assets` into the bundle, which included the editor's own entry chunk (`main-*.js`, ~920 KB, growing with every editor utility) and the dev-only `testHelpers-*.js` (~39 KB, carries `@/editor/bakeShapes`). Nothing in an exported game referenced either, so it was dead weight, not a behavior leak, but it was also the editor's source sitting downloadable on every game's host. Now the export crawls from `runtime.html`: a chunk ships only if its hashed file name appears in the text of the html or of an already-reachable js/css chunk (hashed names are unique, so a name match IS the import edge; this covers static imports, dynamic imports, and preload dep lists without needing a Vite build manifest, and works against the compiled app's embedded dist). `DEV_ONLY_CHUNK = /^testHelpers-/` is skipped and not crawled through: the runtime chunk does name it, but only behind `isDesktopDev()`, which is false on any static host. Verified with a real `platfrom-obby` export served by `python3 -m http.server`: bundle chunks are `runtime`, `WorldLoader`, `GLTFLoader`, `OBJLoader`, `MTLLoader`, `RGBELoader` only; title screen, Start, in-game all work with zero failed chunk requests (the only 404s are `favicon.ico` and the expected `/api/getAppInfo` desktop probe). **Rule for editor-only utilities: plain static imports from editor-only files are enough; never import them from `src/runtime/`, `src/preview/`, `src/shared/`, `src/world/`, `src/scripting/`, or the `src/ui` components `RuntimeApp` uses, or they land in the shared `WorldLoader` chunk and DO ship.**
- v4.83.1 — **Footstep pick is plain random again, plus an opt-in pitch wobble (user: "Ok no, I want random. Also a toggle for pitch-wobble ... it must be little to no performance cost").** v4.83.0 shipped "equal chance but never the same twice in a row", a rule the user had NOT asked for (they specified "an equal-chance for any of them to play"); it is removed: `steps[Math.floor(Math.random() * steps.length)]`, `_lastFootstep` deleted. Measured over 250 strides with 3 sounds: 96 / 74 / 80, 39% back-to-back repeats (about a third is expected of true random). **Pitch wobble:** `PlayerSettings.footstepPitchWobble?: boolean` (Sounds page checkbox, off by default, in `SETTINGS_PAGES.sounds`): each footstep plays at `1 +- FOOTSTEP_WOBBLE (0.06)` playback rate, about one semitone, so even a single sample stops sounding like a loop; it applies during a `set_footstep` override too (same emit path). Plumbing: `audio:play` gained optional `rate`, threaded through `AudioSystem._onPlay` into `_makeSound` (`sound.setPlaybackRate(rate)` before `play()`), a native WebAudio parameter. Verified: rates spread over 0.94 to 1.06 with mean 1.00, none sent when the toggle is off, and the live `THREE.Audio` objects carry the rate. **Cost (the user's requirement), measured by timing the controller's emit during a walk:** 0.05ms per footstep with one plain sound vs 0.027ms with 3 variations + wobble (noise), at about 2.8 footsteps a second = roughly 0.01% of a 120Hz frame budget; nothing was added to the per-frame path. Test artifact worth knowing: a hidden automation tab suspends the AudioContext, so one-shots never end and never dispose (660 live footstep objects piled up in the test tab); in a real window `onEnded` disposes each one.
- v4.83.0 — **Phase 73: footstep variation (user: "how crazy would it be to have like footstep variation? Like if I pick 2 or 3 footstep sounds instead of one ... [the trigger volume overrides] may need to be able to have more than one with random variation also").** Backward compatible: the single sound stays and a list of EXTRAS sits beside it. `PlayerSettings.footstepVariants?: string[]` (in `SETTINGS_PAGES.sounds`, so the Sounds page override carries it) and `ScriptAction.soundVariants?: string[]` for `set_footstep`; `character:set-footstep` gained `variants`. `CharacterController` holds POOLS: `_footstepDefault` (built once from settings by `footstepPool`, which drops blanks and duplicates) and `_footstepOverride: string[] | null`; each stride picks one: equal chance among the OTHER sounds, never the same twice in a row (roll n-1 slots and skip the last index; no per-step allocation). The user asked for "equal chance"; the no-repeat rule is added because pure random produces A-A-A runs, the machine-gun effect variation exists to remove, and with two sounds it becomes a left/right alternation. The override still reverts the way it always has: the volume's exit script fires `set_footstep` with an empty sound, which clears the whole pool. New shared `src/ui/SoundVariantList.tsx` ("+ Add variation", up to 3 extras, disabled until a main sound is picked) used by BOTH the character Sounds page and the `set_footstep` action editor, so they cannot drift; clearing the action's main sound also drops its variations; the script card summary shows "+N variations". `assetRefs.ts` collects both new lists so exports ship the extra samples. Verified in the runtime on level_3 (deterministic stepping, `audio:play` recorded): default single sound 55 of 55 strides identical; 3-sound override over 167 strides 57 / 55 / 55 with 0 immediate repeats; 2 sounds 28 / 27 in strict alternation; blank and duplicate variations collapse to one sound; an empty `set_footstep` returns to `footstep_carpet_000`. UI checked read-only in the editor (the button was NOT clicked: player settings write through to game.json). Docs: AUDIO.md section 4. Not done: per-step pitch or volume jitter, and jump/land variation.
- v4.82.0 — **Phase 72: enemy feel, plus two combat fixes (plan `plans/phase-72-enemy-feel.md`, test plan `test-plans/phase-72-enemy-feel.md`).** **(1) Enemy feel, presentation only (`EnemyAI.ts`):** a damped spring on each enemy mesh's Y scale (volume preserving, feet anchored) kicked when the player LANDS on it (`_stompCheck`: within 0.95m XZ, above its top, close, and falling; physical, so it needs no script; one kick per landing), and about 10 degrees of forward lean while actually moving (`_feel`, applied in `_applyPose` about the enemy's own right axis, offsets included). The stomp script's Death pose freezes the AI the same frame, so the frozen (`hasScriptClip`) branch now also runs `_feel` + `_applyPose`. Authored scales are captured per rec (`baseScales`) and put back on `preview:stop`, since the mover reset only restores position and rotation (verified: left preview at 0.83, restored to [0.5, 0.5, 0.5]). Measured: squash to 0.70, rebound 1.06, settles to 1.000; lean peaks 9.7 degrees. **(2) "Crabs bite and miss" (user report) was the knockback, not the bite.** With a motionless player the first bite lands, then the player is found STANDING ON THE CRAB and the next 13 real bites whiff (`overTop`, by design). The stock crab's "Bite juice" `launch_player` was 180 degrees relative to `player`, which is measured from the CAMERA's look yaw: looking away from the crab (the normal case when chased) it throws the player INTO the crab and the hop lands them on its back (29.5 of 30 seconds spent there). New launch frame `launchRelativeTo: "away"` (script panel: "Away"): straight away from the owning entity's LIVE position, `launchDirDeg` as an offset. `_resolveObjectPose` only knows AUTHORED positions, so ScriptEngine gained `setLivePositionProvider` (wired by App.tsx and RuntimeApp.tsx to the new `ObjectPlacer.getLivePosition`) and `character:launch` gained `awayFrom`; CharacterController turns it into a compass angle (`atan2(-vx, -vz)`), falling back to "backwards from the look" when directly on top. Verified: 0 to 0.2s on the crab's back, and on a clean timeline 2 of 2 real bites landed with the player knocked clear each time. The crab prefab's script was NOT edited (the user had that prefab open and unsaved); the one-field content change is in the test plan and HAZARDS_GUIDE. Noted, not changed: about 18% of this crab's bites are deliberate feints (`FEINT_CHANCE 0.3 x variation 0.6`), which also read as misses. **(3) `respawn_player` "Restore health" did nothing in any game whose health key is not literally `health`** (it was `gameState.resetKey("health")`; platfrom-obby uses `Hearts`, so both the death script and the Kill Floor prefab silently restored nothing). `ScriptEngine._healthKey` now resolves: the action's new optional `healthKey`; a registered `health`; the key the owning script's GLOBAL state trigger watches (threaded through `runActions`/`_dispatch` as `triggerKey`, captured at schedule time so a delayed action still has it); the key this scene's enemies damage (`ai.damageKey`). No match warns in the console. `GameState.hasDefault`. Script panel: "Health key (blank = auto)" under the checkbox. Verified on the user's real level_1: Hearts 1, 0, 3.
- v4.81.3 — **Legs froze briefly after landing while still moving (user report: jump while walking, keep walking, "the walking animation briefly does not play after landing... running too").** The locomotion state machine always played the `jump_land` one-shot to its end before returning to walk/run. For the stock character that is 0.458s at the game's `jumpAnimSpeed` 1.2 = 0.37s: traced at 44 frames in the landing pose while travelling 2.2m at walk speed, then another 0.15s of crossfade. Pre-existing (the land phase predates Phase 70), just more noticeable once running covered 3m in that window. Fix in `CharacterController`: the landing one-shot is for STANDING landings only. `_enterLand(isMoving, running)` goes straight to `_playGround` when a direction is held, and the `"land"` phase now also exits on `isMoving` (start walking during a standing landing and it cuts to walk the next frame). The Phase 70 squash spring still marks the impact, so a moving landing keeps its beat. Verified in the runtime on level_3: frames from touchdown to the locomotion clip 44 to 0 (walk and run); a standing jump still plays `Jump_Land` and ends in `Idle`; standing jump then W 10 frames after touchdown reaches `walk` 1 frame after the key press; total active mixer weight never below 1.0 across tap, full and standing jumps. Probe trap: a clamped finished one-shot holds the pose but is NOT `isRunning()`; sum weights over `mixer._isActiveAction(a)` instead.
- v4.81.2 — **Landing shadow now visibly shrinks with height (user: "the shadow should be like, smaller on the ground depending on how high the character is. right? and little to no performance cost").** It already shrank, but v4.80.0 tuned it to 35% over 6m, which is only about 10% at the top of a 1.75m jump: invisible. Now named constants in `CharacterController.ts`: `SHADOW_SHRINK 0.45` over `SHADOW_SHRINK_HEIGHT 3`m and `SHADOW_FADE 0.45` of `SHADOW_OPACITY 0.55` over `SHADOW_FADE_HEIGHT 6`m, each bottoming out at a floor so the disc still marks the landing spot on a long fall. Measured disc size vs on-the-ground: 86% at 0.9m, 74% at the 1.75m jump peak, 55% at 3m and above; opacity 0.55 to 0.30; always 0.03m above the surface. It is a blob-shadow convention, not physics (a sun shadow would not shrink): the point is the height cue. Cost, measured over 20,000 casts: the one `castRayAndGetNormal` per frame is 8.5 microseconds in level_3 (63 colliders) and 4 microseconds in level_1 (353 colliders), about 0.05 to 0.1% of a 120Hz frame, and it does not grow with collider count (Rapier's broad phase is a BVH); plus one textured quad (one draw call), no per-frame allocation. The whole `CharacterController.update` measured 0.12ms.
- v4.81.1 — **Running dropped the avatar into its bind pose (user report: "the run animation is just like a bare pose like a t-pose almost").** Not the asset (the stock character's `Run` clip is a full clip: 52 channels, 27 bones, 18 keyframes, same shape as `Walk`), and not a grounded-flicker (measured 0 ungrounded frames and 0 clip switches at run speed). Cause: platfrom-obby has `animClips: { walk: "Run" }` (the author mapped WALK to the Run clip), and Phase 71's new RUN intent auto-matches "Run" too. `mixer.clipAction(clip)` returns ONE action per clip, so walk to run made `_play` crossfade the action with ITSELF: `fadeIn` then `fadeOut` on the same action, weight 0, nothing playing, bind pose (measured: total mixer weight 0 after pressing Shift while walking). The v4.81.0 test missed it because it pressed W and Shift together (idle to run), never walk to run: the real user sequence. Fix: `_play` and `_playByName` now share `_crossfadeTo(next, loop, speed)`, which, when `next` is already the current action, keeps it playing and only re-loops/retimes it (restarting it if it was a finished one-shot). This also closes the same latent hole for script clips (`play_animation` naming the clip that is already playing) and for any two locomotion slots mapped to one clip. New `_playGround`: when RUN resolves to the very clip WALK uses (an author override, or a model with no run clip), the clip plays at `runMultiplier` speed so running still reads as running; because `_play` no-ops on an unchanged intent, `_playGround` also retimes the live loop directly. Verified on the real sequence idle, walk, +Shift, -Shift, stop: `Run x1`, `Run x1.4`, `Run x1`, `Idle`, and the total mixer weight never dropped below 1. Probe trap: `action.getEffectiveWeight()` returns a stale cached value for an action stopped by `stopAllAction()`; check `isRunning()` too.
- v4.81.0 — **Phase 71: run (user: "is there a run function now?"; design accepted in chat).** Plan `plans/phase-71-run.md`, test plan `test-plans/phase-71-run.md`. New held action `ActionState.run`: keyboard **Shift** (`bindings.kbm.run`, default ShiftLeft + ShiftRight; stored bindings merge over defaults so existing players pick it up), gamepad and touch when the move stick reaches `RUN_STICK_THRESHOLD = 0.85` of its travel (no new on-screen button: the touch layout has no spare one and both sticks are already analog). New `PlayerSettings.runMultiplier?: number` (Movement page, RUN SPEED x, in `SETTINGS_PAGES.movement`): absent or 1 = no run, which is the default, so every existing level plays exactly as before; verified that with it off, Shift does nothing and the stick maps 0 to 100% onto 0 to moveSpeed as always. In `CharacterController.update`: running = multiplier above 1, run held, and moving; wanted speed = `moveSpeed x multiplier` at full magnitude, fed into the Phase 70 ramp unchanged (0.08s to reach run speed; run can be held or released in the air). With run ENABLED the stick's 0 to 85% span maps onto 0 to full walk speed, so full walk is reached where run takes over (measured at 1.4x: 50% = 3.53, 84% = 5.93, 90% and 100% = 8.4 m/s). New `LocomotionState` `"run"` (Character page slot; auto-matches a clip named "run", which the stock character ships; `_groundClip` falls back to `walk` when the model has none). Forward lean scales up to the run speed. Jump height and air time are unchanged by running; jump DISTANCE scales with speed (walk jump 5.00m, run jump 7.00m at 1.4x, both 1.75m up and 0.82s), which is the level-design cost and why it is opt-in. Content: only the Jump Lab (`level_3`) turns it on (scene override 1.4), and its gap lane gains 7m and 8m gaps; running clears 5, 6 and 7m from an edge takeoff and fails 8m. platfrom-obby's game defaults are unchanged. Out of scope: run toggle, stamina, a touch run button, a key-rebinding UI. Testing note: a crab within its 6m detect radius chases and bites during a settle loop, and its bite knockback reads as a tiny "jump" in the readout, so measure away from enemies.
- v4.80.0 — **Phase 70: character feel ("dependable like roblox without being so rigid, something much more alive like mario").** Plan: `plans/phase-70-character-feel.md`; test plan: `test-plans/phase-70-character-feel.md`. User report: the character feels rigid and awkward, and the concrete difficulties are misjudging distance, misjudging the landing spot, and stomping enemies. Rule for the whole phase: **fixed rules, lively presentation** (the same input always gives the same arc; life comes from things that do not move the landing point). All in `CharacterController.ts` unless noted. **(1) Landing shadow:** a soft dark disc straight under the player, found with ONE Rapier `castRayAndGetNormal` per frame (sensors + own capsule excluded, 40m), laid on the hit normal, shrinking/fading slightly with height; enemies are solid colliders so it sits on a crab's back when you are above it (the stomp aiming cue); third person only, `NO_RAYCAST`, no per-frame allocation. The avatar previously had no shadow of any kind. **(2) Jump arc reshaped, not resized:** a jump rises under `GRAVITY_RISE` (about 15.9) and falls under `GRAVITY_FALL` (about 25.8), derived from one constant `JUMP_RISE_FRAC = 0.56` so that the peak AND the total air time equal the legacy 20/20 arc for any `jumpHeight` (measured before/after at dt 1/120: peak 1.752 to 1.748m, air 0.833 to 0.833s, distance 5.05 to 5.05m), so every gap already built is the same jump. Hold for height: releasing jump while rising applies 3x rise gravity (1-frame tap 0.60m, full hold always 1.75m); new `PlayerSettings.variableJump?: boolean` (Movement page checkbox, absent/true = on; false = every jump is the full fixed height). The reshaped arc applies ONLY between a jump's takeoff and its landing (`_jumpArc`): `character:launch` and walk-off falls stay on `LEGACY_GRAVITY = 20`, verified identical to the millimetre (launch 10: 2.458m / 0.992s / 2.331m), so every authored spring keeps its arc. The `jumpHeight` label mismatch (9.81 takeoff vs 20 fall, the setting yields about half its number) is deliberately preserved: the user deferred it and fixing it would resize every jump. **(3) Horizontal ramp:** the controller now holds `_velX/_velZ` and moves it toward `input x moveSpeed` at a limited rate (was: `input x speed x dt` straight into the move = full speed on frame one, dead stop on release, instant mid-air reversal). Rates are TIMES so they scale with moveSpeed: 0.08s to full speed and 0.08s to stop on the ground (0.27m stop slide at speed 6), 0.2s in the air (a full mid-air reversal now takes about 0.4s; air steering stays strong on purpose, landing precision needs it). `_coyote > 0` is the grounded read (raw `isGrounded` flickers while walking). Zeroed on teleport and ladder mount; mover carry and the launch channel stay separate and additive. **(4) Squash, stretch, lean:** a damped spring on the avatar root's Y scale (volume preserving, anchored at the feet because the root origin is the feet): +11% stretch at takeoff/launch, up to about -11% squash on landing scaled by impact speed, settles to exactly 1. Forward lean 5.7 degrees at full speed, roll up to 11 degrees into turns from the avatar's own turn rate. `_updatePresentation` owns the root's rotation (order YXZ) and scale; the capsule, camera and landing point never see any of it. **(5) Jump readout:** every airborne stretch is measured and `character:jump-stats { height, distance, airTime, drop }` is emitted after landing; `src/ui/JumpReadout.tsx` shows the last one under the perf counter. Editor preference `editorShowJumpStats`, ships OFF (no-selection panel > EDITOR > "Jump readout while playing"). Two measurement traps found and handled: `isGrounded` turns true about 2 frames before the capsule finishes settling (0.11m high after a full jump), so `drop` is read from the FEET 6 frames after landing, not on the landing frame; and while RISING past a ledge the capsule's round bottom grazes the lip and `isGrounded` flickers true for single frames, which reset the air timer and takeoff point mid-jump, so the landing block is now gated on `_velY <= 0` (this also makes the pre-existing land-sound air-time gate correct across a lip graze). **(6) Camera defaults:** stock third-person defaults for new games moved from distance 4 / level to distance 5.5 / angle 20 (`playerSettingsDefaults.ts` and the `PreviewController` fallback literal). **Content (platfrom-obby):** the user's hand-tuned level_2 camera (FOV 60, distance 5.5, height 0.8, angle 25) promoted to the game defaults and the now-redundant level_2 override removed, so level_1 and level_3 share it; the "Crab 1" stomp volume grown from 1.2 x 0.5 x 0.9 to 1.5 x 1 x 1.1 (def + the three placed copies). The old volume topped out at 0.75m while the crab's back is at 0.76m, so a player landing SQUARELY on the crab could never enter it, only glancing side hits registered; this is the exact case HAZARDS_GUIDE already warned about. New `level_3` "Jump Lab": gap lane 2/3/4/5/6m, step staircase 0.5/1.0/1.5/2.0m, crab pen, kill floor, hub checkpoint volume; marker posts on each pad count the metres (the engine has no text objects). Measured there: gaps up to 4m clear from an edge takeoff, 5m is exactly the jump distance (needs a late, coyote takeoff), 6m is impossible. **Findings NOT changed (pre-existing, reported to the user):** a ledge up to about jump height + capsule radius (2.0m here) is climbable because the capsule's round bottom rolls over the lip; a lip graze while rising refreshes the coyote window (a second jump is possible for 0.12s near a ledge); the kill floor respawns at the `checkpoint` state key, which is game-wide, so falling in a new level before touching its first checkpoint gate respawns at the PREVIOUS level's coordinates. Out of scope: sprint, camera behavior changes, avatar castShadow, fall speed cap, applying the faster fall to launches and ledge drops. Typecheck clean; console clean (and none of the pointer-lock exceptions fixed in v4.79.83).
- v4.79.84 — **Spawn Point > Camera: the `thirdPersonPitch` field is labelled CAMERA ANGLE/PITCH (was CAMERA ANGLE).** User was told to set "pitch" and could not find the field: the code, the docs and common camera vocabulary all say pitch, only the label said angle. Label-only change in `PropertiesPanel.tsx` (the `spawn-camera` screen); setting key, tooltip and the row subtitle (MODE · FOV · DISTANCE · ANGLE) are unchanged. Verified in the real panel (Select tool > click the spawn marker > Camera): the longer label fits on one line with its help icon.
- v4.79.83 — **Refused pointer-lock requests no longer throw an uncaught exception (found in a TESTING.md pass).** Chrome's `requestPointerLock()` returns a promise that REJECTS when the lock is refused (unfocused window, no user activation: e.g. a scene transition or preview entry while alt-tabbed, or any automation tab), and nothing caught it, so every refusal surfaced as `Uncaught WrongDocumentError: The root document of this element is not valid for pointer lock`. v4.79.81's `?.()` only guarded the MISSING api (iOS). All four request sites in `PreviewController` (enter, `pause:closed`, canvas mousedown, occlusion Tab) now go through one `_lockPointer()` helper: `requestPointerLock?.()?.catch(() => {})` (older Safari returns undefined, hence the second `?.`). Behavior is unchanged: a refused lock was always best-effort and the canvas-mousedown handler still re-acquires. Verified in an unfocused Chrome tab on the shell origin: runtime Start + `router.go` with an `unhandledrejection` listener recorded 0 rejections (was 1 per entry/transition); the shell window still acquires the lock on a real click. This was the "known pointer-lock automation artifact" noted in earlier test logs.
- v4.79.82 — **Jumping on crabs caused a real slowdown (user report, deployed mobile AND desktop).** The Phase-62 rise tripwire (`_step`'s >0.4m ground-snap diagnostic) fired per crab per frame when crabs clustered (their ground rays hit SIBLING crab bodies — only the player capsule was excluded) with a player bouncing on them, and its `console.warn + JSON.stringify` per frame was the cost. Fix: the `__aiRises` ring buffer keeps recording everything; the warn is throttled to one per 2s globally; and the step is now REFUSED (`return false`) — a >0.4m snap-up in a single ~4cm step is never terrain, it is the floating-crab class itself, so treating it as a ledge both stops crabs climbing each other and kills the repeat-fire. Revert candidate if crab movement regresses on tall terrain steps (none exist at ~4cm/frame speeds).
- v4.79.81 — **Two iOS-Safari bugs from the first real Netlify deploy (user's phone test).** (1) Completing a level crashed with "document.exitPointerLock is not a function": iOS has NO pointer-lock API, and PreviewController's teardown (scene-hop path included) called it bare — every exit/requestPointerLock call is now optional-chained (`?.()`); the kbm-scheme gates already kept the request sites unreachable on touch, this makes them unconditionally safe. (2) Dragging on screen selected the HUD like text (joystick zone, counters highlighted): the three game overlays (PreviewHUD, GameGuiOverlay, TouchControlsOverlay root) gain userSelect/WebkitUserSelect/WebkitTouchCallout none — scoped to the overlays, NOT global, so editor inputs keep normal selection. Bundle regenerated for re-deploy. iOS behavior verified by inspection only (no device harness) — user re-test requested.
- v4.79.80 — **Export no longer warns about disabled-map placeholder files (user hit `Missing: textures: file not found "brick_01/metalness.jpg"` + concrete_01).** Root cause: the stock texture manifest declares every map slot by convention — including `metalness` paths whose files never shipped — with `enabled: false`; `resolveAssetFiles` deliberately bundles disabled slots too (a scene override can re-enable them), so the copier looked for the phantom files and truthfully reported them. Fix: file records gain `optional` (true when the slot is disabled; required wins on duplicate refs) — the bundle still SHIPS a disabled slot's file when it exists, but a missing optional file skips silently; missing files for ENABLED maps still report. Verified headlessly by running the real `exportGameBundle` against the live workspace: zero missing entries (previously two).
- v4.79.79 — **UI placement grid goes 3×3 (user: "instead of 2 rows of 3, should [be] 3 rows of 3… able to put stuff in the center and left-center and right-center").** `UiAnchor` gains `middle-left/center/right`; `UI_ANCHORS` inserts the middle triple (the grid auto-flows 3 columns, so the picker just grows a row); `GameGuiOverlay.ANCHOR_STYLE` adds the middle branch — `top: calc(50% + oy)` with translateY(-50%) (translate(-50%,-50%) at true center), so Y offset shifts down-positive from center while the existing six anchors are byte-identical. Old scenes untouched. Verified live: the picker renders all 9 cells in order.
- v4.79.78 — **Per-game interact binding + the {interact} prompt token (user: "we need a top level UI area to configure what the interact key/button is, per-game… the text prompt would need to become more dynamic. Like Press {INTERACT}… always with the caveat that game-devs… or players… could always change the interact key").** `GameConfig.input.interact` ({kbm?: code[], gamepadButtons?: number[]}) edited from a new **GAME INPUT** section on the no-selection ToolView (press-a-key capture + named gamepad-button select + reset; immediate `writeGame()` per the registry precedent; mirrored onto `WorldState.gameInput` beside every gameUiElements assignment in App + runtime SceneRouter). Precedence in `resolveGameBindings()` (bindings.ts): a stored device binding that DIFFERS from stock = player-explicit and wins; else the game default; else stock — applied at both shells' Play start (PreviewController's ControlSchemeManager construction). Display: `interactDisplay(scheme, bindings)` + `prettyKey`/`GAMEPAD_BUTTON_NAMES`, resolved ONCE per scheme/binding change (App/RuntimeApp `useMemo` on previewScheme + gameInputRev — performant per user's ask) and passed as one string: GameGuiOverlay substitutes `{interact}` (case-insensitive) in Label text at render; PreviewHUD's pill prefix becomes `[<name>]` instead of the hardcoded [E]/[LB]. The ⌨ Prompt wizard and "+ New prompt label…" now default to "Press {interact}". Verified live: capture E→F wrote game.json immediately; wizard label carried the token; in preview the on-screen label rendered "Press F". Verification note: the first live pass froze the hidden tab mid-script (an unstubbed native dialog — window.alert/prompt now join confirm in the probe stubs); recovered via tab-close, residue cleaned from game.json. Out of scope (flagged): a player-facing rebind menu (none exists for any key today) and tokens for other actions.
- v4.79.77 — **Movers auto-maintain a per-entity `moving` state (user: "Adding a mover should… add a localized moving-state… on when moving, off when stopped and delayed, on when moving again. Other scripts can then target the state… to play a sound when moving").** MoverSystem entries track `wasMoving` and write `__ent.<id>.moving` (via the Phase 60 facade — gameState/entKey) on TRANSITIONS only: new `_subMoving()` mirrors `_slideU`'s phase math for dwell-awareness (spins: moving while running; once-mode: running == moving; looping slides: false during both dwell windows of the period); the just-finished branch and `_resetAll` (preview stop) write the final false. `registerEntityStateSchemas` registers the key ({type:boolean, default:false}) for mover-carrying entities so reset/Continue/on_state_equals semantics match authored keys. Editor surfacing: EntityStateSection shows a read-only dashed "moving · auto · bool" row (live ▶ value, tooltip explains targeting) on the object Scripts screen when `hasEnabledMover`; `entityStateKeys` suggests "moving" for mover entities in "Whose state" scopes; `resolvedKeyType` returns boolean for it (true/false/toggle pickers + typed conditions work). Runtime writes happen for ALL mover carriers (platforms/shapes included) though the UI scoping reaches objects, matching Phase 60. Verified live end-to-end in Chrome (manual `__movers.update` stepping, rAF-frozen tab): default false → true mid-move → false in dwell → true on return → correct period wrap → false on preview stop; UI row renders; throwaway cleaned.
- v4.79.76 — **Trigger volumes hear the interact press; the press-E object association is gone (user: "The trigger does the prompt, the script can touch the object if needed… now that scripts can reference objects it seems redundant" — correct; the object-interactable requirement existed only because volumes were deaf to E).** Runtime: CharacterController emits `character:interact-pressed` on every press (device-neutral — E / gamepad LB / touch); TriggerSystem (fresh instance per preview session — new `dispose()` releases the listener, called in PreviewController teardown) fans it out as `trigger:volume-interact` per occupied volume; ScriptEngine fires `on_interact` keyed on the volume, exactly like object interacts. The editor needed nothing: `on_interact` was already in TRIGGER_TYPES for any owner and the chip reads "when the player interacts with ★ this volume". ⌨ Prompt now creates a THIRD script on volumes — "On Press (interact)" seeded with hide-prompt — which IS "what press-E does"; scripts reference objects via targetId as ever. REMOVED wholesale (v4.79.72–74 machinery): `handleWireInteractTarget`, both press-E pickers, the onWireInteract threading through App/LeftPanel/ScriptPanel/PropertiesPanel, and the TargetCombobox/TargetOpt exports. Object INTERACTABLE remains for object-centric interactions (near+facing targeting, the [E] pill) but is no longer part of the prompt flow. Trigger-vs-action design note: an await-press ACTION would make scripts long-running (exit-cancellation semantics); the trigger type composes with conditions/oneShot for free. tsc/build-verified; live pass pending a browser.
- v4.79.75 — **The INTERACTABLE text field gets a label (user: "what does the interact textbox do? it's blank, and unlabeled").** It is `properties.interactLabel` — the text of the built-in in-range HUD pill ("[E] <text>" under the crosshair, default "Interact"; CharacterController `_rebuildInteractCache` → PreviewHUD). Now captioned "HUD HINT — [E] …" with tooltips on both caption and input; placeholder reads "Interact (default)". Distinct from any authored prompt Label element.
- v4.79.74 — **PRESS-E TARGET only shows once the volume has a prompt (user: "the press-e-target is always there even if I haven't added a prompt. does that make sense?" — it didn't).** The volume-section picker gates on `promptElementId`: a promptless volume contributes nothing to the wiring (its role IS "hide this prompt on press"), so the flow reads ⌨ Prompt → PRESS-E TARGET appears. The show_ui-card picker is untouched (it inherently has the element context).
- v4.79.73 — **Press-E pickers become searchable + inspectable (user: "'pick the object E acts on' is not searchable, and is not configurable on the actual UI options").** Both v4.79.72 plain `<select>`s (unusable over a 300-object zone) are replaced with the house `TargetCombobox` (now exported from ScriptPanel along with `TargetOpt`; PropertiesPanel imports it — no cycle, ScriptPanel imports nothing from PropertiesPanel). Both pickers now BIND to the currently-wired object (found by its on_interact containing hide_ui of the relevant prompt element), so the wiring is visible and re-editable rather than a fire-and-forget blank. The show_ui-card gate relaxes from `owner?.kind === "volume"` to `owner?.kind !== "object"` — level-tab show_ui scripts (ownerless) can wire too, and any not-rendering ambiguity from the owner prop is moot. tsc/build-verified; Chrome extension still disconnected (no live pass).
- v4.79.72 — **Press-E target picker (plan `~/.claude/plans/merry-riding-spark.md`, user: "pick an object from the ui or trigger or both, the object I pick becomes interactable, and I can set what it does").** New `App.handleWireInteractTarget({zoneId, objectId, promptElementId?})`: one journaled `updateObject` carries BOTH a full-spread `properties` (interactable: true, only when not already — WorldState shallow-merge hazard respected) and cloned `scripts` where the first `on_interact` is reused or created (`mk()` shape, NO trigger.targetId) with `hide_ui → the volume's prompt` prepended when missing; idempotent re-picks skip the transaction; navigation always runs — `selection:set` the object + `setLeftPanel("scripts")` + a one-tick-deferred `setScriptEditRequest` (ScriptPanel resolves edit requests against the current selection; its selection effect precedes the editRequest effect). Two entry points: a **PRESS-E TARGET** select in the trigger volume's Scripts section (Attach-select idiom minus the mover filter; value binds to the detected wired object via hide_ui-of-this-prompt matching) and a **Press-E object** select on show_ui action cards (volume-owned scripts only), threaded App → LeftPanel → ScriptPanel → ScriptEditor → ActionRow → ActionFields alongside the v4.79.68-71 callbacks. `interactLabel` auto-set deferred by design. tsc/build-verified; Chrome extension disconnected at ship time — the plan's 10-step live checklist is pending a browser (user click-through requested).
- v4.79.71 — **"↗ Configure <element>" jump on show_ui/hide_ui (user: "I meant link to that so we can configure it").** When the action's selected element exists, a button under the dropdown jumps straight to the SCRIPTS → UI tab (new `onOpenUiTab` threaded ScriptPanel → ScriptEditor → ActionRow → ActionFields, implemented as `setTab("ui")` at the top) — full element configuration (anchor, size, color) one click from the script, complementing v4.79.69's inline text field (labels only; the ↗ shows for every element kind). tsc/build-verified; Chrome extension still disconnected, no live pass.
- v4.79.70 — **UI-registry writes no longer vanish (user screenshot: show_ui stuck on "ui_24f75ffc (custom)" with no Prompt text field right after the ⌨ Prompt wizard).** Two compounding defects in `handleUiElementsChange`: (1) its shared `if (!world?.world) return;` guard sat ABOVE the project branch, so a transiently-null world config silently swallowed the registry write while callers (the v4.79.68 wizard, the UI tab) carried on — scripts ended up referencing an element that was never stored, rendering as "(custom)" and hiding the inline Prompt-text editor (which gates on the element existing); (2) even successful project-mode writes only armed isDirty — game.json wasn't written until Save, so a reload kept the write-through scene scripts but dropped the element (the exact dangling-def failure the prefab library fixed with its immediate `writeGame()`, whose precedent comment this now mirrors). Project branch now runs without the world guard (world only needed for the mirror field) and calls `store.writeGame()` immediately; the non-project transaction path keeps its guard. Verified by tsc/build; live repro wasn't possible (Chrome extension disconnected, user active in the shell window) — recovery for stranded scripts: re-pick "＋ New prompt label…" or delete + re-run the wizard.
- v4.79.69 — **Prompt text edits inline on the action card (user: "adding an enter/exit prompt is now 1 click … editing that prompt is still as many clicks away").** When a show_ui/hide_ui action's selected element is a **label**, a **Prompt text** field renders under the dropdown and edits the element itself (blur/Enter commit; same text everywhere it renders, per the title hint) — no UI-tab round trip. New `onUpdateUiElement(id, changes)` threaded beside `onCreateUiElement` (ScriptPanel → ScriptEditor → ActionRow → ActionFields), implemented at the top as an `onUiElementsChange` map-update, so persistence/dirty behave identically to every other registry edit. Verified by tsc/build only — the Chrome extension was disconnected at ship time; the field mirrors the house blur-commit idiom and the registry path live-verified in v4.79.68.
- v4.79.68 — **Press-prompt shortcuts (user: "propose … shortcuts to making a prompt ui linked to the script/object"; picked recommendation A + B).** (A) The show_ui/hide_ui action editor's element dropdown gains **＋ New prompt label…** — picking it creates a Label element in place (auto-named "Press prompt"/numbered, text "Press E", bottom-center, backdrop) via a new `onCreateUiElement` callback threaded ScriptPanel → ScriptEditor → ActionRow → ActionFields (dialogue/menu-option ActionRows don't thread it, so the entry hides there), and selects it in the action. (B) A **⌨ Prompt** button beside + Enter/+ Exit in the trigger volume's Scripts section (and next to + Add on the object Scripts screen) runs `App.handleAddPressPrompt`: creates a Label named "Prompt — <entity>", plus On Enter `show_ui` / On Exit `hide_ui` scripts wired to it (one transaction; registry write via the existing `handleUiElementsChange` path so project game.json + dirty state behave); objects additionally get `hide_ui` prepended to their `on_interact` script so the prompt vanishes when used (their enter/exit triggers still need a Sensor collider — button title says so). Verified live on a throwaway volume: one click produced the label + both scripts correctly targeted; the dropdown entry created + selected a second element. Prompt-C (implicit PROMPT field on entities) was rejected — it hides the mechanism outside the everything-is-a-script model.
- v4.79.67 — **Preview hop return no longer discards unsaved edits or the undo history (user: "when I return from the preview, recent changes are no longer there … if I press cmd+z there is nothing to undo").** The hop-return path (preview:stop with `routeReturnSceneRef` set) re-read the ORIGIN scene from disk (`store.loadScene(back)` → `handleLoadFromJSON`) — "non-destructive" for the visited scene but destructive for the editing scene: unsaved edits vanished and `world:loaded` → `history.clear()` wiped the undo stacks. Only surfaced now because fade-testing made preview hops routine. Fix: the FIRST hop snapshots the LIVE world (`toJSON()` + editor-camera pose stamped into metadata) and the undo/redo stacks (new `HistoryManager.capture()`/`restore()` — entries stay valid because the world is restored to the identical state they reference); the return path restores from the snapshot (disk stays the fallback), re-installs the stacks after world:loaded's clear, and arms `isDirty`. Also restores on a round trip back to the SAME scene id (the hop away had already reloaded it from disk mid-route). Verified live: unsaved checkpoint move survives preview → hop → return, canUndo stays true, undo reverts it.
- v4.79.66 — **Scene-hop fades actually render (user: "the fade still doesn't work in preview").** v4.79.64/65 emitted the fade-in and IMMEDIATELY started teardown — but `FadeOverlay`'s "in" animation needs a rendered frame (`requestAnimationFrame` flips opacity 0→1 into a CSS transition), and the synchronous zone teardown/rebuild blocks the main thread, so the transition never painted and every hop still read as a hard cut. Both hop paths (editor preview in App's `scene:load-request` listener; runtime `SceneRouter.go`) now `await` the fade duration (+50ms) after emitting fade-in, so the screen is fully covered before the world swaps; the held overlay then releases via the arrival fade-out. Verified: with a 1s fade the scene provably stays put past the 1s mark and hops after (hidden-tab probe), preview survives, stop restores the editing scene.
- v4.79.65 — **SCRIPTS → UI tab de-cramped: fully vertical, collapsible, sectioned cards (user: "horribly horizontally cramped. things could be more vertical, and with a collapse expand thing"; option B of `plans/mockups/ui-elements-prototype.html`).** `UiElementRow` no longer packs 3-4 `F` label+control grids per flex row (they overlapped at real panel widths, 280-600px): every field is its own full-width `F` row, grouped under tiny uppercase `UiSectionHead`s — Placement / Visibility / Binding (or Text / Image / Menu per kind) / Graphics. The anchor dropdown became **`UiAnchorGrid`**, a clickable 3×2 grid mirroring the six screen corners, with the X/Y offset pair and a small anchor caption beside it. Cards **collapse to a one-line header** — chevron, kind chip, name, muted `anchor · stateKey` summary, × — with expand state held in `UiElementsEditor` (`expandedIds`); existing cards start collapsed, + New opens the new card. The menu kind's option-list toggle renamed `optionsOpen` (its old `expanded` state now means the card). Verified in a shell tab on level_1: Coin Count / Hearts render collapsed with summaries, expanded Hearts shows all four sections with the three heart pickers stacked, anchor grid round-trips top-right → bottom-center → top-right through world state, no console errors.
- v4.79.64 — **Scene transitions get a configurable fade (user: "can scene transitions have a configurable fade? kind of like death-respawn transitions").** The `load_scene` action already carried the shared `fadeColor?`/`fadeDuration?` ScriptAction fields (used by respawn/fade_screen/despawn) but never forwarded them — `SceneRouter.go()` hardcoded black 0.3s in/out. Now: ScriptEngine forwards both on `scene:load-request` (event payload extended), `SceneRouter.go()` accepts `fadeColor`/`fadeDuration` opts and uses them for the fade-in AND the arrival fade-out (defaults preserve the classic black 0.3s; the error-path 0s fade-out stays), and ScriptPanel's load_scene editor gains the Fade color + Seconds row (both branches — scene dropdown and free-text id) with the fade in the action summary (`level_2 · 1.5s fade`). Follow-up in the same version arc (v4.79.65, user: "it doesn't work in editor preview"): the EDITOR's preview hop (App's scene:load-request listener — a later feature that superseded the stale "deliberate no-op" comments) never faded at all; it now emits the same configurable overlay fade-in before teardown and fade-out after preview re-enter (0s on error so the overlay can't strand), and the stale no-op comments in ScriptEngine/types are corrected.
- v4.79.63 — **Undo finally moves checkpoint markers (user: group-move a volume+checkpoint prefab, undo → "only the trigger-volume moves back. the checkpoint stays").** The long-documented gap: `WorldState._emitChange` (journal replay → bus events, how every system hears about undo/redo) had cases for floors through prefabInstances but never `checkpoint` — undo restored `zone.checkpoints` data silently, so `CheckpointTool`'s markers (which rebuild on `checkpoint:added/updated/removed`) stayed at the pre-undo pose. Added the standard three-way case. Verified live: journaled move (+5x) → marker follows; `__history.undo()` → data AND marker both back at the original position. This closes the last checkpoint second-class-citizen gap from the v4.79.59–62 arc.
- v4.79.62 — **Volume clicks select the whole prefab instance + the Prefab section lists its pieces (user: "clicking the volume can select all the prefab. Also, in the overall-prefab-properties, it should list the items in the prefab, and let you select them individually" — reversing the earlier keep-single decision).** `TriggerVolumeTool`'s Select-tool click on a prefab-stamped volume now gathers every member (all six PREFABABLE kinds) and emits `selection:set` — Phase 47.1 parity with SelectionManager's click-expansion, which volumes never had because their picking bypasses SelectionManager (`selectable:false`). Shift-click keeps toggling just the volume, and the Trigger tool keeps single-select (it edits volumes). `PrefabSection` gains `members`/`onSelectMember`: the whole-instance view shows a **PIECES (N)** list (type · label rows, labels resolved across the six member collections) and a row click emits `selection:set` with that one ref — `_setSelection` resolves volume and checkpoint meshes fine (both carry editorId userData). Verified through the REAL tool path (bus `input:mousemove`+`input:click` at the volume's projected screen pos → `_findVolumeAt` raycast): click → "2 selected · ⬡Prefab · Checkpoint Test", PIECES rows render, checkpoint row click → single CHECKPOINT view.
- v4.79.61 — **Checkpoint prefab members now ride every selection/move/copy path (user: group-dragging a volume+checkpoint prefab "seems to only move the trigger volume" — persisted past v4.79.60 because the fix only covered `_translateRef`).** Sweep of every remaining five-kind enumeration: `SelectionManager._instanceMemberRefs` gathers checkpoints (THE symptom — click-expansion never put the checkpoint in the group, so the drag had nothing to move), `GizmoManager._refDisplayPos` (group pivot centroid) + `_syncPrefabOrigin` (stamp lookup AND member count — origin now moves with a checkpoint-bearing full-set drag instead of snapping members back on next re-expand), `App.collectInstanceStamps` (empty-instance pruning on delete), `App.selPrefabInfo` member-set match (full-set selection shows the Prefab section again), `App.refreshSelectionAfterReexpand`, `groupMembers.COLLECTIONS` (Groups panel/rows see checkpoints — CheckpointDef always had groupIds), and `editor/copyPaste` (COPYABLE + id mint `cp_` + clipboard build + paste case, so Cmd+C/V/D clones checkpoints). Verified live: full-set selection:set renders "2 selected · 1 trigger-volume · 1 checkpoint · ⬡Prefab", Cmd+D duplicates both members; canvas click-expansion is untestable from a hidden tab (TESTING.md lesson) — it is a one-line mirror of five proven siblings. Lesson: adding a kind to PREFABABLE requires sweeping ALL per-kind switches (`grep -n ladders`), not just the pipeline file.
- v4.79.59 — **Checkpoints join the prefab pipeline (user: "do prefabs have the ability to save or contain or be created with checkpoints?" → "yes" build it).** `PREFABABLE` gains `"checkpoint"` — every kind-switch in the pipeline gets the sixth case: `newMemberId` (`cp_`), both capture `lookup`s, `materializeMembers` (position `toWorld` + `facingDeg += origin.rotationY`), `addMember`/`removeMember` (the existing journaled `add/removeCheckpoint` mutators — markers rebuild off their bus events), `collectInstanceMembers` scan, `unlinkInstance` update, and `captureInstanceToPrefab` (`toLocal` + `facingDeg -=`). `CheckpointDef` gains `prefab?: PrefabStamp`. `PrefabEditSession` stages checkpoints into the temp zone and `grab`s them on save — a checkpoint placed in the prefab editor now survives instead of silently evaporating. `App.PREFABABLE_TYPES` + the selection hint updated. Checkpoints are passive id-targeted respawn destinations (`ScriptEngine._resolveObjectPose`), so `remapScripts` gives each instance its OWN checkpoint for free — the kill-floor-segment case. `anchorPoints`/`shiftAnchors` already handle any `position`-bearing def. Verified live end-to-end: capture (checkpoint + volume → template at local coords, facing preserved, originals replaced by stamped fresh-id copies), prefab-editor round trip (staged + survives Save, version bump), and instance origin ROT 90° → re-expanded checkpoint swung around the pivot with facing 90→180. Known inherited wart: the checkpoint undo marker-staleness gap now also covers undone prefab ops containing checkpoints.
- v4.79.58 — **Phase 69 — trigger-volume properties reorganized into option-A accordions (user: trigger properties "feel sloppy and cluttered"; prototyped as `plans/mockups/trigger-panel-prototype.html`, user picked A of A/B/C).** `TriggerVolumeView`'s flat 12-block column becomes six collapsible sections — **Geometry** (label + ⓘ intro, shape/edit-mode segs, position, size, rotation), **Scripts** (+Enter/+Exit, ScriptListRows), **State** (EntityStateSection), **Attach**, **Appearance** (VISUAL + EDITOR SHADING), **Groups** (the existing `GroupsAccordion`, unchanged wiring) — via a new local `Section` component (ROW_BASE header matching GroupsAccordion + a right-aligned mono summary shown while collapsed: `box · 122.2×2.5×137.2 · 45°`, `2 scripts`, key names, host label/`static`, `gradient`/`custom color`). Geometry + Scripts start open. ⬡ Create Prefab + Delete Volume stay pinned below. **Rotation row**: ~1/3-width text input (controlled `rotStr`, resyncs on reselect + external gizmo rotation) + a −180…180 step-5 slider + an in-field × reset (dimmed at 0°), all committing through the shared debounce. **EntityStateSection restyle** (shared — the object Scripts screen inherits it): heading + ⓘ toggle (new `InfoBlurb`; the explainer only renders when pressed, at 11px), each key a card with name · live ▶ value · bool/num/text segmented type picker · ×, STARTS on its own line (checkbox for booleans), numbers put MIN + MAX on the line below; gains `heading?` prop (trigger view passes "THIS VOLUME ONLY" since its Section is titled State). No data-model changes. Verified in-browser against the live shell (Platfrom Obby): all six sections + summaries, slider/reset, ⓘ toggles, spike-cube state card. Plan: `plans/phase-69-trigger-panel-redesign.md`; acceptance: `test-plans/phase-69-trigger-panel.md`.
- v4.79.57 — **Trigger volumes get "⬡ Create Prefab" in their properties (user: "I have a trigger volume that has its own configs and state management… I can't make it into a prefab?").** Volumes were always capturable (Prefabs panel → Create from selection), but `TriggerVolumeView` — its own component — never got the affordance single objects have in their Actions accordion. A blue ⬡ Create Prefab button now sits above Delete Volume (hidden when the volume is already a prefab member — capture of members stays blocked with the unlink-first hint). Captures everything the def carries: size, scripts, state schema/keys, attach-to.
- v4.79.56 — **Phase 68 Part 2: lighting + audio mixer join the game-defaults model.** `GameConfig.lighting?` (ambient/sun/env/skybox/fog) and `GameConfig.audio?.mix`; a scene follows game lighting via `WorldConfig.lightingFromGame` (resolved into the scene's fields at load / on game edits, with `world:lighting`+`world:sky` re-emits), and the mixer by PRESENCE (a scene `audio.mix` wins; absent = the game's). Ownership is implicit-on-edit: touching any lighting value / mixer slider takes it over for the scene (matching how those pages always saved); the Lights and Mixer pages get an `InheritBanner` — amber "This scene overrides the game's lighting/mixer levels" with **Use game defaults** and **★ Make this the game default**, grey "Following the game's … — editing any value overrides it". Seeding folds into a shared `seedAndApplyGameDefaults` (entry scene → game.json for settings, lighting, and mix; adopt + boot-restore + scene switch all apply the layers; close clears them). `makeFreshScene` now writes an EMPTY settings-override layer and `lightingFromGame: true` — the Part 1 gap that would have made new scenes read fully overridden. Runtime SceneRouter applies the same layers; export ships the game-default skybox. WorldState: `setGameLighting`/`setGameAudioMix`/`inheritLighting`/`inheritAudioMix`/`promoteLightingToGame`/`promoteAudioMixToGame`; toJSON writes the scene's mix only when the scene owns it. SceneRouter's game `lightingQuality` path is exempt from the implicit-ownership rule (quality-only changes don't take over).
- v4.79.55 — **Phase 68 Part 1: game-wide player-settings defaults with per-scene, per-page overrides (plan `plans/phase-68-game-defaults.md`).** Data: `GameConfig.playerSettings?` (complete defaults); a scene's `world.playerSettings` is now a SPARSE override layer on disk — legacy full-settings scenes read as fully overridden, byte-identical behaviour. New `src/shared/playerSettingsDefaults.ts`: `DEFAULT_PLAYER_SETTINGS` (replaces four repeated literals in WorldState), `SETTINGS_PAGES` (movement / camera / character / sounds field groups — Controls is device-local), `pageOverridden`, `resolvePlayerSettings` (defaults ← game ← overrides, per field). WorldState: `gamePlayerSettings` + `scenePlayerOverrides`; in memory `world.playerSettings` is ALWAYS the resolved merge (PreviewController/BagOverlay/panel/runtime consumers untouched); `loadFromJSON` captures the file's layer + resolves, `toJSON` persists the sparse layer; mutators `updateScenePlayerSettings`, `setSettingsPageOverride` (ON copies the page's effective values in — nothing visibly changes; OFF deletes them — snap to game defaults), `promoteSettingsToGame`. App: adopting a project SEEDS missing game defaults from the ENTRY scene (immediate game.json write) and hands them to the world (also on every scene switch; closing a project clears the layer); scene-scope edits route through the override layer; new handlers for game-scope edits / page override / promote. Editor: `SpawnSettingsScoped` wraps the spawn settings view — a THIS SCENE ⇄ GAME DEFAULTS scope switch on the root, a per-page banner ("Using the game defaults … Override for this scene" / "Overriding … Use game defaults", non-overridden pages read-only), and **★ Make these the game defaults** on the scene root; single-scene mode (no project) renders exactly as before. Runtime `SceneRouter` resolves the same way from the manifest's game.json; export `assetRefs` collects the game-default character model/sounds. Deviation from the plan: game scope is reached via the in-panel scope switch (more discoverable); the TopBar ⋯ "Game settings…" entry can follow. Part 2 (lighting + audio mixer, same mechanism) is next.
- v4.79.54 — **Motion is its own properties page (user: "move motion outside of geometry … in it's own page").** New `motion` ScreenId ("SLIDE · SPIN · COMPOSED") with a `MotionScreen` drilldown for objects, platforms (polygon slabs show the can't-animate note), and shapes; the category row's summary reads the composed set ("spin Y + slide X", "none", "3 motions"). The MoverSection embeds at the bottom of the three Geometry views are gone.
- v4.79.53 — **Phase 67: multiple movers per entity (user: "what if an object could have 2 movers. like one to spin and one to bob up and down"). Plan `plans/phase-67-multi-movers.md`.** Data: `movers?: MoverDef[]` joins the legacy `mover?` on WorldObject/PlatformDef/ShapeDef (legacy read forever, written never again — the Motion section migrates on first edit); `MoverDef.id?` (`mvr_<uuid8>`, editor-assigned) lets scripts target one mover; `ScriptAction.moverId?` + `mover:set` gains `moverId?`. New `src/world/moverDefs.ts` (`entityMovers`/`enabledMovers`/`hasEnabledMover`) replaces every `x.mover?.enabled` gate (ShapeBuilder/PlatformBuilder kinematic-body creation, ZoneManager's three sync paths + rebuild trigger + AI-host branch, InstancedObjectPool exclusion, PropertiesPanel attach-to host filters). Runtime: `MoverSystem` entries hold `subs: MoverSub[]` — one clock per mover (own duration/dwell/phase/mode/autoStart) — and `_applyPose` composes ONE pose per frame: slide offsets SUM in local space (rotated once), spin quaternions MULTIPLY in list order; a paused sub keeps contributing its frozen offset/angle. Kinematic body pose, mesh orbiting, carry delta, attach-to riders all consume the composed pose unchanged. `mover:set` without `moverId` = every sub (pre-67 behaviour); with = that sub (per-sub once-slide toggle rule). AI-host entries register `subs: []` — the floating-crab class is now impossible by construction. Editor: MOTION is a list (`+ motion`, per-mover enable/×, auto-label "SPIN Y · #2"); commits always write `movers` with ids and clear `mover`. Script actions start/stop/toggle_mover show a "Which mover" select when the resolved target has 2+ enabled movers. Verified live on the real system: spin-Y 90°/s + bob-Y 2m/2s composed exactly (t=0.5 → +0.293m eased & 45°; t=1 → +1m & 90°), `stop` with `moverId` froze the bob while the spin advanced, stop-all froze both, `preview:stop` reset bit-exact; a legacy single-`mover` object still registers and moves untouched.
- v4.79.52 — **Save-state pill beside Save (user: "move the saved indicator over by the save button inside of a little circular pill shape outline similar to option b, while still preserving a save button").** The dot + "saved Nm ago" moved from the breadcrumb to a rounded pill immediately left of the Save button (option B's pill, keeping A's button): green dot + "saved 2m ago" at rest, amber outline/fill + "Unsaved" when dirty; clicking the pill saves too. Save stays as the explicit button.
- v4.79.51 — **Top bar rebuilt (direction F from `plans/mockups/topbar-prototype.html`: command bar + explicit Save + labelled Top; user: "I need 5 improved interactive design options for this top bar because, damn").** `TopBar.tsx` rewritten around the same props/handlers. Left: an "S" mark (brand in its tooltip), then a **breadcrumb** — the project name (click → project menu: New project… / Open project… / Export game… / Close project; reads "no project" with New/Open when none) › the **scene** (name + ★ when it is the entry scene + ▾; click → scene menu: every scene with ★ entry marker and "current", New scene…, Make this the entry scene, Delete scene… with the same entry/only-scene guard) — then a **save-state** dot + text (green "saved 24m ago" from the autosave clock; amber "unsaved changes" when dirty). Right: **Floor G ▾** menu (Ground / Floor 1–3) replacing the four boxed squares, **Top** (icon + label, tooltip "Top-down view"), undo / redo as ghost icons, help, the desktop-only reload icon, then **Save** (turns amber when dirty), the filled green **Play** (project only) and **⋯** (Entry scene select, Export game…, New empty world, Load scene JSON…, Close project). `Popover` gained `align="right"`. Inline stroke icons (`Ic`), 28px controls, 8px gap; still 48px tall, still monospace apart from the Play label. Behaviour preserved: every handler is wired exactly as before (the JSON `Load` file input is now behind ⋯).
- v4.79.50 — **Prefab ⋯ menu lists every instance action (user: "the little 3 dot menu at the top of prefabs doesn't include every prefab option like edit-prefab").** `PropertiesPanel` header menu adds **Edit prefab** (snapshot kind, same handler as the ✎ pencil) and **Select all N pieces** (same as the "all N" button, shown for >1 piece) ahead of Reset / Save to prefab / Unlink / Delete instance. The pencil and "all N" remain as shortcuts; the menu is the complete list.
- v4.79.49 — **Phase 66: script editor as cards with property rows (chosen from the A–G prototype: direction E + the "properties panel" field style; plan `plans/phase-66-script-editor-redesign.md`, prototype `plans/mockups/script-editor-prototype.html`).** View-only rework of `ScriptPanel.tsx`'s editor — no data or engine changes. (1) **Property rows**: the `F` label+control wrapper (which every one of the 37 action forms, the condition form and the trigger form is built from) is now a full-width row — sentence-case label left, control right, hairline under — so every form got the new look with zero per-action rewrites; pickers (TargetCombobox, SoundPicker ▶/VOL, StateScopePicker, KeySuggestInput, ItemPicker…) untouched inside. `S.field`/`S.select` softened (translucent fill, 6px radius); native checkboxes → `.wb-switch` sliding switches and a `RangeField` slider+number for volume-like values (action delay, trigger delay, animation blend) via two rules in `index.html`. (2) **Action cards** (`ActionRow`): icon tile per family (sound / state / player / items / flow / objects; inline SVG `Ic`), title = the thing (`describeAction` resolves ids to labels: objects/volumes/groups/lights, sounds via `assetManager.getSoundDef`, dialogues, items, UI elements), subtitle = verb · parameters · `after Ns`; ⋯ `CardMenu` with wrap-in-if / move-to (listed by condition text) / duplicate / delete; click → the property rows (Action type select with plain-language `ACTION_LABELS`, the type's `ActionFields`, an After slider). The editor keeps ONE card open (`openAction` index); newly added/duplicated cards open. (3) **If-blocks as grouped cards** (`IfBlockCard`): amber header strip per branch (IF / ELSE IF / ELSE) with the condition sentences inline, `+ and`, `×` branch; the branch's cards + a dashed `+ action`; footer `+ else if / + else / unwrap / delete block` (delete removes the block AND its actions — unwrap keeps them). (4) **Conditions as sentences** (`ConditionRow` → `describeCondition`): click to open property rows ordered Whose state → State key → Condition → Equals/Compare/Item → Unless switch, with remove/done; blank new conditions open at once. (5) **Trigger hero card** replaces the LABEL/TRIGGER/CONDITIONS form stacks: icon, the trigger as a sentence (`TRIGGER_LABELS` + resolved target / state scope / dialogue label), "on ★ owner", chips — `timer repeats every Ns` / `runs each time it fires` toggle in place, `no delay` and ⋯ open the trigger form (rows: type, target, interval, repeat switch, delay slider, one-shot switch); script-level conditions read as an `only when …` line with `+ condition`; the script label is the editable header title. Dashed `+ action` / `+ if-block` cards at the end of the list replace the header buttons. Dialogue-option and UI-menu editors share `ConditionRow`/`ActionRow` and inherit the cards. Popup fix surfaced by the review: `TargetCombobox` and `KeySuggestInput` popups closed on the input's blur, so pressing the mouse on their SCROLLBAR dismissed them ("I can search but I want scroll") — any mouse-down inside a popup now `preventDefault`s. Verified live on the user's spike script (names resolve, cards/blocks/conditions/trigger form open and edit) — screenshots per step.
- v4.79.48 — **Left-panel resize can no longer get stuck in drag (user: "after I let go … it stays like in drag mode. If I move my mouse back into that left side panel then it starts collapsing it") + set_state VALUE on its own line.** `LeftPanel`'s edge handle relied on pointer capture delivering `pointerup` to the 6px handle; a release the shell never delivered to it (outside the window, or a cancelled pointer) left `resizing` true, and since the handle sits on the panel's edge, every later hover dragged the edge along. Now the drag is tracked on WINDOW while resizing (`pointermove` → width) and ends on ANY `pointerup` / `pointercancel` (capture phase) or window `blur`; the handle only starts it. Width still persists to `wb_leftpanel_w` on end. Also: the `set_state` action's VALUE field (`F flex="1 1 100%"`) takes its own full-width line — at the panel's default width the three fields (WHOSE STATE / STATE KEY / VALUE) overlapped and clipped the value (pre-existing since the KeySuggestInput min-width, v4.79.38; surfaced by the Phase 65 legibility pass).
- v4.79.47 — **Phase 65: script if-blocks — shared conditions, else / else-if (user: "what if script action-if-blocks could have 2 or more actions… Also I want an else/else-if, in addition to an unless").** Plan: `plans/phase-65-script-if-blocks.md`. Data: `actions[]` stays FLAT; `ScriptDef.blocks?: ScriptIfBlock[]` (`{ id: blk_<uuid8>, branches: [{conditions}], else? }`) and `ScriptAction.block?: { id, branch }` (−1 = else) tag actions into a block — every existing `actions[]` consumer (export asset refs, WorldLoader, InstancedObjectPool, DialogueRunner, prefab target remap) is untouched and cannot miss a nested action. Engine: exported pure `selectBlockActions(script, ownerId)` picks each block's branch ONCE (first passing branch; else `else`; else nothing; a tag to a missing block = untagged, lenient) and `_runActions` dispatches the result — i.e. after the trigger delay, before per-action delays. `_ownedScript` rewrites "self" in block conditions; legacy `action.conditions` still evaluate in `_dispatch`. `remapScripts` remaps block-condition `entityId`/`npcId` (prefabs, capture, copy/paste). Editor: `migrateActionGuards` shows legacy per-action guards as one-branch blocks and persists the new shape on the first edit (identity otherwise); the Actions section renders plain rows and `IfBlockCard`s in order of first appearance (empty blocks last); card = IF / ELSE IF #n.m / ELSE labels, the same `ConditionRow`s + scope as script-level conditions, `+ condition`, that branch's actions, `+ action here`, `× branch`, footer `+ else if` / `+ else` / `unwrap` (never deletes actions); `ActionRow`'s `if` now WRAPS the action, a `move to` select relocates it; the ONLY IF sub-block UI is retired; list summary `· N if`; `harvestRefs` includes block (and legacy) conditions. Verified live: engine — {if a:[x,y], else if b:[z], else:[w]} dispatched exactly the winning branch per state, no-else all-fail ran nothing, `unless` inverted, owner threaded; editor — + If, + action here ×2, wrap, + else if, + else, move-to ELSE, unwrap, legacy guard shown as IF #2 and persisted as a block on the first edit, Delete cleaned up (scene byte-identical). Probe notes: the script Delete button is a native confirm (stub `window.confirm` in hidden tabs); `IfBlockCard`s can't be found by inline `style.border` text (computed rgba spacing differs) — use button document order.
- v4.79.46 — **Save to prefab — the reverse of Reset from prefab (user: "if I edit an instance of a prefab, it doesn't persist those changes back to the prefab… What if there was a button in the prefab options to do that?").** New `captureInstanceToPrefab(world, zoneId, prefab, instanceId)` in `src/prefab/expand.ts` (pure): clones every stamped member of the instance, moves it back into template space with the new `toLocal` (exact inverse of `toWorld`: translate by −origin, un-rotate by −yaw; member rotation.y / ladder rotationY minus the origin yaw; values tidied to 1e-6 so 90° yaws don't persist 1e-16 noise), strips the stamp, sets `def.id = memberKey` and keys the template entry by the member's EXISTING memberKey — so `reexpandInstance` keeps entity ids on every other instance (external refs, groups, selection survive). Intra-instance script targets and `attachTo` hosts remap world id → memberKey (the template id convention `materializeMembers` remaps back through); external refs pass through. Template order follows the old template; members deleted from the instance are dropped and reported as `removedKeys`; version+1. Snapshot kind only (generators are parametric — returns null). App `handlePrefabPushToPrefab`: persists via `applyPrefabs` (game.json immediate write, not in the undo journal — same as Edit prefab → Save), then re-expands EVERY instance (source included — content no-op, version stamp current) in ONE transaction under `withInstanceReselect`. UI: **Save to prefab** (amber, snapshot only) in the header ⋯ menu and the Prefab section beside Edit prefab; the ConfirmDialog body is built from a dry run of the capture: `Overwrite prefab "X" (v1 → v2) with this instance's N pieces, and update the K other placed instances to match? R pieces you deleted here will be removed from the prefab and from every instance. …` Verified live end-to-end through the real UI on a throwaway prefab: instance B placed rotated 90° (R-key path), one member moved/renamed + one deleted, Save → template = (−3, 0.5, 0) rot 0 exactly as hand-computed; instance A (yaw 0) got the same member id moved to origin+(−3,0.5,0), the rename, and lost the deleted piece; the rotated source round-tripped bit-exact; selection survived; undo restored both instances' pieces (def stays v2), redo re-applied. v1 scope: added (unstamped) entities near an instance are NOT members and don't come along. Probe notes: prefab placement in a hidden tab must be driven over the bus (`input:click` with worldPos; the frame-loop raycast is frozen); a scene-tab × uses a native confirm — never regex-click "×" near the Prefabs panel.
- v4.79.45 — **Esc closes editor modals (user: "ESC should still close modals").** Companion to v4.79.43 (backdrop click removed). New `src/ui/useEscapeClose.ts`: modals register their close fn in a module stack on mount; ONE window `keydown` listener (capture phase, `stopImmediatePropagation`) closes the TOPMOST entry — so a confirm stacked over another modal closes alone, and the editor's own Escape handling (App:1867 preview exit / tool disarm, drag cancels via `input:keydown`) does not also fire while a modal is up. Listener detaches when the stack empties, so Esc with nothing open reaches the editor exactly as before. Wired into the same 14 modals as v4.79.43, each calling the SAME fn as its ✕/Cancel (`SoundPickerModal` uses `finish`, which commits multi-picks — Esc = close = commit, matching its documented every-way-out rule). Verified live: Esc closes the recorder with 0 keydowns reaching a bubble-phase window listener; after close, Esc reaches it (1). Probe note: the Chrome extension's key press does not reach a hidden tab — dispatch a KeyboardEvent on `document.activeElement` instead.
- v4.79.44 — **Recorder trim ▲/▼ steppers in 0.1s (user: "can the trim start/end have the up down arrows for 0.1 second intervals? without breaking the ability to type and parse the number as has happened in the past").** New `TrimField` in `SoundRecorderModal`: the plain text input is UNCHANGED (typing "0.45" still round-trips), with a stacked ▲/▼ button pair beside it and ArrowUp/ArrowDown on the input, each stepping the PARSED value ±0.1 (rounded to 0.1, clamped ≥0) and writing the result back as a string. Deliberately not a native `type=number` spinner — that is what ate the "." mid-decimal (v4.79.36). Steps are functional updates (`patchTrim(id, key, prev => …)`), caught by the probe: three same-tick ▲ clicks first gave 0.1 (closure-stale value); now 0.3 — matters for click-repeat and held keys. Verified live: ▲▲▲ → 0.3, ▼ → 0.2, typed 0.45 kept, ArrowUp → 0.6, ▼ below 0 clamps, duration readout tracks each.
- v4.79.43 — **Editor modals no longer close on a backdrop click (user: "can we prevent closing this modal, or any I guess, when merely clicking outside of it").** Every editor overlay had `onClick={e => { if (e.target === e.currentTarget) close(); }}` on its backdrop — one stray click outside the card threw away in-progress work (a recorded take, importer metadata, a re-origin setup). Removed from all 14: SoundRecorderModal, AudioImporterModal, ModelImporterModal, GraphicsImporterModal, MaterialImporterModal, SkyboxImporterModal, SoundPickerModal, ReoriginModal, ThumbnailStagerModal, EditMetadataDialog, DeleteAssetDialog, ConfirmDialog, ScriptDetachDialog, PrefabInstancesDialog. Each already has an explicit Cancel/✕/Done control (checked before removing), so nothing became a trap. House rule going forward: editor modals close ONLY via their own buttons (or Esc where a modal handles it). Deliberately untouched: the in-game `BagOverlay` (gameplay HUD — click-anywhere/confirm-to-close is its intended controller-friendly behavior).
- v4.79.42 — **Recorder takes: per-row trim + waveform (user: "the 2 play buttons for my 2 recordings seem to play the same recording. Also there is only one trim set instead of one for each recording").** Two changes to the take rows. (1) **Trim fields live on every row** (TRIM START / END under each take) instead of one shared pair bound to the picked take — each take always had its own stored trim, but with one visible field pair that wasn't legible. The bottom of the modal keeps only NAME. (2) **A per-take waveform** (64-bucket normalized peak envelope, `computePeaks` once at decode; 120×18 SVG) sits in each row, with the trimmed-off fractions shaded — so two takes are visibly different audio and the trim is visible as a cut, not just a number. On the "same recording" report: verified in Chrome with an async recorder stub that each row's ▶ plays its own take (fingerprinted the buffer handed to `createBufferSource`: 440 Hz vs 1760 Hz), and the take/decode path has no shared buffer — so if two real-mic takes still sound identical, their waveforms will now show whether the CAPTURE produced identical audio (a CEF/MediaRecorder-side issue) or not. Hidden-tab probe note: a stub that fires `dataavailable`/`onstop` via setTimeout gets the ~1s timer throttle — poll for the button instead of fixed sleeps.
- v4.79.41 — **Multi-take sound recorder (user: "what if the recording modal let you add multiple recordings. that you could preview and trim and eventually pick one to import").** `SoundRecorderModal` reworked from single-buffer to a **takes list**: every recording appends a `Take { id, buf, trimStartStr, trimEndStr }` row (● Record → ● Record another; newest take starts picked). Each row shows a ●/○ pick radio, "Take N", its trimmed duration ("0.4s (of 0.6s)"), a per-row ▶/⏹ preview (plays THAT take with ITS trim, without changing the pick), and ✕ delete (pick falls back to the last remaining take; last delete returns to the empty state). The TRIM START/END fields below edit the picked take and **each take remembers its own trim** — switching the pick swaps the fields to that take's stored values (v4.79.40's string-state per-keystroke pattern, now per take). Import → sends exactly the picked take at its trim to the import pipeline; disabled while recording. Verified live (stubbed MediaRecorder handing synthesized sine WAVs, shipped code downstream): two takes 1.0s/0.6s, independent trims (0.2 vs 0.1/0.1) survive pick-switching, cross-row preview leaves the pick alone, delete falls back correctly, and the handed-off `multitake_test.wav` decoded to exactly 0.800s (picked take minus its trim). Physical mic capture unchanged. Probe gotcha for future sessions: the PANEL's ● Record and the MODAL's ● Record share a label — scope DOM lookups to the zIndex-60 overlay.
- v4.79.40 — **Recorder trim applies as you type (user: "the trim is only applied on-blur from the field, so it does not apply when you change the value and then immediately try to play it… not cool").** The v4.79.36 review pass moved the trim fields to the house defaultValue+onBlur number pattern (to stop a controlled `type=number` from eating the "." in "0.5"), which traded one papercut for another: type a trim, click ▶ immediately → stale value (a button click doesn't reliably blur the field in CEF/macOS). Fixed with the third pattern: trim fields are now **string state on `inputMode="decimal"` text inputs** — fully controlled (commits every keystroke, `stopPlayback()` per change), decimal-safe (the raw string round-trips, no badInput ""), with `trimStart`/`trimEnd` derived per render via `parseFloat`+clamp≥0. ▶, the live duration readout, and Import all read the derived values, so all three see the current text with no blur required.
- v4.79.39 — **Entity scopes suggest ONLY their own keys (user: "I shouldn't see the global hearts on a certain whose-object. conversely, other objects should not list state-keys specific to other objects").** `entityStateKeys` returned `[]` for an entity with no registered keys, and every consumer treated empty as "fall back to the global list" — so "★ this object" on a schema-less entity suggested Hearts/coins. The function now returns UNDEFINED only for the global scope and a possibly-empty exact array for any entity/group scope; `KeySuggestInput` falls back to the global list only on undefined (an empty array = no popup). Verified on the user's live spike script: the cube-scoped guard key field (cube has no schema) now offers nothing; the sibling-volume scope still offers exactly `spikes-up`; global scope unchanged. Cross-entity leakage was never possible (suggestions always came from the resolved scope) — the lie was global-into-entity.
- v4.79.38 — **`state_equals` condition + plain-language condition labels (user: "it can only do if has-state? … can't check the state's value? if state equals").** Two gaps behind the complaint: `has_state` is truthy-only and `compare_number` coerces to numbers — no condition could check a boolean or string VALUE at all. New `state_equals` ConditionType: passes when the key's current value equals the authored one via `jsonEquals` (extracted from `_fireStateEquals`, so the condition and the `on_state_equals` trigger share one equality: strict primitives, JSON-deep objects); `unless` gives not-equals. UI: key field with scoped suggestions + an Equals field that renders a true/false picker when the key is registered boolean (`ConditionScope` gains `stateKeyTypes`; entity schemas first, merged global schema for global scope). The condition dropdown also drops raw type strings for labels ("state is set / true", "state equals value", "number compare (< > =)") — the old names undersold what existed. Prefab remap covers it (generic entityId pass). Runtime verified via real dispatch: boolean + string equality both polarities, unless inversion; UI screenshot-verified.
- v4.79.37 — **Per-action conditions + "unless" (user: "can we add to script actions -conditions? … maybe if a state is this, then do this. Idk if we need an else option").** Data: `ScriptAction.conditions?: ScriptCondition[]` and `ScriptCondition.not?: boolean`. **Engine**: `checkScriptConditions` refactored to a per-condition `passes()` verdict with `not` inversion ("unless" — the condition must FAIL; unresolvable `self` still fails closed regardless); the guard sits at the TOP of `_dispatch`, which the per-action delay's setTimeout wraps — so a delayed action reads the world when it actually fires, by construction. `not` works everywhere conditions exist (script guards, dialogue options, GUI menus) since the evaluator is shared. **Design choice — no else/else-if structure**: "else" = a second action with the mirrored (`unless`) condition — flat UI, no nesting for prefab capture to recurse into; a branch-action can come later if real levels grow multi-way branches. **Prefab capture**: `remapScripts` now remaps condition `entityId` at BOTH levels (script + per-action) — also fixing a pre-existing gap where script-guard `entityId`s weren't remapped at all. **UI**: an **if** button on each action row (amber once guarded) appends a condition; guards render in an amber-bordered "ONLY IF (all must pass — checked after the delay)" block reusing `ConditionRow`, which gains the **unless** checkbox. Verified live: guard blocks on false / runs on true, `unless` inverts both ways (via `__test.runAction` → the real `_dispatch`); the harness's runAction bypasses `runActions`' delay scheduling, so after-delay evaluation is asserted structurally, not probed. Sequencing ("run in order" + `wait`) deliberately deferred to its own plan. Guide: OBJECT_SCRIPTS_GUIDE.md.
- v4.79.36 — **Review-pass fixes across v4.79.25–35 (user: "is all the code around all of this good? of good quality and performant?").** Two parallel review agents swept the ~1,100-line diff; six real findings, all fixed: **(1) EnemyAI walk-loop leaks** — the script-clip-freeze and missing-mover-entry `continue`s bailed before the hysteresis block, leaving the keyed loop playing over a frozen/deleted enemy; both now `_stopWalkSound` first. **(2) Keyed stop-vs-first-load race (AudioSystem)** — `audio:stop` before the async `_makeSound` resolved was a no-op, then the loaded loop registered and played until preview end (unreachable — the next play overwrote the key). New `_keyGen` per-key generation counter: play captures a generation, stop/re-play bumps it, and a stale registration disposes the sound on arrival (probe-verified: play+immediate-stop on a cold buffer → nothing registers, `_all` unchanged; control play registers/stops normally). **(3) Recorder trim fields** mangled typed decimals (controlled number inputs rewrite the DOM at the "." keystroke — "0.5" became 5); now the house `defaultValue`+`onBlur` pattern. **(4) Group scopes couldn't reach the boolean toggle picker** — `resolveStateEntities` now resolves a group id to its member entities: key suggestions union member schemas and `resolvedKeyType` takes the first member schema carrying the key (entity scopes deliberately never fall back to the global schema). **(5) Recorder AudioContext leak** when cancelling mid-first-recording (`onstop` minted a context after unmount) — `deadRef` bails. **(6) Boolean select showed "— pick —" over a legacy non-boolean value** — now surfaces it as a disabled "(not a boolean — pick below)" option. Verdicts otherwise: per-frame additions are edge-triggered with zero steady-state allocation; React-side scans are O(300) at keystroke frequency; undo/transaction boundaries held up. Watch-item (not fixed): `_findEntityMesh` full `scene.traverse` per AI sound event — sub-ms and rare, but the scaling cliff if entity-parented sounds become high-frequency.
- v4.79.35 — **Boolean state keys get a true/false/toggle Value picker (user: "the state key, that is set to a boolean, lets me enter anything for its value… I should be able to pick from true/false/toggle").** `set_state`'s Value field now resolves the current key's REGISTERED type — entity scopes read the target entity's `stateSchema`, global scope reads a new `stateKeyTypes` map (scene overrides game) threaded ScriptPanel → ScriptEditor → ActionRow → ActionFields — and for `boolean` renders a select: `— pick — / true / false / toggle (flip current)`. **Toggle** stores the `"__toggle__"` sentinel; `ScriptEngine.set_state` resolves it per scoped key as `!gameState.get(key)` (group scopes flip each entity independently). Unregistered/other-typed keys keep the free-text input with `coerceStateValue`. Verified live on the user's real spike script: set_state targeting the sibling "hurt spikes" trigger → key popup suggested its `spikes-up` (scoped, flip-up) → picking it flipped Value into the boolean select with all three options. Guide: GAMEPLAY_STATE.md §5.
- v4.79.34 — **State-key suggestions get a flip-aware custom popup (user: "clicking the state key option does nothing visually. goes like off the bottom of the screen. if it's at the bottom it should be above the input not below it").** Root cause: the key fields used native `<datalist>` — the BROWSER positions that dropdown and (in the CEF shell) doesn't flip it above the input, so rows near the panel bottom opened an invisible offscreen list. Native popups can't be styled or repositioned, so: new **`KeySuggestInput`** (ScriptPanel) — an input with a custom TargetCombobox-style suggestion list that measures the space below on focus and opens ABOVE the input when < 190px remains; typing filters; option click commits on mouseDown (beats blur). No-`suggestions` fallback reads the global `#wb-state-keys` datalist options at open (zero prop threading; the datalist element stays for PropertiesPanel's DAMAGE KEY field). Replaced ALL `list="wb-state-keys"` inputs (set_state, adjust_number, delete_state — now also scope-aware — store_position/teleport keys, conditions, the state-trigger watch key, and the UI-tab bar/counter/icons bindings). `TargetCombobox` itself got the same flip-up measurement (the Whose-state/action-target popups clipped identically). Wrapper needs `width:100%, minWidth:90` (an unsized relative div collapsed the flex-squeezed condition-row fields to slivers — caught by screenshot). Verified live: bottom-row condition key field → popup opens UPWARD with Hearts/coins/checkpoint; real-click required (programmatic `.focus()` doesn't fire React's focus handler — probe lore).
- v4.79.33 — **Entity state: prefab-sibling targeting, scope-aware key suggestions, searchable Whose-state (user: on their spike prefab set_state "doesn't list the state of the trigger… I can't search that dropdown… what good is a thing's local state if it can't be set" + "why doesn't the spikes-object have local-state too?").** Four findings/fixes. **(1)** Objects DO have local state — the STATE section lives at the bottom of the object's Scripts drilldown (volumes show it inline, hence the asymmetry); the Scripts CategoryRow summary now advertises it (`1 script · 2 state keys`). **(2) Prefab-sibling targeting**: `StateScopePicker` (and `ConditionScope`) gain `ownerId` — the blanket `noPrefab` filter now ADMITS members of the owner's own instance (⬡-prefixed), because member ids survive re-expansion (`existingIds` preserves them) and capture remaps intra-prefab refs; other instances' members stay hidden. Threaded at the set_state/adjust_number scope, inventory scopes (give/take/transfer), state conditions, and the on_state_changed trigger. **(3) Scope-aware key suggestions**: new `entityStateKeys(targetId, ownerId, …)` — when Whose-state targets an entity, the key input's datalist switches from the global list to THAT entity's registered `stateSchema` keys (per-row `useId` datalists; set_state, adjust_number, has_state, compare_number, and the state-trigger key via TargetPicker's new `stateKeySuggestions`). **(4) Searchable**: `StateScopePicker`'s `<select>` replaced with the existing `TargetCombobox` (type-to-filter, grouped) — all five Whose-state sites became searchable in one change. Verified live: a spring-jumper volume's set_state scope listed "⬡ spring1" + the sibling volume; picking spring1 suggested exactly its `bounces` key; a scratch level script's Whose-state filtered on typing "volume". **Probe post-mortems:** an "add action" mis-click converted the spring script's real launch_player action to set_state (fields dropped) — restored byte-exact from HEAD; the script Delete button opens a NATIVE confirm(), which froze the tab for the extension (documented hazard) — tab close cancels the dialog, cleanup done via the data API + Save.
- v4.79.32 — **Microphone sound recorder (user: "what if next to import sound, there was a Record sound that opened a new modal for recording, playing back, saving, replacing, maybe trimming beginning/end… then import into asset store").** New `src/ui/SoundRecorderModal.tsx` behind a **● Record** button beside `+ Import Sound` in the AUDIO panel. Flow: `getUserMedia` → `MediaRecorder` → decode the take to an `AudioBuffer` → the recorded phase offers ▶ Play (trim-aware — previews EXACTLY the saved range via `bufferSource.start(0, trimStart, dur)`), **● Re-record** (replaces the take), TRIM START/END second fields, NAME → **Import →** encodes the trimmed buffer as **16-bit PCM WAV** (`bufferToWav` — MediaRecorder's webm/opus can't be re-encoded post-trim without a codec; PCM is trivial and decodes everywhere) and hands a `File` to the EXISTING import pipeline: `AudioImporterModal` gains `initialFiles?: File[]` (lands straight in the metadata phase; `onFilesChosen` widened to `FileList | File[]`), so category/tags/attribution/manifest-write are all reused. App state: `soundRecorderOpen` + `recordedFiles` (cleared on importer close/complete). Mic errors surface in-modal ("check the mic permission"). **Verified end-to-end on the live shell with stubbed capture APIs** (hidden tabs can't run audio rendering, so a fake `MediaRecorder` handed the modal a synthesized 1.0s sine WAV — everything downstream was the shipped code): record→stop→decode→"0.5s (of 1.0s)" after trimming 0.2/0.3 → Import → importer opened in metadata with `rec_test.wav` → full import → **the written `/assets/audio/rec_test.wav` decoded to exactly 0.5s** → cleaned up via Manage→Delete (manifest + file confirmed gone). The genuinely untestable-headless bit is physical mic capture/permission (CEF may prompt) — needs a human ear.
- v4.79.31 — **Enemy AI sounds — optional detect / walk / attack slots (user: "a sound to play on player detection, a looping or periodic sound while walking/patrolling, and a sound on attack… each slot optional… with the sound picker and volume control consistent with other recently updated areas. can be prefab'd").** `EnemyAIDef` gains `detectSound/detectVolume`, `walkSound/walkVolume`, `attackSound/attackVolume` (all optional; VOL overrides the clip default, cap 4). **Runtime hooks in `EnemyAI.update`**: detect = the idle/return→chase edge (state snapshot before `tick`); attack = `intent.attackStart`; walk = a keyed loop (`ai_walk_<id>`) started while `moved` and stopped after 0.3s of stillness (hysteresis vs wall/ledge probe stalls), on bite start, on despawn-dormancy, and on preview stop (AudioSystem disposal). **All three are spatial and FOLLOW the enemy**: `audio:play` gains `entityId?` — `AudioSystem._onPlay` parents the PositionalAudio to `_findEntityMesh(entityId)` with the emitter falloff (linear, ref 1 → max 20), falling back to position/flat if the mesh isn't built. **UI**: the Enemy AI screen gains a SOUNDS group — three `SoundPicker` + VOL rows (character-sounds idiom, `previewVolume` honored). **Export**: `assetRefs.collectObject` ships the three ids. **Prefabs**: the config rides `WorldObject.ai`, which snapshot templates capture — instances inherit and can override per copy. Verified on the live crab (headless `__enemyAI.update()` pumping — rAF frozen): chop fired exactly on the detect edge, metalclick per bite (alongside the level's own on_enemy_attack script sound), entityId parenting probe → sound parented to the crab mesh, linear/1/20, exact gain. The walk loop couldn't trigger headless — physics never steps without rAF, the ground ray misses, and the ledge rule zeroes `moved` (measured movedDistance 0 in chase) — mechanism verified, real-gameplay listen recommended. Crab's ai reverted after probing.
- v4.79.30 — **Prefab section moves to the top of single-selection views, collapsed (user: "on trigger volumes and objects, the prefab accordion should be at the top of the properties list and collapsed by default"; clarified via question — the whole-instance view keeps top + expanded).** `PrefabSection` gains `defaultOpen` (default true); the object root view renders it ABOVE the CategoryRows and TriggerVolumeView renders its `prefabSection` node first (was after Delete Volume), both with `defaultOpen={false}` and `key={record.id}` so the collapsed state resets per instance. The whole-instance multi branch is untouched (top of that view, expanded). Screenshot-verified all three views: object → collapsed "Prefab · Chest…" row above Geometry; trigger volume → same above the volume blurb; select-all → expanded section with the full button row.
- v4.79.29 — **"⛶ Select all N" + "Edit prefab" join the Prefab section's button row (user: "add the all and edit options to the prefab accordion thing").** `PrefabSection` gains `onSelectAll` / `onEdit` props and leads its action row with **⛶ Select all N** (only when `onSelectAll` is passed AND memberCount > 1) and **Edit prefab** (new amber `PrefabBtn` variant matching PrefabPanel's Edit tint; snapshot kind only). Wiring: the single-select root and TriggerVolumeView render sites pass both; the whole-instance multi branch passes only `onEdit` (everything is already selected there). `onEdit` wraps the existing `onEditPrefab(prefab.id)`. Screenshot-verified BOTH views (per the clipping lesson): single chest piece → `⛶ Select all 4 · Edit prefab · Reset · Unlink · Delete`; after Select-all, the 4-selected view shows `Edit prefab · Reset · Unlink · Delete` with no redundant select-all.
- v4.79.28 — **Header ⋯ menu for instance actions + confirmation dialogs (user: "these 3 should probably have confirmation dialogs too… they should be in like a dropdown hamburger or something, next to the select all option").** The membership line gains a **⋯** button (after ⛶) opening a small dropdown with the Prefab section's three instance actions — Reset from prefab (hidden for orphans, matching the section's degraded view), Unlink, Delete instance (danger-colored) — a fixed click-catcher closes it; state resets on selection change. **Confirmations**: new generic `src/ui/ConfirmDialog.tsx` (ScriptDetachDialog chrome: title/body/confirmLabel/onConfirm/onCancel, backdrop cancels); App wraps the three `onPrefab*` props it passes to PropertiesPanel with `setPrefabConfirm("reset"|"unlink"|"delete")`, so BOTH entry points (section buttons and ⋯ menu) confirm through one dialog with action-specific copy; the real handlers (`handlePrefabReexpand`/`Unlink`/`DeleteInstance`) run only on confirm. All three are journaled/undoable and the copy says so — the confirm guards against accidents, not unrecoverability. Verified live: ⋯ lists all three, Delete instance → dialog → Cancel is a no-op on the user's chest instance; full confirm path on a scratch prefab deleted member + record and the count-0 prefab cleaned up via ×.
- v4.79.27 — **"⛶ all N" in the membership header line (user: "maybe a select-all option by the prefab instance name… to select all items in an instance like an object and its trigger, to move them together").** A plain viewport click already selects the whole instance (Phase 47.1); the gap was getting BACK to that from a shift-clicked single piece — the panel offered no route. The v4.79.25 header line gains a small `⛶ all N` button (shown only when the instance has >1 member; the line itself only renders for single-piece selections, since full-instance multi hits the separate multi-select branch): `selPrefabInfo` now also carries `memberCount` (one `collectInstanceMembers` scan per selection change), and a new `onSelectInstance` prop → App's `handleSelectInstanceMembers` → `selection:set` with the full member refs — landing in the whole-instance multi view (one gizmo, Prefab section). A dropdown was considered and rejected: the only other action (edit) is already the name link. Verified live on the user's chest instance: single member selected → "⛶ all 4" → click → "4 selected".
- v4.79.26 — **Legacy ghost instance records swept on scene load (user: "spike step 2 still lists 2 instances even though there is 1").** v4.79.25's pruning only prevents NEW ghosts; records emptied before the fix still counted. `syncPrefabInstances` (the existing scene-load/project-open sweep that already handles staleness + generator auto-heal) now removes any record whose `collectInstanceMembers` set is empty, before the def lookup. Safe there: entities and records parse from the same scene file, so empty = genuinely deleted, never not-yet-loaded, and `restoringRef` already guards the boot window. Verified live: a fresh page load dropped Spike Step 2's records from 2 (real + ghost) to 1 (the real spike-cube instance) with a console info line. The dialog's amber "empty record (leftover)" row is now a transitional case only (visible if a ghost forms and is inspected within the same session).
- v4.79.25 — **Prefab UX: membership line, ghost-record pruning, delete-review dialog (user: couldn't delete a prefab whose badge said 2 while believing 0 instances existed; also "I'm not a fan of disabled buttons").** Diagnosis on the live world: (a) capture-from-selection makes the selection instance #1 — by design but invisible; (b) deleting an instance's members one at a time left a **ghost `prefabInstances` record** with 0 members that counts forever. Three changes. **(1) Pruning** (App.tsx): `collectInstanceStamps` (reads `entity.prefab.instanceId` per ref BEFORE removal — refs carry no data) + `pruneEmptyInstances` (post-removal `collectInstanceMembers(...).size === 0` → `removePrefabInstance`), wired INSIDE the existing transactions of `deleteRefs`, `handleDelete`, and `handleDeleteConfirm` (whose state-local `"volume"` type maps to ref-type `"trigger-volume"`) — records are journaled, so one undo restores members + record atomically (probe-verified: delete both members → records 0; TopBar ↩ → members 2 AND record back). **(2) Delete-review dialog**: new `src/ui/PrefabInstancesDialog.tsx` (DeleteAssetDialog chrome); PrefabPanel's × is never disabled — count > 0 opens the dialog listing every instance (zone · origin · piece count) with **Go to** (activate the record's zone if needed → force Select tool → `selection:set` with the member refs → glide `editorCamera.targetFocus` to `record.origin.position` — targetFocus ONLY, setting focus too would snap) and **Delete** (`deleteInstance`, own transaction = one undo step; rows refresh in place); ghost rows render amber "empty record (leftover)" with Delete only; footer **Delete prefab** unlocks at 0 rows (the dialog deliberately does NOT auto-delete when the list empties — library deletion isn't in the world journal); a `prefabTick` effect refreshes open-dialog rows on undo/redo — rows are computed on open/delete/event only, zero per-frame work. **(3) Membership header line** (PropertiesPanel): under the name/subtitle, when `prefabInfo && isRoot`: snapshot → `⬡` + name as a link into `onEditPrefab` (new prop → App's `handleEditPrefab`); generator → `ƒ` + plain name (not editable); orphan → `⚠ record.prefabId · definition missing`. Verified end-to-end in the live shell with a scratch prefab (created via the real ⬡ Create Prefab button, exercised through every flow, self-cleaned) plus a read-only open of the user's real "Spike Step 2" dialog: 1 live instance row + 1 amber ghost row, exactly the reported mystery. Side effect: the session-only Spike Step 2 def got persisted to game.json (additive). Plan: `~/.claude/plans/steady-wondering-thunder.md`; guide: PREFABS_GUIDE.md (Creating / Selecting / new "Deleting a prefab" section).
- v4.79.24 — **Positional play_sound one-shots use the emitter falloff, not WebAudio's inverse default (user: "sounds in the editor preview seem louder than in-game… more happening when spatial/play-at is used").** Attached emitters configure their PannerNode (`linear`, ref 1, max 20) but `_onPlay`'s positional branch never passed `dist` to `_makeSound`, so script play_sound with a target/position got THREE's PannerNode defaults — the **inverse** distance model (~1/d): with the LISTENER on the 3rd-person camera ~4m from the player, even a sound at the player's feet played at ~¼ volume, far below emitters and the flat editor ▶ preview at the same authored volume. The positional one-shot branch now passes `{ ref: 1, max: 20 }` (linear ≈0.85 at 4m). Probe-verified on the live shell: `panner.distanceModel/refDistance/maxDistance` read `linear/1/20`, gain 0.5 = the scene's 50% MUSIC mix. AUDIO.md §3 documents the falloff + the camera-is-the-listener caveat (positional will always sit a bit under the flat preview; that residual gap is physics, not a bug).
- v4.79.23 — **Gameplay one-shots start at their authored gain — no creation ramp (user: "I set 0.5 and the panel preview gets significantly quieter, but in gameplay it sounds like full volume; 0.1 is inaudible in the panel but only slightly quieter in gameplay").** Exactly the reported asymmetry, and the panel was the honest one. THREE's `Audio.setVolume` doesn't SET the gain — it `setTargetAtTime`s toward it (timeConstant 0.01, ~30–50ms to converge) from the gain node's creation default of **1**. A sustained sound converges before you notice; a short percussive one-shot (footstep, jump) plays its entire attack transient — the part that determines perceived loudness — during the ramp at near-full gain, so authored VOLs were largely inaudible in gameplay. The editor preview (v4.79.21) sets `gain.gain.value` directly, hence the mismatch. Fix: `_applyGain` gains an `immediate` flag — `_makeSound` passes true (direct param set at creation; also benefits playlist clip starts and `startSilent` fade-ins, which previously blipped at gain 1), while fades and mixer slides keep the smooth `setVolume` ramp. Verified on the live shell: `_onPlay` at volume 0.1 / 0.5 / default → `gain.gain.value` reads exactly 0.1 / 0.5 / 1 immediately after creation (the direct set also makes this readable in a suspended context, where the old ramp sat frozen at 1 — the v4.79.18 probe lore). **Correction to v4.79.21's side-note:** `footstep_carpet_000` has NO manifest volume (default 1); the 0.1 measured then was the user's own FOOTSTEP VOL experiment, not a clip default.
- v4.79.22 — **Script play_sound / play_music: preview honors Volume, and Volume can boost (user: "in scripts play_sound too?").** The runtime always passed `action.volume` through (`ScriptEngine` → `audio:play` / `music:play`), but the ScriptPanel's pickers didn't feed it to the ▶ preview, and the Volume fields were labeled "Volume 0–1" with `max={1}` — capping out the v4.79.18 boost range in the one place that sets it. Both actions' `SoundPicker`s now pass `previewVolume={action.volume}`, and the fields become "Volume" (placeholder 1, min 0, no max, tooltip "1 = the clip's own level, higher boosts (up to 4)"). `stop_sound`/`set_footstep` pickers unchanged (no volume of their own; the footstep override plays at the character's FOOTSTEP VOL at runtime). Same wiring pattern probe-verified in v4.79.21.
- v4.79.21 — **The ▶ preview honors the field's VOL (user: "does changing the volume and then pressing the preview play, play with that volume considered? it should").** It didn't — `SoundPicker`'s preview was a bare `<audio>` element at full volume, blind to both the field's authored VOL and the clip's own manifest volume. Rebuilt as a module-level shared WebAudio path (`playPreview`/`stopPreview` — one AudioContext for every picker, browsers cap live contexts and one-preview-at-a-time is right anyway): gain = `min(4, previewVolume ?? def.volume ?? 1)`, the exact `_onPlay` runtime semantics (authored value OVERRIDES the manifest default; `<audio>` couldn't have represented a boost above 1 at all). New optional `previewVolume` prop, passed by the three Character Sounds rows and the entity emitter's Sound field. Verified via a `createGain` capture probe on the live shell: JUMP VOL 2 → preview gain node exactly 2; FOOTSTEP → 0.1 (correction, v4.79.23: that 0.1 was the user's own value in the FOOTSTEP VOL field at probe time, NOT a manifest default — `footstep_carpet_000` has no `volume` in the manifest). (Side commit 3c5b5c0: the user's 181-sound library import — footsteps/impacts/foley, manifest purely additive — was sitting untracked and is now committed.)
- v4.79.20 — **Character Sounds reachable from the spawn menu too (user: "player audio is on the main properties menu when nothing is in-context, but it should also, perhaps, be duplicated to the character controller properties").** New `spawn-sounds` ScreenId: the spawn drilldown gains a **Character Sounds** row (`N of 3 set`, between Character and Controls) that renders the SAME `CharacterSoundsPage` component the root Audio menu's `audio-character` screen uses — one component, two entry points, so the routes can't drift. Verified in-browser: spawn menu shows the row, it opens the shared page (VOL fields and all), ← Back pops to the spawn menu.
- v4.79.19 — **Spawn/player settings restructured into a drilldown menu (user: "a lot of options… too many to show all at once. Some related ones are separated and far apart… things like jump height should be very consistent across the FPS and 3rd person options").** `SpawnSettingsView`'s ~15-field flat list became the v4.79.2 audio-menu idiom: the spawn root keeps POSITION and shows four `CategoryRow`s with live summaries — **Movement** (`speed 6 · jump 3.5`), **Camera** (`3rd person · 3.4m back` / `FPS · fov 75`), **Character** (model label or `capsule only`), **Controls** (`this device`) — opening new ScreenIds `spawn-movement`/`spawn-camera`/`spawn-character`/`spawn-controls` on the existing panel stack (the component now takes `screen`/`onOpen`; the `__spawn__` header shows the sub-screen title past root). Grouping is by CONCERN, not camera mode: **Movement** = move/jump/climb speed, explicitly noted as shared ("apply identically in FPS and 3rd person" — the consistency ask; they always were shared, but mode-gated neighbours made that illegible). **Camera** = the CAMERA MODE switch (moved off root, like the SINGLE⇄PLAYLIST switch lives in its page) above BOTH framing groups — FPS (FOV, eye height) and THIRD PERSON (distance/height/angle) — always visible and editable with an ACTIVE badge on the playing mode, instead of the old render-only-the-active-mode gating that hid half the fields. **Character** = model select + both scales side by side (labels cross-reference) + jump anim speed + the six animation slots (clips-gated only; no longer hidden in FPS mode — a blurb notes they play on the 3rd-person avatar). **Controls** = the device-local `ControlsSection` (7 fields off the root). `summaryFor`'s exhaustive switch gained the four ids (they build their own row summaries). Verified in-browser with real navigation: spawn flag click → root menu with live summaries → each page renders (Camera shows both groups + ACTIVE badge on THIRD PERSON) → ← Back pops correctly.
- v4.79.18 — **Volume boost above 1 + per-sound character VOL fields (user: "the volume of my spike cube sound is kind of low… I can't seem to set it louder than 1, with any effect" / "for player sounds, I can['t] control the volume of them at all").** Two findings. (1) The emitter VOLUME field accepted any number but `AudioSystem._applyGain` ran the whole product through `clamp01`, so authored base >1 silently did nothing — and linear distance attenuation (refDist 1 → maxDist 20) only subtracts from there. The clamp is now `max(0, min(4, base × fade × authoredMix × playerMix))`: base may boost (WebAudio applies >1 gain fine), capped at 4 so a typo can't blast ears; master and the mix buses stay 0..1. Emitter UI label now says so. (2) Character sounds had no volume at all — `PlayerSettings` gains `footstepVolume`/`jumpVolume`/`landVolume` (optional, default 1), `CharacterController._emitSound` passes them through the existing `audio:play` `volume` field, and `CharacterSoundsPage` puts a VOL input beside each picker. Verified in-browser on the live shell: VOL fields render; gain math probed end-to-end via `_onPlay` + a `setVolume` capture — base 3 → 1.5 applied (this scene's MUSIC mix is 0.5 — correct multiply), base 9 → 4 (cap), base 1 → 0.5. **Probe lore:** `getVolume()` reads the CURRENT ramp value of `setTargetAtTime`, which never advances while the AudioContext is `suspended` (hidden tab, no gesture) — it reads 1 regardless of what was set; capture the `setVolume` argument instead.
- v4.79.17 — **The between-rows strip can insert silence too (user: "insert here should be able to add silence also, or another button for it or in the modal idk").** Another button: the strip under each entry is now a pair — `+ insert here` (the multi-select picker, unchanged) and `+ silence` (splices `{silence: 2}` at `i + 1` directly, no modal — silence's main job is spacing BETWEEN clips, so append-at-end + ▲▲▲ was the tedious path). Both animate the new slot in. Shared `STRIP_BTN` style hoisted next to `MINI_BTN`. Screenshot-verified against the live shell: 13 rows → 13 paired strips.
- v4.79.16 — **Playlist toolbar moved to the top (user: "the buttons to add clip, silence, preview, loop - should be at the top not the bottom").** The `+ clip / + silence / ▶ preview / LOOP` row in `AudioSlotEditor` playlist mode moved from below the entry list to directly under the SINGLE ⇄ PLAYLIST switch (`marginTop: 4` → `marginBottom: 8`); the empty-state hint's "below" flipped to "with the buttons above". Verified with a read-only screenshot against the live shell (toolbar renders above row 1, list rhythm unchanged). Behavior untouched — `+ clip`/`+ silence` still append at the END of the list; per-position adds are the rows' own `+ insert here` strips.
- v4.79.15 — **⧉ duplicate is now duplicate-BELOW (user: "when I duplicate-below, it's the above card that expands… disorienting").** v4.79.12 spliced the copy at `i` and v4.79.13+ animated index `i` — with identical adjacent rows that's the same DATA as inserting below, but the expanding slot sat at the clicked row's own position, so the eye read "the card above the copy expanded". Now `duplicate(i)` splices at `i + 1` and `flashIn([i + 1])`: the clicked card stays put and the copy visibly grows in beneath it. Tooltip + AUDIO.md updated to "below". Two-line index shift on the verified v4.79.14 mechanism (the flashed index IS the animated slot); not re-verified in-browser — the session's test shell died (window closed) and probing the user's live instance mid-work risks the state-sync races that bit earlier.
- v4.79.14 — **Playlist add/delete animates HEIGHT, not opacity (user: "a flicker… does very little… In a list of equally sized and shaped things, an add or remove is virtually imperceptible").** The v4.79.13 fade was the wrong signal — identical rows swapping content reads as nothing happening; what's perceptible is *neighbours moving*. The keyframes now animate `max-height` (`wb-row-in`: 0→90px + fade, 0.25s; `wb-row-out`: 90px→0 + fade, 0.2s; 90px exceeds the tallest row so growth ends at true height) on a new per-entry **overflow-hidden wrapper div** holding the card AND its `+ insert here` strip — the animating unit is the full row slot. Crucial layout change: the list container dropped its `gap: 8`; row spacing moved into the strip's own margins (3px each side; card→card rhythm still 18px, `+ clip`-row offset still 7px) so a collapsed wrapper closes its space COMPLETELY — with a container gap, every delete would leave an 8px seam that snaps shut at the data write. `remove()`'s deferred write went 150→200ms to match the out duration. The wrapper replaced the v4.79.12 `<Fragment>` (import dropped). **Verification lore:** CSS animation timelines FREEZE in hidden Chrome tabs (`Animation.currentTime` pinned at 0, `playState` still "running") and page timers stretch to ~1s — naive height sampling reads garbage; `open_application` couldn't foreground the MCP tab (another window's tab stays active). Verified instead by seeking the Web Animations API directly — `anim.pause(); anim.currentTime = t` recomputes layout synchronously even hidden: grow 0/35/72/87/88px at t 0/60/125/190/250, shrink 88/53/18/4/0 at t 0/50/100/150/200 — full collapse, no seam.
- v4.79.13 — **Playlist rows fade in/out on add and delete (user: "any kind of super basic appear or disappear animation").** Two keyframes in index.html's global style block (`wb-row-in`: fade + 4px slide-down + scale from 0.97, 0.25s; `wb-row-out`: fade + scale to 0.95, 0.15s) — the app's first `@keyframes`; everything animated before this used inline `transition`s. `AudioSlotEditor` drives them by INDEX, because rows are keyed by index so a mount animation can't find "the new row" (an insert re-renders existing DOM nodes and appends one at the END): `bornIdx: number[]` marks rows that just appeared (duplicate ⧉, `+ silence`, and both multi-select commits call `flashIn`) — applying the `animation` property fresh replays it, and a 300ms clear removes it so a later add at the same index replays again (an unchanged value never does). Deletes: `remove(i)` now sets `dyingIdx`, plays `wb-row-out` with `forwards` (row stays invisible if the write lags a frame), and commits the actual `writeEntries` 150ms later; a second ✕ during that window is ignored (a stale-base write would resurrect the removed row or delete the wrong one). Verified via `document.getAnimations()` probes: `wb-row-in` running after ⧉, `wb-row-out` running with the row still in the DOM, row count drops only after the animation. **Probe post-mortem:** the first probe clicked ⧉ and ✕ in the same synchronous tick — React hadn't committed the re-render between them, the stale-closure remove wrote a pre-duplicate base, and the user's live playlist lost its first entry (restored from the committed scene via `__world.updateWorldAudio`). Same-tick programmatic clicks on state-coupled buttons are now a known probe anti-pattern: await ≥50ms between click and read.
- v4.79.12 — **Playlist rows: ⧉ duplicate + "+ insert here" (user: "add a duplicate above and insert-here option below each item… in the music playlist").** `AudioSlotEditor` playlist mode, two per-row additions. **⧉** joins the ▲▼✕ cluster on every entry (clips AND silences): `duplicate(i)` splices `{...entries[i]}` at `i`, so the copy lands directly above the row. **`+ insert here`** is a slim dashed strip below every card (negative margins tuck it into the column gap) that opens the v4.79.11 multi-select modal titled `INSERT CLIPS`; `pickerFor` grew a `{ insertAt: number }` arm and the multi commit is now one splice — `"add"` appends at `entries.length`, insert splices at `insertAt`, both landing checked clips in check order as a single `writeEntries` (one undo step). The strip after the last row is deliberately kept even though it equals `+ clip` — consistency beats saving 14px. Card JSX moved into a `card` variable returned inside a keyed `<Fragment>` with the strip. Verified in-browser on the user's live pizzi playlist: ⧉ on row 1 → rows 1+2 both PIZZI03; `+ insert here` below row 2 → checked highUp then highDown → overlay-click close → rows 3/4 = highUp, highDown (check order, splice point, and the overlay close path all correct); test entries removed via ✕ after.
- v4.79.11 — **Playlist `+ clip` is multi-select (user: "I should be able to select multiple before I close the modal. Added to the playlist in the order that I check marked them").** `SoundPickerModal` gains an optional **`onPickMulti`** mode (replacing `onPick` at that call-site): rows toggle a numbered check badge showing the pick ORDER (not just checked-ness — unchecking renumbers the rest), and a footer button reads `ADD N CLIPS` / `CHECK CLIPS TO ADD` when empty. Every close path — the footer button, the ✕, clicking the overlay — commits the checked ids in check order via one `finish()` helper (unchecking is the cancel), so the flow the user described ("select multiple, close, they're in the playlist") works whichever way the modal is dismissed. `AudioSlotEditor` splits its modal render: `pickerFor === "add"` passes `onPickMulti` (one `writeEntries` appending all entries — a single undo step), while clip-row replacement and every `SoundPicker` field keep single-pick `onPick` (picking one sound to fill one field has nothing to multi-select). Ordered `picked: string[]` state, not a Set — the array IS the commit order. Verified in-browser: checked HIT03→HIT00→HIT05 (badges 1/2/3 in that order, not list order), unchecked HIT00 (HIT05 renumbered 3→2), re-checked it (rejoined at 3), ADD 3 CLIPS → playlist rows 4/5/6 = HIT03, HIT05, HIT00; test entries then removed via row ✕.
- v4.79.10 — **Panel playlist preview is gapless (user: "plays with significant gaps between each sound. like a second or 2, even with no gaps added. while gameplay has no gaps").** Exactly right — the v4.79.9 preview chained ONE `<audio>` element by swapping `src` in `onended`, so every transition paid a fresh fetch+decode (~1–2s of dead air); the runtime never does this (preloaded `AudioBuffer`s, synchronous `onEnded` advance). The preview now works the same way: `startSeq` awaits `assetManager.loadSound` for every clip (the SAME permanent buffer cache gameplay preloads — cached after first run), then schedules every clip up front at its exact cumulative start time on a lazily-created editor-side `AudioContext` (`AudioBufferSourceNode.start(t)` + per-clip `GainNode`, silences as pure time offsets) — sample-accurate hard cuts, better than the runtime's onEnded chaining. Row-highlight timers and the auto-stop share the same computed boundaries; `stopSeq` stops all scheduled sources + clears timers, unmount also closes the context. Still editor-only (PropertiesPanel, not in the runtime chunk); `AudioSystem` untouched. Side benefit: buffers load via fetch, immune to Chrome's hidden-tab media-element load deferral (the v4.79.9 testing gotcha). Verified on the user's own 5-clip no-silence pizzi playlist: measured row boundaries 0.60/0.61/1.36/0.61/1.21s vs afinfo durations 0.57/0.57/1.32/0.69/1.15s (150ms sampling), total 4.55s vs 4.38s theoretical — zero inter-clip gaps.
- v4.79.9 — **Whole-sequence playlist preview in the editor panel (user: "a full song preview option within that panel" — un-parks a Phase 64 V2 item).** `AudioSlotEditor` playlist mode gains a **▶ preview / ⏹ stop** button beside `+ clip`/`+ silence`: plays the composed sequence ONCE, top to bottom — per-clip volume (`entry.volume × SoundDef.volume`), real dt silence gaps, the playing row highlighted blue, auto-stop at the end (LOOP is deliberately not honored — an endless preview would just need hand-stopping). **Editor-only by construction** (the user's constraint): it's a plain `HTMLAudioElement` inside PropertiesPanel, which the runtime bundle never imports — `AudioSystem` untouched, and `dist/assets/runtime-*.js` grep-verified free of the code. Mechanics: a `seqToken` ref invalidates stale `onended`/timer advances; `stopSeq` runs on ⏹, on any `writeEntries` (an edited sequence mid-preview would play a stale list), on mode flip, on row-▶ clip preview (shares the `<audio>` element), and on unmount. **Testing gotcha discovered (env, not code): Chrome defers media-element loading in hidden tabs** — `play()` never settles, `loadstart` → `stalled`, no network request ever issued; the sequence appeared stuck on entry 1 until the tab was foregrounded, then ran perfectly (sax → 2s silence → steel, auto-stop). Joins the §rAF-pauses-in-background-tabs caveat in TESTING lore: foreground the tab before testing ANY editor audio.
- v4.79.8 — **Every sound field opens the modal picker (user: "add the same new improved audio picker modal for the other audio selection areas. Like character sounds, ambient, object sound emitters, etc.").** Instead of rewiring nine call-sites, **`SoundPicker` itself was rebuilt** from a whole-library `<select>` into the v4.79.6 pattern: a label button (current sound's label, or `— none —` / `Select a sound…`) opening `SoundPickerModal`, the existing ▶ preview, and — with `allowNone` — a ✕ clear reporting `onChange("")` (the empty-string convention every allowNone caller already handled). All consumers upgraded for free: the single-track music/ambient slot, the three Character Sounds fields, the object/platform/shape sound-emitter screen, and ScriptPanel's four sound actions (play_sound ×3, play_music). The modal's remembered filter state is module-level, so it's shared across every picker AND the playlist flow — one mental "where I left off". The never-passed `filterCategory` prop was dropped in the rewrite (zero callers). Verified in-browser: Character Sounds FOOTSTEP field → modal → pick `highDown` → field shows label + ▶ + ✕, ✕ clears back to `— none —`; a play_sound action added to the level death script renders the button and its modal overlays the SCRIPTS panel correctly (z 100), temp action removed after.
- v4.79.7 — **SINGLE ⇄ PLAYLIST mode flips retain both sides (user: "it kind of deleted the playlist down to a single item. with no going back … like how I can switch from 3rd person view to first person and both are retained").** The v4.79.0 flip was destructive by data model — a slot held EITHER `soundId` OR `playlist`, so `toSingle` collapsed the sequence to its first clip. Now a slot holds BOTH plus **`mode?: "single" | "playlist"`** (types.ts): the editor's flips just set `mode` and spread the rest through (`writeEntries` / the single-track picker preserve the inactive representation; picking "— none —" in single mode keeps a retained playlist instead of clearing the slot), with the old seeding kept as a first-flip-only affordance (single track → entry 1 when no playlist exists; first clip → single track only when `soundId` is empty). Runtime: new module helper **`activePlaylist(slot)`** (`entries.length && mode !== "single"`) replaces the bare `playlist?.entries?.length` checks at BOTH decision sites — `activate()` and `_reconcileAuthored` — so a live mode flip mid-preview stops/starts the right representation; absent `mode` = legacy playlist-wins-when-present, making every existing scene byte-compatible. `AudioMenuSection.slotSummary` respects `mode` too (a parked 3-entry playlist reads as its single track's label). `assetRefs` untouched — it already collects both representations' sounds, so retained-inactive clips still ship (correct, slightly generous). Verified in-browser through real button clicks: SINGLE flip keeps the 3-entry demo playlist in data + seeds sax00, changing the single track to steel09 then flipping PLAYLIST→SINGLE round-trips both sides untouched, game entry in single mode plays steel09 with zero sequencer states, in playlist mode starts the music sequencer. Also recovered the user's collapsed demo playlist from the committed scene file (the loss lived in the autosave).
- v4.79.6 — **Playlist clips pick through a modal sound browser (user: "should not be one dropdown with every single audio clip").** `AudioSlotEditor`'s playlist mode dropped the whole-library `SoundPicker` `<select>`: `+ clip` and clicking a clip row's sound name now open **`SoundPickerModal`** — a fixed-overlay modal (AudioImporterModal chrome) wrapping the AUDIO panel's browsing surface: search box, `AssetFilterBar` over the same facet specs (`SOUND_FACETS`, now exported from AudioBrowser so panel and modal cannot drift), and the row list with ▶/⏸ preview. Clicking a row appends `{soundId}` (title `ADD CLIP`) or patches that row (title `CLIP n`) and closes. **The modal remembers where you left off**: search/segment/selection live in a module-level `savedState` restored on every mount — add a clip from a filtered view, close, reopen, and the same filtered view is showing (survives page navigation; resets with the app). Clip rows render the sound label as a button + a separate ▶ preview; SINGLE mode and every other `SoundPicker` call-site keep the dropdown (small lists). Verified in-browser through real clicks on the Background Music page: filter `Music` + "steel" → pick appends entry 4 and closes → reopen shows the same filter → `CLIP 4` re-pick swaps the sound in place → ✕ removed the test entry.
- v4.79.2 — **Audio screen restructured into a drilldown menu (user: "too much going on").** The one-page Audio screen (4 mixer sliders + two slot editors + character sounds + blurbs) became the house idiom: a menu of four `CategoryRow`s with live summaries — **Mixer** (`100 · 100 · 100 · 100%`), **Background Music** / **Ambient** (track label, `playlist · N entries`, or `none`), **Character Sounds** (`N of 3 set`) — each opening its own sub-page via the existing screen stack (new ScreenIds `audio-mixer`/`audio-music`/`audio-ambient`/`audio-character`; ← Back pops to the menu). `AudioMixerSection` split into `AudioMenuSection` + `AudioMixerPage` + `AudioSlotPage` (shared by both slots, hosts `AudioSlotEditor`) + `CharacterSoundsPage`; also v4.79.1 folded the playlist entries into two-line cards (full-width picker line + VOL/▲▼/✕ line) after the one-line rows truncated names in the panel width. Verified in-browser: menu rows + summaries render, drill-in shows the playlist editor, Back returns to the menu.
- v4.79.0 — **Phase 64 — music/ambient playlists (composed clip sequences).** Grilled design (plan: `plans/phase-64-audio-playlists.md`): background music/ambient no longer require one long pre-mixed file. **`WorldAudio.music`/`.ambient` slots** hold EITHER the classic single track (shape untouched, `soundId` now optional in the type) OR an inline **`AudioPlaylist`** — `entries: PlaylistEntry[]` of clips `{soundId, volume?}` and silence gaps `{silence: seconds}`, `loop` (default true). One clip at a time per channel, hard cuts, authored order; layering = music + ambient each running their own playlist on their own bus. **AudioSystem sequencer**: per-channel `PlaylistState` map — `activate()` starts a slot's playlist over its single track and PRELOADS every clip buffer (AssetManager's permanent cache — nothing fetches/decodes mid-gameplay, the user's performance constraint); silence gaps are dt-counted in the existing `update()` tick (the engine's only clip-end signal self-destructs the node, so gaps are timers, never fake audio); clip ends advance via an overridden `onEnded` (finish + advance, token-guarded against stale async starts); missing defs warn+skip synchronously, async load failures advance with a fail-streak that parks the sequence after a full fruitless ring; loop-off parks it silent until scene reload; `deactivate()` clears state (scene re-entry restarts — existing contract); `_reconcileAuthored` restarts a slot only when its playlist JSON actually changed; script `music:play`/`music:stop` call `_stopPlaylist("music")` first — scripts win the channel. **Editor** (`AudioSlotEditor` in PropertiesPanel): per-slot SINGLE ⇄ PLAYLIST switch (mode flips convert data both ways — the single track seeds entry 1, the first clip seeds the single track); playlist mode is a script-actions-style list — numbered rows, SoundPicker + per-clip volume, SILENCE seconds rows, ▲▼ one-step reorder, ✕, `+ clip`/`+ silence`, LOOP. **Export**: `assetRefs` collects every playlist entry's soundId for both slots so published games ship the clips. Verified end-to-end through the real UI + sequencer probes (acceptance: `test-plans/phase-64-audio-playlists.md`); demo: platfrom-obby level_1 music = sax jingle → 2s silence → steel jingle @0.5, loop on. V2 parked: crossfade, shuffle, script actions targeting playlists, whole-sequence editor preview, save-position persistence. Guide: AUDIO.md §2.
- v4.78.4 — **Cursor flicker over every button in the Deno shell: CEF ignores INHERITED cursor values — explicit `cursor: pointer` on clickables' contents fixes it.** User report: in the desktop app the cursor flipped arrow↔pointer while moving over virtually every button (toolbar, submenus, filter chips, modal buttons) — "like the letters get a finger pointer and the non-letters don't". That description turned out to be exactly right: Chromium-in-Chrome resolves the effective cursor through inheritance (per-pixel `elementFromPoint` sweeps measured uniform `pointer` — which is why the page kept exonerating itself), but the `deno desktop` CEF host resolves the cursor from the innermost hit element and drops inherited values over text/icon children, showing the arrow between glyph hits. Fix: a global rule in BOTH entrypoints' inline `<style>` (index.html + runtime.html): `button, button *, [role="button"], [role="button"] *, select, label, summary, a, a * { cursor: pointer }` (+ disabled-button default) — every clickable and everything inside it declares pointer explicitly, no inheritance involved. User-confirmed immediately fixed in the shell, no side effects. Diagnostic residue kept: repo issue #1 (findings + a 40-line `deno desktop` repro) — the underlying inherited-cursor drop is an upstream CEF/deno-desktop bug if anyone wants to file it. **Lesson recorded**: DOM/cursor measurements taken in a Chrome tab do NOT transfer to the CEF shell — same Blink, different cursor plumbing; test shell-reported symptoms in the shell's terms.
- v4.78.3 — **Tag suggestions are clickable chips (datalist retired) + one cursor across the filter strips.** Two user reports. (1) Clicking a tag-suggestion in the Import Sounds modal's `<datalist>` dropdown did nothing — TagInput's commit-on-pick relied on the pick firing a React-visible input event, and the shell's native datalist popup sets the value silently, so the suggestion landed in the input uncommitted ("I must fully type it"). The datalist is replaced with **in-DOM suggestion chips** rendered under the input (up to 8 unused suggestions, substring-filtered by the draft as you type, committed on `mousedown` with `preventDefault` so the input's blur-commit can't race a half-typed draft into a second wrong tag) — deterministic in every engine, and the options are now visible without focusing the field. Benefits both TagInput consumers (import modal + edit dialog). (2) Sweeping the cursor across the pack/tag chip rows flickered pointer→arrow→pointer at every 4px flex gap — the chips were `cursor: pointer` but the row containers were `auto`. The strip containers (segmented control, value-chip row, elsewhere-chip row in `AssetFilterBar`, plus AssetBrowser's own two strips) now carry `cursor: pointer`, so each strip reads as one interactive zone. Verified in-browser: a real mousedown on the `kenney` chip commits it instantly (chip appears, suggestion leaves the row), typing `sax` filters the row to `sax-jingles`, no `<datalist>` remains in the DOM, and the strip containers compute `pointer`.
- v4.78.2 — **Import dialogs default to a wide file filter; the format check moved into the modal, visibly.** User report: importing `.ogg` sounds required manually switching the shell file dialog's type dropdown away from its `.mp3` default — the bare-extension `accept` lists became one dialog filter entry per extension, defaulting to the first, greying every other supported format. Per the user's design ("give a wider selection and check after"): the audio/graphics/skybox importers' `accept` now LEADS with the combined MIME wildcard (`audio/*` / `image/*`) so the dialog's default filter admits every supported format at once (the per-extension entries remain as narrower options; ModelImporterModal deliberately unchanged — `.glb` first is the right default there and no single MIME spans model+texture files). The check-after became visible in AudioImporterModal: `onFilesChosen` records the skipped names and both phases render an amber "Skipped N files — not a supported audio format: …" note — previously unsupported picks were dropped SILENTLY, and an all-unsupported pick did nothing at all (a dead Browse button). Verified in-browser with synthetic DataTransfer picks: ogg+mp3+txt+tiff → meta phase with 2 sounds and "Skipped 2 files… notes.txt, photo.tiff"; an all-junk pick stays on SELECT FILES with the note instead of silence.
- v4.78.1 — **"(no pack)" bucket — the Pack facet shows for one-kit-plus-unlabeled libraries (the invisible source/kit filter, audio).** User report: the v4.66.0 kit/pack filter — the feature's original ask — seemed missing "at least in audio". It was coded (`AudioBrowser` pack spec reading `attribution.sourceName`) but auto-hidden: the sound library is 62 sounds from ONE kit ("Digital Audio", Kenney) + 3 unlabeled synthesized fixtures, `buildFacets` drops blanks, and one distinct value fails the ≥2-distinct visibility rule — indistinguishable from "not implemented" from the user's seat. Fix in `assetFilters.ts`: **`FacetSpec.blankBucket?: string`** — items whose `read` yields nothing count into a synthetic bucket (set to `"(no pack)"` on all six browsers' pack specs), a new visibility clause treats *one real value shared by ≥2 + ≥1 unlabeled item* as a showable split (the bucket never justifies a facet by itself), and `matchesFacets` maps empty reads to the bucket so the chip actually filters. Materials/graphics/decals stay hidden (their per-texture singleton `sourceName`s still fail the shared-by-≥2 clause — the case the rule was built for); models unchanged. Verified in-browser through the real panels: Sounds now shows Pack with `Digital Audio 62` / `(no pack) 3` (picking the latter → exactly the 3 fixtures), Skyboxes gains the same legitimate split (`Skyboxes 5` / `(no pack) 3`), the other four panels byte-identical. Acceptance addendum: `test-plans/asset-facet-filters.md` #21–27.
- v4.78.0 — **Phase 63 — free-roaming enemies + Enemy AI range rings in the viewport.** Two user asks. **(1) `EnemyAIDef.freeRoam?: boolean`** — no leash, no walk-home: detection skips the `fromPost ≤ leash` gate, and on losing the player (give-up radius or vertical gap only) the brain goes straight to `idle` in place instead of `return` — it re-acquires from wherever it stopped and never walks back to its authored post. `on_player_lost` still fires. Pure-brain change (`BrainParams.freeRoam`, resolved `?? false` in the adapter) — homebound behavior is byte-identical when the flag is off. Headless suite grows to **41 assertions** (detect far beyond leash, chase survives any post distance, lost → idle-in-place with no home movement, re-detect from the stop point, homebound leash still breaks). Panel: **FREE ROAM** checkbox on the Enemy AI screen; the LEASH RADIUS row hides while it's on. **(2) SHOW RANGES IN VIEWPORT** toggle (top of the Enemy AI screen) — new editor-only **`src/editor/AiRangeRings.ts`** (constructed in App.tsx beside ColliderEditor) draws color-coded ground circles around the enemy for the radii being edited: green detect / yellow give-up / red attack / blue leash (omitted for free-roamers), matching dots appear next to the field labels as a legend while the toggle is on. Driven by the new **`ai:range-preview`** bus event, re-emitted by the panel on every value edit so the circles track typing (defaults resolved with the same rules as the runtime: give-up = detect × 1.5 etc.); rings clear on toggle-off, deselect, screen close, and preview:start (`depthTest:false` LineLoops, renderOrder 999, lifted 0.06m). Guide: HAZARDS_GUIDE enemy recipe covers both.
- v4.77.1 — **Despawned entities are truly intangible: hidden physics bodies park underground (the standable dead-crab box, root-caused).** User report: the stomped crab still left an impassable invisible box you could stand on — despite v4.76.4's collider+body `setEnabled(false)`. Reproduced with a real character-controller settle: after the actual stomp actions (Death hold + delay-1 fade-1 despawn) and with body AND both colliders measuring `enabled:false`, the player still depenetrated up and rested at exactly the live crab's height (9.955 vs ground 9.167). Root cause: **Rapier (0.19) leaves a body disabled *in place* with a stale broadphase proxy — the KCC keeps colliding with it until the body's translation changes** (proven live: parking the disabled body 1000m down made the spot pass-through, and even restoring its translation kept it intangible — the flush is what mattered, not the position). v4.76.4's flag-toggling was verified with state reads, not a controller settle, so the ghost survived it. Fix: new `ZoneManager._setColliderSolid(collider, visible)` — used by every branch of `_setEntityHidden` (objects, platforms, stairs, shapes, floors, walls, volume sensors, attached-volume sensors) — disables/enables the collider + parent body AND parks the body `PARK_DEPTH` (2000m) underground on hide, restoring the saved translation on show (skip-if-repositioned guard covers `_resetAll` re-posing a parked body first; `_parkedBodies` bookkeeping is preview-scoped, cleared in `_restoreDespawned` since Rapier reuses handles). Safe universally because every collider's parent body is dedicated to one entity (`createStaticCollider`/`createSensorCollider` mint a body per collider; mover/AI kinematic hosts are per-entity). Verified with controller settles on the live crab: corpse spot = ground rest immediately after the real stomp (was live-height), `spawn_object` re-lands the body (8.35) and it's solid again, preview exit with a parked corpse restores the editor body to rest (8.34), re-enter → crab at post, solid, chases to attack. TESTING.md gains the gotcha: probe intangibility with a controller settle, never enabled-flags/rays (all reported the ghost gone while it held weight).
- v4.77.0 — **Phase 62 — enemy AI hardening (pure brain + arbiter + watchdog + headless tests; zero behavior change).** Follow-up to the phase-61 postmortem: all five post-ship AI bugs lived at integration seams (transform ownership, mixer ownership, ray exclusions), not in the state machine — so this phase turns those implicit conventions into enforced structure before new behaviors widen the seam surface (plan: `plans/phase-62-ai-hardening.md`). **(1) `src/preview/enemyBrain.ts`** — the decision core extracted as a PURE module (no three.js/physics/engine imports): `tick(mem, params, senses) → BrainIntent` carries the verbatim phase-61 machine (detect w/ vertical gap + leash gate, give-up hysteresis, walk-home arrival, attack windows with a single damage moment, counter-seeded feints, bounded cooldown jitter, orbit bias), with the tuning constants (MAX_VERTICAL_GAP, ORBIT_*, FEINT_CHANCE, ARRIVE_EPS, HIT_RANGE_SLACK, OVER_TOP_SLACK) as the shared single source; `EnemyAI` is now the adapter — senses in (rays, player position), intents out (kinematic writes, clips, trigger fires). **(2) Animation priority arbiter (`ObjectPlacer`)** — the scattered mixer-ownership booleans (`_scriptClip` set + aiPlay refusal interplay) became one per-object `_channel: {level, clip}` with levels autoplay(0) < ai(1) < script(2): `aiPlay` refuses under a script clip, `setAutoPlay` refuses under ai OR script (previously it could stomp a held clip), `previewClip`/`stopPreview` own level 2, `hasScriptClip` reads the channel; public API unchanged. **(3) `src/world/transformWatchdog.ts`** (dev-gated, armed from `installDevGlobals` in both shells) — transform writers report `(entityId, writer)`; two different writers on one entity within a frame (boundary = queueMicrotask flush, so manual deterministic stepping works too) warns once per (entity, pair) naming both systems. Reporters: MoverSystem's update loop, `EnemyAI._applyPose`, ObjectPlacer's `object:updated` transform patch. The floating crab (v4.76.8, a week of live-trap debugging) would have been one console line on day one — provocation-verified: forcing the crab's dormant aiDriven entry `running=true` for 5 frames yielded exactly one warn ("written by both MoverSystem and EnemyAI"), silent otherwise. **(4) Headless brain tests** — `scripts/test-enemy-brain.ts` via **`deno task test:ai`** (~50ms, no browser): 35 assertions over detect gating, hysteresis/leash, arrival, locomotion intents + orbit-bias bounds, attack window (start / exactly-one damage moment / no locomotion / jitter within [1±0.4v]), whiff semantics (feint / slack range / over-top / vertical gap), and feint distribution over 100 attacks with a max-run resonance check. Browser-verified identical behavior via deterministic stepping on the live crab: detect→chase (Walk), approach 4.0→1.35m, bite lands Hearts 3→2 at reachable height, over-top bites whiff (4 attacks, 0 damage), leash return settles idle 0.225m from post, Dance script-clip freeze (zero movement, currentClip nulled, `__aiRises` empty) with clean Walk resume, and both arbiter refusals. Acceptance: `test-plans/phase-62-ai-hardening.md`.
- v4.76.8 — **The floating crab, solved: a leftover disabled mover def re-armed on preview exit and drove the AI host.** User report (reproduced 3-of-4 live, NEVER synthetically): jump through the checkpoint-3 dance trigger before the AI chases → the crab rises ~2m, dances mid-air, settles on the ground. Diagnosis by live in-page traps in the user's play tab after all static forensics failed: a property-setter trap on the crab mesh's `position.y` caught the writer's stack (minified `Ev._applyPose` — which turned out to be **MoverSystem**, not EnemyAI: two classes share the method name), and an entries dump showed the crab's aiDriven host entry `kind:"slide" axis:"y" running:TRUE`. Root cause, two halves: (1) the crab object carries a **leftover `mover` def** — a disabled 2m Y-bob (the coin recipe) with `autoStart:true` — and ZoneManager's phase-61 aiHost path passed `obj.mover ?? synthetic` into the dormant entry; (2) **`MoverSystem._resetAll` re-armed `running = def.autoStart ?? true` with no aiDriven guard** on every preview:stop — so from the SECOND preview run on, the "mover" bobbed the crab whenever EnemyAI wasn't writing the mesh, i.e. exactly during script-anim freezes (the Dance; also the earlier bite-float sighting). The AI's per-frame `_applyPose` masks the bob mid-chase (why it needed dance-before-chase) and snaps it back on resume (why it "comes back down"). Fixes: `_resetAll` keeps aiDriven entries parked; ZoneManager always hands aiHost registrations an INERT def. Verified: exit/re-enter leaves running=false; a danced crab with movers stepping holds 8.339 (was ~10.3). **Testing lessons:** synthetic loops must step `movers.update` too (all six update-loop systems — omitting movers hid this for a week of sessions); and when static analysis exonerates everyone, a live `Object.defineProperty` setter-trap on the victim's property catches the actual writer's stack.
- v4.76.7 — **Post-interlude locomotion resync (the drifting-after-a-Dance bug) + `__physics` dev global.** User report: the crab "stops briefly to dance" mid-chase, then never plays Walk again — drifts in the idle pose. The Dance is the USER'S OWN "Checkpont 3 trigger, dance" volume (`on_player_enter → play_animation Dance` at the crab — a celebration authored pre-AI, living on a checkpoint volume where it wasn't findable from the crab). The pause itself is v4.76.5's script-clip freeze working as designed; the bug was the aftermath: `stopPreview` reverts the mixer to the auto-play Idle, but `EnemyAI.rec.currentClip` still said "Walk" → the clip was never re-issued. Fix: the freeze branch nulls `rec.currentClip`, so locomotion re-issues its clip on resume (verified: Walk 3 frames after the dance ends). Also `__physics` (raw physicsWorld) added to the editor dev globals. **Testing lesson recorded**: in a frozen (rAF-starved) extension tab the physics world never steps, its query structures stay empty, and EVERY `castRay` misses — an editor-side `__enemyAI.update()`-only stepping loop makes the AI refuse all movement (ground ray finds nothing) and reads as a regression. Deterministic AI loops must interleave `__physics.step(dt)` (the runtime recipe already did via `physicsWorld.step`).
- v4.76.6 — **Contrast pass II: 160 dark-grey-on-dark text colors lifted across 30 UI files.** User caught the Enemy AI blurb at `#606070` — the exact style banned in the v4.33.5 pass (and freshly reintroduced by the phase-59–61 panels, which copy-pasted an old hint-box style). Full audit classified ~200 sub-`#8a` text colors into READ-TEXT / LABEL / DELIBERATE-MUTED; **160 lifted**: explainer/hint/blurb/empty-state/secondary read-text (`#333340`–`#808080` family, incl. the 26-site `#606070` monospace hint boxes, the 17-site `#404050` 9px explainers, TopBar's near-invisible `#3a3a50` autosave indicator, browser/importer empty states, in-game DialogueOverlay prompt + picked options, bag/menu empty states, MainMenu author line) → **`#98a2b8`**; darkest mini field labels + shared constants (AttributionFields LABEL, importer STEP_LABELs, EditMetadataDialog S.label, ThumbnailStager SLIDER_ROW, PropertiesPanel INFO) → **`#8b94a8`**. Left alone: disabled states, chevrons/✕ glyphs, danger accents, inactive tabs, borderline ≥`#8a`. THE RULE (third time's the charm): text a user READS is never darker than `#8b94a8`; body/hint text is `#98a2b8`; primary is `#c2cadb`+. Verified by screenshot (Enemy AI screen, Scripts+STATE screens, TopBar) and by grep (remaining sub-#8a hits all match the muted exception list).
- v4.76.5 — **Death performances play clean: script clips freeze the AI; the player is never "ground".** User report: a stomped crab usually skipped its Death animation and floated up "as high as the player bounced" before fading. Root cause: the stomp script's despawn has **delay 0.5 + fade 1** (the standard play-anim-then-fade recipe), and during that window the enemy AI was fully alive — (a) its locomotion clips (walk/attack) kept overriding the script's held Death clip on the shared mixer, and (b) the ground-snap ray didn't exclude the PLAYER's capsule, so with the bounced player directly overhead the ray read the capsule as ground and the crab snapped up to it every frame — riding the bounce. Fixes: **ObjectPlacer tracks script-driven clips** (`_scriptClip` set; `previewClip` adds, `stopPreview`/preview:stop clear; `hasScriptClip()`; `aiPlay` refuses while set) — a play_animation outranks AI locomotion until it finishes; **EnemyAI freezes entirely** (`continue`) while its entity has a script clip — enemies die where they stand; **the ground ray excludes the player collider** (new `PreviewController.playerCollider`; the wall probe intentionally still sees the player — enemies shouldn't walk through them). Verified deterministically: 5s of AI with the player hovering overhead leaves the crab at ground level (was: levitating), and during a held Death the AI is frozen with `aiPlay` blocked.
- v4.76.4 — **Despawned enemies leave no ghost: parent-BODY disable + attached volumes go dark.** User report: after stomping the crab, an invisible obstacle remained — standable, walk-around-able. Two causes. (1) **Rapier's character controller still collides with a `collider.setEnabled(false)` collider whose rigid body is live** — measured deterministically: the player settled GROUNDED at exactly the dead crab's collider top (feet 9.12 vs box top 9.10). `_setEntityHidden`'s object branch now also toggles each collider's **parent body** (`c.parent()?.setEnabled(visible)`) — object collider bodies are dedicated statics or the entity's own mover/AI kinematic body, so nothing shared is touched; post-fix the same drop lands through at the platform (9.17). (2) **Attached trigger volumes now die with their host**: `_setEntityHidden` disables the sensors of every volume with `attachTo === id` (re-armed on spawn) — a corpse's stomp zone was an invisible bounce pad (its `player_falling`-gated script re-fired on anyone landing there); TriggerSystem's diff emits a clean volume-exit when the sensor goes dark. Verified via runtime `physicsWorld` collider probes (crab box + both riding sensors read disabled) + deterministic drop trajectories.
- v4.76.3 — **Jump-on-a-biting-crab fixes: occupancy-aware enter conditions, bite whiffs above the enemy, feint-roll decorrelation, controller dt clamp.** User scenario: crab starts a bite, the player jumps ON it before the damage moment — the stomp didn't fire AND the bite landed. Four fixes. (1) **Sticky enter** — `TriggerSystem` emits per-frame `trigger:volume-stay` for occupied volumes; `ScriptEngine._fireEnterOccupancy` re-tries `on_player_enter` scripts whose conditions FAILED at entry, firing the moment they pass mid-occupancy (once per visit, `_enterFired` per-volume set cleared on exit/deactivate; `_evalAndRun` now returns whether it ran). Fixes the close-jump: the player enters the tall stomp zone while RISING (condition correctly fails), and the old edge-triggered enter never re-fired on the descent inside. Also unlocks e.g. has_item gates satisfied while standing on the plate. (2) **`player_falling` fallingNow threshold −0.01 → −2.5 m/s** (matching the grace recorder): with per-frame retry, a hair-trigger threshold fired on teleport settle-drops and airborne flicker frames. (3) **Bite whiffs above the enemy** — EnemyAI's damage check adds `overTop`: player center more than (enemy AABB height × scale + 0.35) above the enemy's feet is out of the forward lunge's reach; standing/landing on its back is safe (verified: 6.7s on the crab's back = 0 damage; 6.7s in front = bitten to 0). (4) **Feint roll decorrelated** — the clock-sin roll resonated with the regular attack cadence (caught live: every bite in a 4s window feinted); now a hash of per-enemy phase + attack counter. Plus: PreviewController's updateFn clamps dt to 0.05 like the physics step (frozen-tab resume frames spiked the controller's gravity integration — phantom falls, and a real alt-tab hazard). Verified live end-to-end incl. the exact user scenario mechanics.
- v4.76.2 — **Respawn no longer shows the old animation after the fade.** User report: after dying (held death pose) or falling off the edge, the post-respawn fade-in revealed the avatar mid-crossfade — "getting up" from the death pose, or briefly still in the fall loop. Cause: every avatar clip change is a 0.15s crossfade, and the `character:teleport` warp only cleared the script-anim FLAG — the next frame blended out of the old pose, outliving the fade. Fix: new `CharacterController._snapAnimToIdle()` called in the teleport handler — `mixer.stopAllAction()` (kills all fades) + hard-start idle at full weight, locomotion resumes normally from there. Applies to every warp (respawn_player, teleport_player, checkpoint) — a warp resets the pose by design. Verified live: from an active held Death pose, immediately post-warp `_currentClip` = "idle" at weight 1 with the Death action fully stopped (not fading).
- v4.76.1 — **Goomba rule: `player_falling` condition (stomp only counts when landing).** User report: the crab is short enough to walk over — entering the stomp zone grounded killed it for free. New composable **`ConditionType 'player_falling'`**: passes only while the player is airborne AND descending (velY < −0.01), with a **120ms landing grace** recorded by `CharacterController` (`_fellAt` stamped each frame the player falls faster than 2.5 m/s, BEFORE the landing clamp zeroes velY) — needed because a thin/flush stomp zone's enter event lands on the same frame as grounding (found live: the user's 0.13m-thin zone flush with the crab's back never saw a falling frame at entry). The 2.5 m/s floor keeps slope-walking micro-falls from passing. Plumbing: `CharacterController.verticalVelocity`/`msSinceFalling` → `PreviewController.playerMotion` → module-level `setPlayerMotionProvider` in ScriptEngine (the free `checkScriptConditions` can't reach instance state; both shells wire it) → condition case fails closed outside play. ScriptPanel: condition in the dropdown with an explainer row (no fields, no entity scope — it's about the player). Content: the crab-stomp script gains the condition. Verified live: grounded eval false; a real 4m drop through the riding stomp zone despawned the crab (no damage taken); walking at the crab is body-blocked and bites instead; condition false again once settled. Guide: HAZARDS_GUIDE enemy recipe "Goomba rule" bullet.
- v4.76.0 — **Phase 61 — basic enemy AI (detect → chase/circle → attack).** Grilled design (plan: `plans/phase-61-enemy-ai.md`; premise correction: no NPC triggers pre-existed — `on_player_detected`/`on_player_lost`/`on_enemy_attack` are born here, owner-stamped like `on_interact` in `_ownedScript` + `_runActions` so "★ this object" works inside their hooks). **`WorldObject.ai?: EnemyAIDef`** (panel **Enemy AI screen** on objects: detect/give-up/attack-range/speed/damage/damageKey/cooldown/damage-moment/variation/leash + idle/walk/attack clip pickers with name auto-match). **`src/preview/EnemyAI.ts`**: per-frame state machine (idle/chase/attack/return) — horizontal detection with a 3.5m vertical gap, hysteresis + leash with walk-home, orbit-drift/jitter/feints scaled by ONE variation knob (per-enemy id-hash offsets), damage adjusts the GLOBAL damageKey at the authored moment then fires `on_enemy_attack`; dormant while `__despawned` (death stays Phase-60 authored); **dt clamped to 0.05** (frozen-tab rAF hands out multi-second deltas). **Kinematic host by mover reuse**: `ZoneManager._buildObjectColliders` treats `ai.enabled` like `mover.enabled` but registers a DORMANT `aiDriven` MoverSystem entry (never advanced, immune to `mover:set`; new `entryFor()` hands EnemyAI the body/mesh-rests/origin) — buying solid-while-moving colliders, phase-53 attached-volume riding (the crab's stomp zone follows the chase), and exact rest-pose reset on preview:stop for free. Movement rays: forward wall probe (stops) + downward ground snap (refuses ledges — enemies can't leave their platform), both excluding the own body + sensors. **`ObjectPlacer.aiPlay`** per-entity clip channel (bypasses the GLOBAL one-shot preview slot so simultaneous attacks don't steal animations) + `clipNamesFor`/`clipDuration`; `PreviewController.playerPosition` accessor; update registered between movers and the physics step in BOTH shells; `__enemyAI` dev global. `spawn_npc` removed from the action dropdown (Phase-13 stub; tolerated in old data). Crab in platfrom-obby wired as the living example (damageKey `Hearts`, hook = flash + knockback; old hand-made bite scripts disabled-preserved). Verified via deterministic manual stepping — chase curves in, Hearts 3→2 at the damage moment, leash return, stomp rides at 0.05m, dormant when despawned, byte-exact pose reset (acceptance: `test-plans/phase-61-enemy-ai.md`). Guide: HAZARDS_GUIDE "Recipe: a real enemy".
- v4.75.0 — **Phase 60 — per-entity state (enemy health, chest open/closed, NPC memory) + persistent despawns.** Design grilled with the user (plan: `plans/phase-60-entity-state.md`). **Facade by choice:** entity-local values are namespaced keys in the ONE GameState store — `__ent.<entityId>.<key>`, entity item counters `__ent.<id>.inv.<itemId>` (new **`src/scripting/entityState.ts`** spells the convention: `entKey`/`entInvKey`/`despawnedKey`/`registerEntityStateSchemas`) — so saves/Continue, New-Game reset, min/max clamping, `state:changed` reactivity, and `__`-prefix STATE-tab hiding all apply unchanged; no UI ever shows a raw prefixed key. **Data:** `WorldObject.stateSchema?` / `TriggerVolume.stateSchema?` (authored in a new panel STATE section on the object SCRIPTS screen + volume view, with live values while playing); registered after every `configureSchema` (which clears the map — App preview-start + SceneRouter.go). **One scope everywhere:** the state actions reuse `targetId` as entity scope ("self"/entity/GROUP fan-out via `_resolveTargets`); `ScriptCondition.entityId` + `ScriptTrigger.entityId` (single entities) — "self" resolves on the existing `_ownedScript` index-time rails, scoped state triggers simply index under the namespaced bucket key (firing path unchanged), and `checkScriptConditions(conditions, ownerId?)` resolves at eval time for the un-indexed dialogue/menu surfaces. **DialogueRunner threads the launching entity** (show_dialogue passes ownerId) into option conditions + actions. **`transfer_item`** (fromId/toId endpoints): atomic conserving move `min(count, source balance, dest stack space)` — an empty source moves nothing, duplication impossible; give/take stay as creation/destruction with a "Whose inventory" scope. **Despawn persistence:** spawn/despawn write `__despawned.<id>` booleans; `ZoneManager._applyStartHidden` consults them state-wins-when-present over `startHidden` — deferred one microtask past the preview:start listener chain because App's reset/restore handler registers AFTER ZoneManager's (bug caught in verification; the runtime was ordering-safe: SceneRouter restores before preview.enter). `npc_alive`/`npc_dead` removed from the condition dropdown (never implemented; tolerated no-op on old data). UI: `StateScopePicker` in ScriptPanel across actions/conditions/state-trigger + transfer editor. Verified live incl. instance independence, clamped scoped hits, this-entity death handler, conserving transfers, New-Game reset (acceptance: `test-plans/phase-60-entity-state.md`; a pre-existing test-harness spawn+update mesh-duplication artifact was hit and documented, not fixed). Guide: STATE_ITEMS_GUIDE.md "Layer 4".
- v4.74.0 — **Phase 59 — trigger volume shapes: sphere, cylinder, capsule.** Standalone trigger volumes were box-only; attached object colliders already spoke all four shapes. **`TriggerVolume.shape?`** (absent = box, old scenes untouched) with the `AttachedCollider` size encoding — sphere `x` = radius; cylinder/capsule `x` = radius, `y` = full height; `position` stays XZ center + Y BOTTOM for every shape. New **`src/world/volumeShape.ts`** (`volumeExtents` + `volumeGeometry(vol, inset)`) shared by ZoneManager visuals (wireframe/fill/front-arrow), the gradient `visual` fill (uSizeY = shape height), and TriggerVolumeTool's AABB pick. Sensor: `registerVolumeSensor` emits `cuboid`/`ball`/`cylinder`/`capsule` (capsule half-height clamped `≥0.01` so `h<2r` degrades gracefully) on both the static and mover-host paths — Phase-53 attached volumes ride in any shape, runtime parity free. Editor: TriggerVolumeView gains a SHAPE row + per-shape SIZE fields (W/H/D vs RADIUS[+HEIGHT], radius floor 0.25) with footprint-preserving conversion on switch; **drag handles work on every shape** (ColliderEditor semantics on the Y-bottom convention: round side handles drag the radius with XZ center pinned, +Y drags height — the radius on spheres — bottom pinned, −Y drags the bottom top-pinned and is hidden on spheres; handle set rebuilds on shape change). A first cut shipped box-only handles citing a stale COLLIDERS_GUIDE "round shapes are numeric-only" note (user correction; guide + stale ColliderEditor header comment fixed). Verified live incl. a shape proof: a player inside the old box AABB's corner but 2.68 m from the r=2 sphere center does NOT fire `on_player_enter`; entering the sphere does. Plan: `plans/phase-59-trigger-volume-shapes.md`; acceptance: `test-plans/phase-59-trigger-volume-shapes.md`.
- v4.73.2 — **Play camera tracks window resizes + correct aspect in editor preview.** User report: resizing the window mid-play stretched/skewed the game (preview AND runtime — same path). Two gaps in `SceneManager`: `_handleResize` updated only the editor camera's aspect (the active `_previewCamera` kept its stale ratio against the resized canvas), and `setPreviewCamera(cam)` never stamped the play camera's aspect on entry — the camera was constructed from the WINDOW size while the editor canvas is the VIEWPORT (minus toolbar + properties panel), so editor preview rendered a few percent horizontally stretched its whole life (the exit path restoring the editor aspect was the tell). Fix: `_handleResize` also updates `_previewCamera` when active; `setPreviewCamera(cam)` sets `cam.aspect` from the canvas. Verified in the runtime: camera aspect tracked the canvas exactly through a 1502→900px resize (1.8356→1.2448, both matching). (autosave-vs-disk freshness check).** User report: pressing TopBar ↻ repeatedly never picked up scene-file changes made outside the editor (Claude editing `level_1.json` directly) — the boot sequence restored any <24h autosave unconditionally, only reading the scene file when no autosave existed, so a window's autosave shadowed disk edits on every reload (and a later Save would have silently reverted them). Fix in the App.tsx boot IIFE: the project session is read up front (`restoreLastProject` hoisted; the restore block below reuses it) and a new **`sceneFileMtime()`** helper HEADs `/games/<id>/scenes/<sceneId>.json` (serveDir emits `last-modified`); when the file is **newer than the autosave's savedAt**, the autosave is skipped (console-noted) and the existing `!restored` path loads the scene from disk. Unsaved work still survives ↻: beforeunload re-writes the autosave at reload time, making it newer than any prior file edit — the content-gate (v4.14.1) means a CLEAN window writes nothing and disk wins. Verified: a tab sharing the stale workspace autosave + project session loads the disk values (characterScale 0.9) instead of the autosave's 0.6. User report: the follow camera sat low/level (raising CAMERA HEIGHT lifted the pivot but kept looking forward), and the avatar dwarfed a crab NPC — invisible when walked up to. Two fixes. (1) **`PlayerSettings.thirdPersonPitch?`** (degrees DOWN, default 0 = old behavior): seeds the orbit `_pitch` in the `CharacterController` constructor (clamped to ±MAX_PITCH; third-person only) — looking down raises the spring-arm camera and aims it at the pivot, so the authored start view can be "up and angled down" while mouse-look stays free. Panel: CAMERA ANGLE row after CAMERA HEIGHT (third-person only). (2) **`character.gltf` was 3.75m tall natively** while the capsule assumes a 1.8m character (radius 0.3 + halfHeight 0.6, ×2 each, at scale 1) — the visible avatar was 2.08× its own collider at every Character Scale. Fixed in the asset: a `__scalefix` wrapper node (scale 0.48, re-origin-tool pattern) makes the model 1.8m native, so visuals match collision at any scale; skinned mesh scales correctly because the armature root sits under the wrapper (same mechanism as the controller's own `root.scale.setScalar`). Applies wherever the asset is used (incl. two scenery placements in test-project2; `character_gun.gltf` NOT touched — still 3.75m, same fix applies if adopted). platfrom-obby authored to `thirdPersonHeight 0.8` + `thirdPersonPitch 18`; verified in the runtime — crab visible standing adjacent.
- v4.72.0 — **Dev-shell tooling parity: `desktop:dev` watcher task, dev globals in shell-served pages, no-store HTML.** Three dev-loop fixes discovered testing the shell as the daily driver. (1) **`deno task desktop:dev`** = `desktop:hmr` + `npm run build:watch` (`vite build --watch`, ~3s incremental, no typecheck) — `src/**` saves land in `dist/` automatically; the manual `npm run build` step is gone from the day-to-day loop. (2) **`isDesktopDev()`** (`src/shared/desktopApi.ts`): `detectDesktop()` now caches the `getAppInfo` body; when the shell reports `dev: true`, `App.tsx`/`RuntimeApp.tsx` install the `window.__*` dev globals + `__test` harness after boot (extracted `installDevGlobals()`, called immediately under vite DEV as before, or post-detection under the shell) — the production dist the shell serves had compiled them out (`import.meta.env.DEV` false), leaving shell-origin test tabs without the TESTING.md programmatic harness. Packaged apps report `dev: false` → end users never get globals. The runtime's playing-mode `FpsCounter` gained the same `|| isDesktopDev()` gate. (3) **`desktop/serve.ts`**: dist-served **HTML** responses get `cache-control: no-store` (hashed chunks stay cacheable) — the browser's heuristic cache was pinning a stale `index.html` → stale bundle hash across rebuilds, silently defeating the reload-after-rebuild loop. TESTING.md §0 restructured desktop-first around all of this (quick-start checklist + mode-map table; §3 steps marked `[vite]` where StrictMode/HMR-specific).
- v4.71.0 — **Phase 58 — packaging matrix + final FSA cleanup: the conversion ships.** `deno.json` gains `compile:mac-arm64` / `compile:mac-x64` / `compile:win-x64` / `compile:all` — all cross-compiled from one machine (CEF backend, `--include dist` so the stock library ships in-binary per the overlay model, `--exclude node_modules --exclude-unused-npm` saves 110 MB), outputs gitignored under `build/`. Measured: **421 MB mac-arm64 .app, 458 MB mac-x64 .app, 273 MB win-x64 .msi**; the arm64 app verified in packaged mode (fresh workspace → `dev=false`, dist from embedded VFS, editor + chunks + stock assets all 200). Cleanup: **`src/fsa.d.ts` and `src/lib/fileHandleStore.ts` deleted**; TopBar loses the `onLoadFSA` branch, the dead reopen banner (`projectPendingName`/`onProjectReopen`), and the Publish prop/block. README + PUBLISHING_GUIDE §0 rewritten around the desktop app and Export game…. Known follow-ups (plans/phase-58): signing/notarization (ad-hoc signatures — Gatekeeper right-click→Open on other Macs), `Deno.autoUpdate()`, deploy providers, app icon, real-hardware mac-x64/Windows smoke. Plan: `plans/phase-58-desktop-packaging.md`; acceptance: `test-plans/phase-58-desktop-packaging.md`.
...
- v4.63.2 — **"★ this object" populates the CLIP dropdown; `hazard_spiketrap` split into holdable Up/Down clips** (user, on a timed spike trap: "why does this objects selected script, not load any clips for the 'this object' on play_animation?" and then "repeating intervals always unplay the animation at the end… they don't hold at all"). **Two independent problems, one symptom.** (1) *Editor:* `ActionFields` resolved clips with `zoneObjects.find(o => o.id === action.targetId)`, but the `★ this` option saves the **literal sentinel `"self"`** (kept literal so scripts survive duplicate/copy/prefab stamping — `ScriptEngine.loadZone` rewrites it to the owner id at load, `ScriptEngine.ts:176-181`). `"self"` is never an entity id, so `targetObj` was `undefined`, `targetClips` was `[]`, and the CLIP `<select>` degraded to the free-text input with the actively-misleading placeholder "pick an object target for a list" — *while an object target was picked*. Fix is one resolve step (`resolvedTargetId = action.targetId === "self" ? owner?.id : action.targetId`); the placeholder corrects itself since it keys off the same `targetObj`. This is the third bug in the same family as v4.61.2/v4.61.3 — an unpopulated CLIP list is the standard symptom of a target the panel can't resolve, and `"self"` was the one case v4.61.2 explicitly left as "unchanged and correct". (2) *Asset:* `animationHold` was working perfectly and still looked broken, because **`SpikeTrap_Activate` is a complete pop cycle, not a rise** — decoded from the .gltf, its 14 LINEAR translation keys on node 0 (`Spikes`) go y=0 → 0.5005 by t=0.125, plateau to t=0.333, back to **y=0 by t=0.542**. `clampWhenFinished` freezes the *final* frame, which is spikes-**down**, so no script wiring (extra scripts, an open/close state flag, a second retract action) could ever hold them up — the clip had no up-pose to hold. Split additively via a Python re-bake (`gltf-clip-authoring` skill): **`SpikeTrap_Up`** = keys 0–8, times unchanged, ends y=0.5005; **`SpikeTrap_Down`** = keys 8–13 retimed to 0–0.208, ends y=0. Rotation channel dropped from both (2-key constant identity = inert). `SpikeTrap_Activate` byte-identical and the original 25680-byte buffer preserved as an exact prefix (+240 bytes appended), so the second trap and any other consumer are untouched; both names added to the asset's `animations` list in `public/assets/models/manifest.json`, which is what the CLIP dropdown actually reads. Authoring shape is then **one script, no global state**: `on_timer` interval 3 / repeat, action 1 `SpikeTrap_Up` + Hold at delay 0, action 2 `SpikeTrap_Down` + Hold at delay 1.5 — up-time and cycle length tune independently via the per-action `delay` (`ScriptEngine.runActions`). Verified numerically against the written file (all 19 accessors resolve in range; per-clip key count / start / duration / end-Y asserted).
- v4.63.1 — **Player one-shots/holds play through movement; "★ the player" gets its own group** (user: "it doesn't play. also why is player listed as 'this' in that dropdown" — their death script fires while a movement key is held, and v4.63.0's blanket move-cancel killed the Death one-shot the next frame). The move-cancel now applies to LOOPING clips only (ambient emotes shouldn't moonwalk); one-shots and holds are deliberate beats usually fired mid-input (death, hit-react) and play through — a finished one-shot still returns to locomotion, hold still keeps the pose until `__auto__`/warp/climb. Picker: the pinned player entry moves from the "This" group header to its own "Player" header. Verified by manual stepping: Death fired while W held played its full ~1s (was cancelled in 1 frame), and a looping Wave still cancels to walk on the first movement frame.
- v4.63.0 — **play_animation can target the PLAYER — a script-override channel on the avatar's locomotion state machine** (user: "I can't seem to do action play_animation on the player, as a target. that odd?" — not odd: the avatar's animation is owned by the idle/walk/jump/climb state machine, which re-asserts a clip every frame, so the player was deliberately absent from the target list). New sentinel target **`"player"`**: the engine emits **`character:play-animation { clipName, loop?, hold? }`** (never entity-resolved); `CharacterController` gains `_scriptAnim` + `_playByName` (exact clip-NAME lookup, case-insensitive fallback — unlike `_play`'s intent mapping; `_currentClip` = `script:<name>` so it never collides with locomotion intents). **Override semantics:** `_updateAnim` short-circuits while a script clip is active; it clears on movement input (no moonwalking — locomotion takes back over), on a finished one-shot (hold keeps the final pose indefinitely), on `"__auto__"` (the pinned resting-clip option = return to normal movement anims), on teleport/respawn, and on ladder mount (climb owns the animation). **Editor:** `ActionTargetPicker` gains `includePlayer` — play_animation uses a dedicated picker pinning `★ the player` above `★ this object`; the CLIP dropdown lists the avatar model's clips via new `playerModelAssetId` threading (App → LeftPanel → ScriptPanel → ScriptEditor → ActionRow → ActionFields, from `world.playerSettings.modelAssetId`; UI-menu option rows skip the thread and fall back to free text). Verified by manual stepping with the input pump on the user's avatar ("character", 19 clips): idle → `script:Wave` looped and held; a movement tap cancelled cleanly to walk → idle; `Death` with hold kept the pose 4s past clip end; `__auto__` returned to idle; the real asset registry lists all 19 clips for the dropdown. (Debug detour: an apparent jump-flicker on cancel was actually the probe walking the avatar off the platform edge — legitimate airborne transition, not a bug. And the un-versioned `import('/src/core/AssetManager.ts')` returned a dead duplicate registry — HMR-stamped URL required, per the TESTING.md trap.) Known v1 edges: a scripted pose persists through a no-input jump (vertical motion doesn't cancel), and only one script clip at a time (the object channel is separate).
- v4.62.5 — **Panel hint: on_health_zero names its hardcoded key** (user: "my death script don't seem to respawn player so like is on_health_zero working" — it fires only for the state key literally named `health` (ScriptEngine's `state:changed` gate), and the user had renamed theirs to `Hearts`, so the death script went silent; predicted as a risk when the rename was first spotted in the hearts-meter session). The ScriptEditor now shows an italic hint under an on_health_zero trigger: fires when the key named exactly `health` drops to 0 — use on_state_equals with your key and value 0 otherwise. The user's fix is authoring-side: death script trigger → `on_state_equals`, key `Hearts`, value `0` (their schema has `min: 0`, so damage clamps to exactly 0 and the transition fires; GameState's emit-on-change keeps it from re-firing while dead, same as on_health_zero's semantics).
- v4.62.4 — **Avatar facing uses the input direction only — fixes the slight spin while moving backwards through a launch** (user: "the character seems to kind of spin slightly most of the time when moving backwards. is that odd?"). The third-person facing target was `atan2` of the FULL frame displacement `dir` — which includes gravity, mover carry/push, and (since v4.62.0) the decaying horizontal launch shove. During a launch tail the target heading is input-blended-with-shove and sweeps as the shove decays; moving backwards puts it near the ±180° wrap where the shortest-turn side flips — the visible micro-spin. Fix: capture `faceX/faceZ` right after the input vector is built (before gravity/launch/mover terms join `dir`) and aim the model at that; a launch alone no longer turns the avatar at all (facing holds when input is idle, matching platformer feel). Verified by manual stepping WITH the input pump — the v4.55 gotcha bit again: the first probe held KeyS without `preview.input.update(dt)` and produced a trivially-constant yaw (no input registered); with the pump, walking backwards through a backward launch converged 12°→0° and held exactly 0° across the whole decay, and a strafe still swung smoothly to 90°.
- v4.62.3 — **New launch_player actions default to owner-relative direction** (user, after their rotated pad kept launching world-aimed because the v4.62.1 checkbox was unticked: "in future that should be checked by default"). `ActionRow`'s type-switch — the single creation path for every action, since + Add makes a set_state and the user re-types it — now seeds `launchRelative: true` when switching to launch_player on an entity-owned script (no owner → no seed, the flag is meaningless there). Deliberately NOT a semantic default: an absent flag still means world-compass, so every saved world (including the user's two straight-up bouncer pads) is byte-identical in behavior; only freshly created actions start ticked. Diagnosed from the user's live data: their pad had `rotY −179.3, dir 180, relative absent` — rotating the volume turned the box+arrow but not the launch, and a dispatch probe with `relative: true, dir 0` resolved to −179.3 ≈ the working 180.
- v4.62.2 — **Trigger volumes show a front arrow, spawn-marker style** (user, on the relative launch direction: "0 is front? I was thinking like an arrow, like spawn points have"). `_buildVolumeMesh` adds a cone child to every volume wireframe, poking out of the front face (local −Z — what launchRelative's "direction 0" means), tinted like the wire (`editorTint` respected), sized `0.16r/0.45h × clamp(min(w,h)/2, 0.9…1.6)` so it stays readable on big volumes without dwarfing small ones. As a wire child it rides rotation, mover attachment (Phase 53), and the existing dispose traverse for free; editor-only like the interior fill — `_setVolumeFillsVisible` now toggles ALL wire children (was `children[0]`), so preview/game hide it. Verified in-browser on the user's CrabBiteTrigger: 9/9 volumes carry cones; framed shots show the arrow out the front face and rotating with the box.
- v4.62.1 — *(superseded by v4.63.3: `launchRelative` is now the deprecated fallback for `launchRelativeTo: 'entity'`, and the checkbox described below is a 3-way World/entity/Player picker — the runtime behavior of the entity mode is unchanged.)* **launch_player direction can be relative to the owning entity's rotation** (user: "the 'direction' property is relevant to what, the world? It would be nice if the trigger could have like a front, be rotated, and then a 0 direction would always be like back towards the front"). New `ScriptAction.launchRelative` checkbox ("Direction is relative to this volume's rotation (0 = its front)" — shown only on entity-owned scripts): at dispatch the engine adds the owner's Y rotation to `launchDirDeg`, so a rotated dash-pad volume launches out of its own front (the −Z face at 0°) and turning ROTATION (Y°) turns the launch with it. Plumbing: entity triggers already stamp the owner into `trigger.targetId` (v4.57-era `_ownedScript`), so `_runActions` now threads it as `runActions(actions, ownerId?)` → `_dispatch(action, ownerId?)`, and the launch case resolves rotation via the existing `_resolveObjectPose` (which returns `facing` for volumes/objects/platforms/shapes/checkpoints — so an object-owned launch is owner-relative too). DialogueRunner's `runActions` passes no owner → the flag falls back to the authored world direction. Verified through the engine's public dispatch with a bus spy: relative+owner rotated −90° → dirDeg −75 (15 + −90); relative with no owner → 15; flag off with owner → 15.
- v4.62.0 — **launch_player gains a horizontal component (dash pads / angled boosters)** (user: "the launch_player action should have a horizontal equivalent. does it?" — it didn't: the action wrote only the jump channel, and horizontal motion is recomputed from input every frame with no persistent velocity). `ScriptAction` gains **`launchHSpeed`** (m/s, 0/absent = straight up) and **`launchDirDeg`** (spawn-facing compass: 0 = the way a fresh spawn faces = world −Z; 90 rotates to −X — same convention as `facing`); `character:launch` payload carries them. `CharacterController` gains a **horizontal launch/knockback channel** (`_extVelX/Z`): set on launch (replacing, not stacking, so repeat pads feel consistent), added into the KCC move each frame (walls still block), **exponentially damped — −1.2/s airborne so an arc carries, −8/s grounded so landings kill the slide** (a 10 m/s ground dash travels its theoretical v₀/k ≈ 1.25 m and stops in ~½ s), snapped to zero below ~0.2 m/s; zeroed on teleport (no shove through a warp) and on ladder mount. Panel: Upward / Sideways / Direction fields on one row + updated hint ("Set Upward to 0 for a pure sideways dash"). Verified via deterministic manual stepping (hidden tab, HMR-stamped PhysicsWorld import per TESTING.md): dirDeg 0 arc moved pure −Z (X frozen to the cm), initial carry ~8.7 m/s decaying gently airborne, vertical peak matching v²/2g; dirDeg 90 grounded dash slid −X 1.27 m and stopped.
- v4.61.3 — **play_animation gains a pinned "↩ auto-play (resting) clip" option — the script-reachable STOP** (user: "in the clips list, the 'auto play' animation should be an option, pinned near the top. kind of like the '★ this target' is for targets"). The CLIP dropdown pins sentinel value **`__auto__`** right under "— pick clip —"; `ObjectPlacer`'s `object:play-animation` handler routes it to **`stopPreview(id)`**, which crossfades to the target's auto-play resting clip (or fades to bind pose when none is set) — so one option both "plays the resting clip" AND stops a looping/held scripted clip, closing the "no stop_animation action" gap documented two turns earlier. Loop/Hold/Blend row hides while `__auto__` is selected (the stop path ignores them; the resting clip always loops); `clipKnown` treats the sentinel as known so it doesn't duplicate as "(custom)". Both shells inherit (same ObjectPlacer). Verified via the real dispatch on the user's Crab1 (autoPlay "Idle"): bus trace `start:Bite_InPlace` (loop) → `__auto__` → `stop:obj_e043` (crossfade back to Idle). Also: TESTING.md §3 gains rule 8 — a state write and DOM read in the SAME `javascript_tool` call sees the pre-update DOM (React batches); write and read in separate calls.
- v4.61.2 — **Object-only action targets no longer offer "★ this volume"** (user, on a bite-trigger volume: "when doing an action of play_animation on a target of 'this volume' I don't get a real list of animation clips. I instead need to select the actual crab"). `ActionTargetPicker` added the `★ this ${owner.kind}` self option unconditionally, but the shared object-only picker (play_sound-at-object / open_door / close_door / move_object / change_material / play_animation) and the mover picker (start/stop/toggle_mover) serve actions that no-op on volumes at runtime — picking "this volume" saved a `self` target that resolved to the volume id and did nothing, with play_animation's CLIP dropdown degrading to free text as the only symptom. Both pickers now pass `owner` only when `owner.kind === "object"` ("★ this object" on object-owned scripts is unchanged and correct); `despawn_object`'s picker keeps the volume self-target (despawn genuinely works on volumes). Verified in-browser on the user's real content via a throwaway volume (created/deleted through the Trigger tool + Delete Volume): a volume-owned play_animation's target list shows Groups/Objects with no THIS section, and targeting Crab1 populates the CLIP dropdown with the model's ten real clips.
- v4.61.1 — **UI-tab element cards: the kind chip moved above the name and grew** (user: "kind chip is too subtle. needs to be a tad bigger and above the element name not next to it"). `UiElementRow`'s header is now two rows — an uppercase 11px kind chip (was 9px inline beside the name, reading as decoration) on its own line with the ✕ delete right-aligned, then the ELEMENT NAME field full-width. Verified in-browser: COUNTER / ICONS banners clearly label the user's two saved elements.
- v4.61.0 — **`icons` UI element kind — repeated full/half/empty icon meter (GTA stars / Zelda hearts)** (user: "how in GTA you can have 5 stars… if you have 2.5, then 2.5 more are also there but empty outlines… I need that but for like 3 hearts"; their health state is 0–3 with decimals and they'd made full/half/outline heart graphics). Existing kinds could not express it (bar = continuous fill, counter = literal "×2.5", image = static, no per-element conditional visibility), so the union gains **`UiIconsElement { kind:'icons', stateKey, count?=3, max?=count, fullGraphicId, halfGraphicId?, emptyGraphicId?, size?=24 }`** — `count` icons each worth `max/count` state units. Overlay render: value rounds to the **nearest half-icon when a half graphic exists** (else nearest whole); slot i is full at `v≥i+1`, half at `v≥i+0.5`, else empty (missing empty graphic → the full one at 0.25 opacity); test hooks `data-ui-icons`/`data-ui-slot`. Everything else inherited free by design: `state:changed` live re-render, `__ui.<id>` visibility, anchors/backdrop, scene/game.json persistence (no validator gates kinds), both shells. Editor: `icons` in `UI_KINDS` + `blankUiElement` (label "New Hearts", stateKey "health"), field block with the `wb-state-keys` datalist, Icons/Full-at/Icon-px `numField`s and three `GraphicIdField`s (Full / Half optional / Empty optional). Docs: GUI_GUIDE kind table + JSON reference recipe. Verified in-browser end-to-end through the real UI-tab editor on the user's Platfrom Obby project (kind dropdown → + New → Pick popovers): Hearts=3 → full×3, 2.5 → full,full,half, 1.2 → rounds to full,empty,empty, 0 → outline×3, live in preview at top-right beside their coin counter. **Gotcha caught live: DOM reads in the same synchronous `javascript_tool` call as `runAction(set_state)` see the PRE-render overlay** (React hasn't flushed) — set and read in separate calls. The authored element was kept at the user's request (game.json committed): bound to their real `Hearts` state key — their schema had replaced `health` (0–100) with `Hearts` (0–3), so the element's initial `health` binding read 0 and was corrected before saving.
- v4.60.4 — **Thumbnail stager: pan controls — move the model in the frame** (user: "a half heart is not in the same middle as a whole heart… move left right up and down either with some kind of drag, drag toggle, arrows I don't care"). The stage always centered the camera on the bbox center, so asymmetric art (the half-heart icon) sat off-center relative to its siblings. `StageParams` gains optional **`panX`/`panY`** (fractions of the fitted bounding radius; positive = model right/up; `DEFAULT_STAGE` zeroes them so Reset view clears pan and import-time/`ReoriginModal` renders are byte-identical). `ThumbnailStage.render` applies pan as a pure camera translation after `lookAt` — right/up basis columns off `matrixWorld`, camera moved by `-pan·radius` so the model shifts the intuitive way; orientation (and thus orbit feel) unchanged. Modal gets all three requested controls: a **Move row** (◀ ▶ ▲ ▼ nudge ±0.05/click + X/Y `NumField`s, −2…2, width 56 so "−0.05" isn't truncated — `NumField` gained an optional `width` prop), and **shift-drag pans** in the preview (`0.004/zoom` per px so the drag stays ~1:1 with the cursor at any zoom; plain drag still orbits); hint line now "drag to orbit · scroll to zoom · shift-drag to move". Verified in-browser on Closet: typed X 0.5 shifted the model right, ▲×3 → Y 0.15, ◀ → −0.05, plain drag orbited while the pan held, Reset zeroed pan, icon mode rendered the shifted model on the checkerboard. (Shift-drag itself can't be driven by the extension — modifiers don't reach page MouseEvents, per the 2026-07-21 finding — but it shares the exact commit path the verified nudge buttons use.)
- v4.60.3 — **Thumbnail stager: numeric inputs beside the Zoom/Light sliders** (user: "can the models > stage-thumbnail modal also have a numerical input for the zoom/light?"). Each slider row gains a 52px `NumField` — a numeric twin kept in sync through the same `params` state, so slider drags, wheel zoom, and typed values all agree. Typing uses a local draft string (a controlled clamped number input would turn "0.5" into 0.4 at the first keystroke), live-commits values clamped to the slider's min/max (`zoom` 0.4–3, `light` 0.2–3), and snaps the display back to the actual param on blur/Enter (typing 9 into Light commits 3 and shows 3 after Enter). Verified in-browser on the Closet model: typed zoom 2 moved the slider and visibly re-rendered the preview; Light 9 → clamped 3, brighter render; modal cancelled, nothing written.
- v4.60.2 — **Grid-floor toggle joins the EDITOR section** (user: "another option to hide that bottom black floor with the grid thing"). Third checkbox under CREDITS — "Grid floor (the dark ground plane + grid lines)" — hiding the editor's two `GridHelper`s and the 200×200 dark demo-ground plane. Same persistence pattern (`editorShowGrid`, `'0'` = off), default ON. New **`SceneManager.setGridVisible(v)`** (refs captured in `_setupGrid`): toggles `visible` on all three while LEAVING them in the scene — placement tools are unaffected (InputManager's `worldPos` is a math-plane intersect, and three's raycaster ignores `visible` anyway) — and sets the helpers' **`hideInGame` tag to `v`**, which is the load-bearing subtlety: game-mode exit runs ZoneManager's blanket `_setHideInGameVisible(true)` restore, and a still-tagged hidden grid would be resurrected by it. App applies the persisted value right after `new SceneManager(...)` (the effect runs once, before any toggle). Verified in-browser through the real checkbox on the Platfrom Obby tab: toggle off → both helpers + ground `visible:false`, sky renders where the dark plane was; **Start Game enter/exit → still hidden** (the restore-pass trap); reload → still hidden (boot path); toggle on → all three back with `hideInGame` tags restored. Autosave byte-identical throughout; the game save the Start Game exit overwrote was restored from the session snapshot.
- v4.60.1 — **Global toggles for the preview perf counter and crosshair** (user: "under editor settings under credits — could we add a global editor option to hide or show the fram/draw-call counter in the top left, also one for the crosshairs in the center"). Two checkboxes in the ToolView EDITOR section, under the CREDITS button: "Perf counter while playing (FPS · draw calls, top-left)" and "Crosshair while playing (center of screen)". Both default ON (current behavior) and persist to localStorage the way `editorQuality` does (`editorShowPerf` / `editorShowCrosshair`, `'0'` = off, absent = on) — global editor preferences, never SceneFile data. Wiring: App state + `handleTogglePerfCounter`/`handleToggleCrosshair` beside `handleQualityChange`; the perf flag gates the `{isPreview && <FpsCounter/>}` render, the crosshair flag becomes PreviewHUD's new optional `showCrosshair?: boolean` prop (default `true`, so RuntimeApp — which renders the same HUD and its own DEV-only FpsCounter — is untouched; the toggles govern the editor shell only). Verified in-browser through the real click path on the Platfrom Obby project tab: crosshair unchecked → preview showed the perf readout but no center crosshair; perf unchecked + crosshair rechecked → second preview showed the crosshair but no readout; both keys round-tripped localStorage and were restored to defaults after the session.
- v4.60.0 — **Graphics browser gains Manage mode: edit metadata + delete, at parity with audio** (user: "the graphics asset menu is missing all the editing… I can't delete an unused graphic right now"). `GraphicsBrowser` had no manage mode at all (flagged "No manage/delete mode in v1" since Phase 48); it now follows the AudioBrowser pattern — a **Manage** button beside "+ Import Graphics" turns the checkerboard tiles into multi-select checkboxes with Edit (n) / Delete (n) / Done. **Edit** → the shared `EditMetadataDialog` (noun union widened with `"graphic"`): label / category (defaults seeded `Icons`/`HUD` + categories in use) / attribution, single + bulk apply-to-all; no TAGS field because `GraphicDef` has no `tags`. **Delete** → the shared `DeleteAssetDialog` (noun `"graphic"`, files checkbox off by default like models), and the usage warning knows graphics are referenced **two different ways**: `ItemDef.icon` stores the graphic's *path* while UI elements store its *id* (`graphicId`), so `App.handleRequestGraphicDelete` scans `worldItems` by path and `world.world.uiElements` + `world.gameUiElements` by id (contexts surface as "item icons" / "game UI"). Confirm paths (`handleConfirmGraphicDelete`/`handleConfirmGraphicEdit`) follow the sound/skybox recipe exactly: `ensureDir(graphicsDir)` grant → splice/patch `public/assets/graphics/manifest.json` (`patchEntry` for edits; optional `removeEntry` of the image on delete) → `assetManager.removeGraphics(ids)` / `updateGraphic(id, patch)` (new registry mutators, no cache to evict) → `setGraphics`. Not carried over from the models browser: 📷 re-stage and ⌖ re-origin (3D-mesh-only concepts). Verified in-browser through the real UI (OPFS-stubbed picker per TESTING.md §9): delete of an in-use graphic warned "1 reference across 1 group" (the platfrom-obby coin counter's `coin_icon`), delete of an unused one showed no warning and spliced the manifest + grid, edit renamed `heart` in both manifest and tile with path/dimensions preserved; real `public/assets/graphics/` untouched.
- v4.57.0 — **Group ergonomics: duplicate no longer silently grows a group, Cmd+G groups a selection, and a member can select its group** (user: "when I duplicate items in a group, they should be in no group, or maybe a duplicated group... there should be a shortcut to add to a new group... one of the actions should be 'select group'"). **(1) Duplicate/paste group policy** (`copyPaste.ts`): `pasteClipboard` deep-cloned each def and dropped only the `prefab` stamp, so `groupIds` rode along and every copy silently joined the ORIGINAL group. New `planGroupRemap` decides per referenced group — copy the group's ENTIRE membership and the copies land in a fresh `"<name> copy"` group; copy a subset and they land ungrouped. The new group defs are created inside the paste transaction, so one undo takes the copies and the group together. Source memberships are snapshotted onto `ClipEntity.groupIds` at `buildClipboard` time (the single funnel for both `copySelection` and `copySelectionMulti`) rather than read back off the cloned def — paste REWRITES `def.groupIds`, so re-pasting the same clipboard would otherwise remap from the previous paste's result and drop the group on the second Cmd+V. Applies to Cmd+D and Cmd+C/Cmd+V alike (one paste path). **(2) `handleGroupSelected`** (App): mints a group and writes it onto every `multiSelected` ref in one transaction (group creation inside it, so undo removes the group too), then opens the Groups panel to rename. Bound to **Cmd/Ctrl+G** (previously unbound; guarded by the same `typing` check as copy/paste/duplicate) and surfaced as an `⊞ Group Selected` button in the multi-select PropertiesPanel. Shortcut works from 1 item up; button shows at 2+. **(3) Select group**: a full-width action in `ActionsAccordion`, one button per group the entity actually belongs to — labelled plain `"Select group"` when it's in one, `"Select group: <name>"` when in several, so multi-group membership needs no disambiguation dialog. Reuses the existing `handleSelectGroupMembers` (emits `selection:set`). A matching per-row `Select group` button also sits in `GroupsAccordion`, which is how trigger volumes get it (`TriggerVolumeView` renders no ActionsAccordion). **First pass put it ONLY in the Groups accordion and the user reported it missing — they had asked for it in the ACTIONS list, which is a different accordion.** Verified in-browser through the real paths (window `keydown` for both shortcuts, DOM clicks for every button): whole-group duplicate → `Crates copy` with both copies joined and originals untouched; subset duplicate → copy carries no `groupIds` and no group is minted; Cmd+G on 2 ungrouped → `New Group` holding both; `Select group` from one member → selection expands to all members (`2 selected`). Test entities and the four test groups removed afterwards, autosave verified clean.
- v4.56.3 — **Dirt tiled platforms stack with the kit's real band pieces (fixes the repeated lip, the dark centre square, and the shadow-acne stripes)** (user: "why does the dirt tile set for the tiled platform prefab have a different color center?", then "the sides do not blend well as you did not properly mix sides and tops"). Three defects, one wrong assumption. **v4.44.4 recorded "dirt only ships full-height blocks" — that is false.** The kit's `platform_grass_*_center_tall` / `*_bottom_tall` pieces carry **no green at all** (materials are `Dirt_1/2/3`) and have **no top face** — they ARE the dirt body of a grass platform. `assetFor`'s dirt branch was repeating `platform_dirt_*_tall` for every band, and that piece carries a beveled top rim meant to END a platform, so each layer stacked its own rounded lip with a shadow gap beneath it. The dirt branch is now the grass stack with only the top band differing: top = `platform_dirt_*_tall` + flat cap `platform_dirt_center`, middle = `platform_grass_*_center_tall`, bottom = `platform_grass_*_bottom_tall` + underside sheet `platform_grass_bottom_tall` (dirt interiors previously got no underside at all). **The dark centre square** was the interior cap using `platform_dirt_center_tall` — a side-band piece whose `Dirt_1` is `0.3895` where every other tile in the kit uses `0.48689` (that value appears in exactly 1 of 24 pack files). Recolouring it was the wrong fix and was reverted; `platform_dirt_center` is the correct flat cap and its `Dirt_1` already matches, so the tops agree with **zero** colour edits (measured off the render: centre `177,133,87` → `196,163,130` against neighbours at `191–196`). **The faint horizontal stripes** on corner/side tops were shadow acne: the pack marks these closed solids `"doubleSided": true`, and three.js derives `shadowSide` from `side`, so a DoubleSide material casts from its own lit top face and self-shadows. The centre tile was the one file in the set WITHOUT the flag, which is exactly why it alone rendered clean — the tell that cracked it. Flag stripped from the 7 dirt tile glTFs (one line per material, nothing else touched); verified from above (stripes gone) and from the side (`Dirt_2/Dirt_3` strata bands all still render — a blanket `shadowSide=BackSide` was rejected instead because 8 placed assets contain thin single-quad double-sided submeshes that would lose their shadows entirely, and a global `sun.shadow.normalBias` 0.03→0.08 was rejected as a lighting-wide change for a per-asset defect). Verified by swapping all 24 lower-band tiles live on the user's own 4×4×3 instance — continuous strata top to bottom, no lips, no gaps — then reverting every swap with the autosave confirmed untouched. **Existing instances need a regenerate to pick this up** (members are baked). Known remainder: `platform_grass_*_tall` still carries `doubleSided`, so grass tops still self-shadow. **Riding UI fix:** the left toolbar rail rendered inactive icons AND labels at `#7a7a7a` with a further `opacity: 0.7` on the label — roughly `#5c5c5c` on the `rgba(28,28,28)` rail. It now uses the same pair the ASSETS flyout already used (`#9aa3b5` icons / `#c2cadb` labels, full opacity) across all four button variants (tools, Assets, Prefabs, Scripts); active tints unchanged.
- v4.56.2 — **Damage-over-time recipe (lava) in HAZARDS_GUIDE + the timer Repeat checkbox** (user: "is there like an action that can cause something over time, like minus 5 health per second?" then "there should be a section about in_lava and trigger intervals... with screenshots"). The composable answer — volume enter/exit scripts flip an `in_lava` flag; a LEVEL-scope `on_timer` interval-1 **repeat** script conditioned on `has_state in_lava` runs `adjust_number health -5`; the central death handler catches 0 — is now a full guide section with two new screenshots (`docs/images/lava-pool.png`, `lava-tick-script.png`) plus poison/regen/moving-drain variations. Writing it exposed a real gap: **`ScriptTrigger.repeat` was engine-supported (`_startTimers` setInterval vs setTimeout) but had NO panel control** — UI-authored timers could never repeat, so the whole recipe was unbuildable outside hand-written JSON. `ScriptPanel`'s on_timer fields gain a "Repeat every interval (off = fire once)" checkbox. Verified live end-to-end with every script authored through the real panel: one recorded event trace shows enter → `in_lava=true` → wall-clock ticks 15→10→5→0 → `on_health_zero` → respawn (fade/teleport/health 100/fade-out) → exit → `in_lava=false` → drain stops. (The trace also captured a hidden-tab-only artifact — the frozen TriggerSystem delayed the post-respawn exit event, so the drain re-killed the respawned player until manually stepped; visible tabs fire the exit within a frame. Documented in the test plan.)
- v4.56.1 — **HAZARDS_GUIDE.md + a state-trigger authoring fix on entity scripts.** New illustrated builder guide (`HAZARDS_GUIDE.md`, screenshots in `docs/images/`: crusher rig, ATTACHED TO dropdown, on_state_equals fields, respawn_player fields) covering the central death handler (`on_state_equals health == 0 → respawn_player`), kill floors, crushers, enemy sight-cones, damage-over-death, and the checkpoint fallback chain; GAMEPLAY_STATE.md §5a's manual checkpoint loop now cross-links it. Riding fix, found while capturing the screenshots through the real UI: on an ENTITY-owned script (a volume/object's script list), `on_state_changed`/`on_state_equals` rendered the "Target: this volume (implicit)" hint instead of the state-key input — the key was unauthorable, and the engine's volume-script targetId injection would then mis-key the trigger to the volume id. The TargetPicker gate now treats state triggers like `on_dialogue_end` (always shows the picker, which for state triggers IS the key input). Acceptance: `test-plans/phase-53-state-respawn-attach.md`.
- v4.56.0 — **Phase 53 (part 3) — attachable trigger volumes: sensors that ride movers** (user: crushers that kill from below, enemies whose sight-cone moves with them). `TriggerVolume.attachTo?: string` — a mover-enabled platform/shape/object in the same zone; the def keeps its **world-space rest pose**, so the Trigger tool, resizer, gizmos and panel position fields needed zero changes — host-local conversion happens only at collider build (`ColliderBuilder.registerVolumeSensor(vol, host?)`: `inv(originQuat) × (worldCenter − origin)`). The enabling physics primitive is **`PhysicsWorld.createSensorColliderOn`** — a sensor parented to a kinematic body with `KINEMATIC_KINEMATIC | KINEMATIC_FIXED` active types; KINEMATIC_KINEMATIC is load-bearing (kinematic sensor parent vs kinematic player capsule produces zero intersection pairs without it — the exact case `registerAttachedColliders` declared out of scope in Phase 31). `MoverSystem` gains `hostFor`/`attachMeshes`/`detachMeshes`; the volume's wireframe AND gradient fill ride the host (the fill is a runtime-visible hazard glow) while remaining zone-group children. **Lifecycle invariant:** host rebuilds free the kinematic body and `register()` wipes attached meshes, so `ZoneManager._reattachVolumes(hostId)` remove+re-adds attached volumes synchronously at the end of every host build path and in the host removed handlers (deliberately NOT via `*:rebuilt` bus events — ZoneManager owns every site; also new `isValid()` guards, since an attached sensor's collider dies WITH the freed host body). Fallback: missing/mover-disabled/body-less host (CSG/polygon platforms, degenerate hulls) → the classic static world-space sensor. `TriggerSystem` is untouched (same handle→id map, per-frame overlap diff — which is exactly why a sensor sweeping over a STATIONARY player fires). Copy/paste gains an entity-id pre-pass map so a host+volume copied together stay attached (volume copied alone keeps the original host); prefab expansion remaps `attachTo` through the member id map like `remapScripts`. Panel: ATTACHED TO select on the trigger-volume view (mover-enabled entities in optgroups, dangling ids shown as "(missing/no mover — static)"). Verified in-browser on a slide-mover crusher rig: sensor parented (bodyType 2) at the exact converted pose while an orphan control stayed fixed at its authored pose; sensor/wireframe/fill tracked the platform in perfect lockstep through a full 4m leg + ping-pong turn (constant 0.95 offset at every sample, manual mover+physics stepping — tab was OS-hidden); **the crusher case fired** (stationary player: enter → exit → enter as the sensor swept down, away, and back); platform rebuilds re-attached (still kinematic, still moving, collider/body counts flat across 3 rebuilds); preview exit restored sensor + visuals to rest exactly; pasted platform+volume pair kept `attachTo` on the fresh platform id. Plan: `plans/phase-53-state-respawn-attach.md`.
- v4.55.0 — **Phase 53 (parts 1+2) — `on_state_equals` trigger + `respawn_player` action + real fade-out** (plan: `plans/phase-53-state-respawn-attach.md`). **Trigger:** `TriggerType` gains `on_state_equals` (`targetId` = state key, new `ScriptTrigger.stateValue: JsonValue` = the value to match); `ScriptEngine`'s single `state:changed` subscription — whose payload always carried `value`, previously discarded — now also runs `_fireStateEquals` (reads the `on_state_equals:<key>` index bucket directly, NOT via `fire()` which runs whole buckets; equality mirrors `GameState._equal`) and fires the **previously-dead `on_health_zero` stub** when health transitions to ≤ 0. Transition-only semantics fall out of GameState's emit-on-real-change: sitting at the value can't re-fire, recovery + re-hit fires again (verified live: `dead=1` → no-ops while clamped at 0 → `health=50` → `health=0` → `dead=2`). Panel: trigger in the dropdown, state-key input reuses the TargetPicker fallback + `wb-state-keys` datalist, "Equals value" input parses via the existing `coerceStateValue`. Documented edges: schema-seeded values never emit (no fire on initial value); `delete_state` emits `value: null`. **Action:** `respawn_player` — fade in (default 0.4s) → under cover: destination **stored-pose `positionKey` → checkpoint `targetId` (via the checkpoint-aware `_resolveObjectPose`) → `world.defaultSpawn`** → `character:teleport` → optional `restoreHealth` via new public `GameState.resetKey(key)` (re-seeds from schema default THROUGH `set()` so it clamps + emits; `reset()` deliberately stays silent — New Game must not fire state triggers) → fade out; the whole timer lives in `_timers` so `deactivate()` cancels it (verified: exiting preview mid-fade → no late teleport, overlay cleared, editor input alive). **The fade freeze bug is fixed:** `overlay:fade-in` suppresses input in BOTH input layers and `overlay:fade-out` — declared since Phase 10.5 — was emitted by *nothing*, so one authored `fade_screen` froze the player for the rest of the session. Now: `FadeOverlay` gains `direction: "in"|"out"` ("in" ramps opaque and HOLDS; "out" ramps clear then unmounts; duration-0 out = the old hard cut), both shells subscribe `overlay:fade-out` + clear fade state on true preview exit (guarded so router transitions keep their covering fade), `InputManager` un-suppresses on `preview:stop`, `fade_screen` schedules an engine-side `fade-out {duration:0}` at fade end (today's exact visual, input released, one site serves both shells), and `SceneRouter.go()` emits a real fade-out on arrival — runtime scene transitions become a genuine fade-through-black (error path fades out at duration 0 so the error screen isn't blacked out). Verified in-browser end-to-end in BOTH shells, incl. runtime scene_01→scene_02 (fade-in → load → fade-out, no residual overlay) and held-W movement after arrival (~6m/s via faithful input+physics+controller stepping — note for future testers: manual stepping must include `preview.input.update(dt)`, the suppression gate lives there). Docs: GAMEPLAY_STATE.md §4/§5, STATE_ITEMS_GUIDE.md; acceptance: `test-plans/phase-53-state-respawn-attach.md`.
- v4.54.0 — **Cylinder collider shape + the shape row becomes a dropdown** (user: "could we add a cylinder type? the row for those buttons may be out of space so might need to change it to a dropdown"). `AttachedColliderShape` gains `"cylinder"` — same params as capsule (`size.x` radius, `size.y` FULL height, full XYZ rotation) but flat-ended: `colliderWorldTransform`'s half-height is simply `h/2` (no cap-radius subtraction) and `ColliderBuilder` emits `RAPIER.ColliderDesc.cylinder(halfH, r)` (verified live: `shape.type === 10` with exact radius/halfHeight). It rides the capsule rails everywhere else — Rotate gizmo offered, Resize handles (±X/±Z radius drags center-pinned, ±Y height drags far-end-pinned; cylinder's Y pin is plain `halfExtents.y`, no cap-end add), ROTATION row, Refit (r = max(w,d)/2, h = bounds height) — via a shared `round = capsule || cylinder` predicate in `ColliderEditor._onDragMove`. Editor wireframe is a `CylinderGeometry(1,1,1,12)` scaled `(r, h, r)` — unlike capsule it needs no per-frame geometry rebuild. **The per-card shape button row is now a labeled SHAPE `<select>`** — five shapes ("cylinder" is long) don't fit the panel's 280px min width; hull keeps its auto-fit-on-pick behavior as an option (disabled when `hullPointsFor` is unavailable, suffixed "(auto-fit from model)"), and trimesh appears as a display-only option when it's the current shape (bake-only, never offered). `reshapeCollider` conversions: box ↔ cylinder = `2r × h × 2r` (a cylinder→box fall-through into the hull-verbatim branch was caught in-browser and fixed), sphere → cylinder = r, h=2r, capsule ↔ cylinder = identical params passthrough. Verified in-browser on a spawned rock: Rapier cylinder registered + rebuilt live across dropdown switches driven through real change events (round trip exact), cylinder wireframe hugging the collider in the viewport, 6 resize handles shown, panel offers Move/Rotate/Resize + R/H fields + Refit. Handle-drag geometry on cylinder shares the already-drag-tested capsule code paths (v4.48.0); a live drag remains the one-click human check.
- v4.53.2 — **Clicking `More ▾` now closes an open popout** (user report, tag mode). The button always had a toggle (`setPopoutOpen(v => !v)`), but the outside-click closer runs on document **mousedown** and treated the More button as "outside" — so a click on More while open closed the popout on mousedown, then the button's own **click** toggled it right back open, reading as "clicking More does nothing." Fix: the outside-click handler also excludes `moreBtnRef` (like `popoutRef`), so the button's click event is the sole owner of the toggle. Applies to both category and tag mode (same button). Verified in-browser: More opens → More closes; clicking elsewhere with the popout open still closes it.
- v4.53.1 — **`animated` tag auto-assigns at import** (user: "could it be an auto assigned tag? to existing ones, and for future ones? assigned only to models with animations even when imported along with ones that dont? or would a dedicated filter toggle be best?" — auto-tag chosen over a dedicated toggle: the tag rides the existing filter system with counts and AND-composition instead of adding a third filter mechanism). `ModelImporterModal` unions `"animated"` into each entry's tags **per model** when that model's clip discovery found animations — the batch-shared tags field stays shared, so a mixed batch tags only the animated members. It's a **seed, not an invariant** (assigned at import only, mirroring the Phase 51 backfill philosophy): removing the tag from a model later sticks, nothing re-adds it. The library's 5 animated-but-untagged assets (`character_gun`, `skull`, `crab`, `bee`, `enemy` — all imported after the Phase 51 backfill ran, which is exactly the gap this closes) were backfilled directly in the manifest, bringing `#animated` to 22 = every asset with clips.
- v4.53.0 — **▶ animation badge on asset tiles** (user: "add like a tiny icon or something to the corner of an asset in the asset manager — if it has-animations?"; icon style picked via option list: play triangle over running figure / film strip / keyframe diamonds). Tiles whose `asset.animations` is non-empty render a 14px dark chip in the thumbnail's top-right (mirror of the Manage checkbox chip top-left, so the two never collide) holding an 8px near-white ▶ SVG; its `title` lists the count + clip names ("13 animations: Attack_Headbutt, …" — innermost title beats the tile's label tooltip). Data source is the existing manifest field (populated at import since Phase 10.7, seeded across the library by the Phase 51 backfill), so no new plumbing; caveat unchanged from the field itself: pre-clip-discovery imports have `animations: undefined` (= unknown, no badge) until re-imported. Complements the `#animated` tag filter — the badge answers per-tile at a glance, the tag answers "show me only these". Verified in-browser: 22 badges across the library, animals/character badged, furniture clean, tooltip lists real clip names.
- v4.52.3 — **Asset panel contrast pass + bigger/thicker icons** (user: "could it be a little bigger, a little thicker? The other buttons are hard to see too... small grey font on the grey background. needs to be more white. think accessibility contrast."). The no-grey-on-grey rule applied to the whole AssetBrowser: category pills / `More ▾` / popout items / Manage / Done / tile labels go `#808080`→`#c2cadb` (tile labels also 8→9px), empty-state and "No tags yet" hints `#505050`/`#7a7a7a`→`#9aa3b5`, and — the worst offenders — the disabled Edit/📷/⌖/Delete states go near-invisible `#555`→visible-muted `#8b93a5` (still clearly disabled next to the brightened enabled colors `#80aaff`→`#9dbdff`, Delete `#cc6666`→`#e08585`). LeftPanel's panel title (`ASSETS`) brightens `#808080`→`#dde3f0`. `IconReorigin` grows 14→18px with strokeWidth 2.4→3.4, bigger arrowheads, brighter axis colors (`#7ce884`/`#ff7b6f`/`#77aaff`), white hub, and disabled opacity 0.3→0.45 so the dimmed state stays readable. The 📷 emoji — fixed-color, muddy at small sizes on the dark background — is replaced by new `IconCamera` SVG (body/prism/lens strokes) that takes the standard color prop, so it brightens and dims exactly like its Edit/Delete siblings. Verified in-browser, zoomed, in both checked and unchecked states.
- v4.52.2 — **Re-origin button gets a proper RGB gizmo icon** (user: "a tiny 3-axis 3-color translate icon"). The ⌖ text glyph is replaced by new `IconReorigin` in `src/ui/icons.tsx` — an isometric 3-axis translate gizmo (one up-arrow rotated 0/120/−120° around the hub: Y green `#63c96a`, X red `#e25b5b`, Z blue `#5b8fe2`, neutral center dot), matching the viewport transform gizmo's color language. Unlike the other icons it takes no `color` prop (the axis colors ARE the icon) — disabled state dims via `opacity 0.3` through a `dim` prop, consistent with the greyed Edit/Delete text beside it. Verified in-browser at both states.
- v4.52.1 — **Re-origin modal gains a marker preview** (user: "what if it showed you like a preview of where it currently is on the model, and where the 2 options will move it to?"). The modal now opens with a 256² orbitable render of the model (same `ThumbnailStage` + drag-to-orbit/scroll-to-zoom interaction as the thumbnail stager) overlaid with three depth-test-off markers: an **amber sphere + axis cross at the current origin**, a **green sphere at the Base target** (bbox bottom-center) and a **blue sphere at the Center target** (bbox center), plus a line from the current origin to the selected target showing the move. Picking a radio swaps the emphasis (selected marker 1.3×/full-bright, the other dimmed to 0.35) and recolors the line; the radio labels and the "origin now" legend carry matching color dots so the mapping needs no reading. Implementation piggybacks on `ThumbnailStage` taking any `Object3D`: the markers are grouped WITH the model before staging, so the framing bbox automatically includes the current origin even when it sits far outside the geometry (the buried-gizmo case that motivated the feature — a grass-like model's origin ~1 below the mesh stays in frame). Marker radius scales with the model (2.2% of the bbox diagonal), `renderOrder` 998–999 keeps them visible through geometry. Verified in-browser on Tree: Base view shows the green marker sitting on the amber origin cross at the trunk base with Center dimmed mid-canopy; switching to Center brightens the blue sphere with the origin→center line up the trunk; orbit re-renders correctly.
- v4.52.0 — **Phase 52 — Re-origin models from the asset browser (move a model's pivot).** (user: "I might be interested in being able to re-origin models myself one day" — prompted by the grass_1/2/3 fix, commit 66059a2, where the Platformer Kit grass shipped with geometry baked ~0.95 above its origin, so ground-snap placement buried the entity origin and gizmo below the terrain.) Manage mode gains a **⌖** button beside 📷 (enabled at exactly one checked asset) opening **`src/ui/ReoriginModal.tsx`**: loads the model, computes its Box3, states in plain language where the base sits relative to the origin ("base is 0.33 below the origin, off-center sideways by X 0.11") and offers **Base** (bottom-center — props, plants, characters) or **Center** (floating/spinning things), with the exact geometry shift previewed and a warning that the file rewrite isn't undoable. Apply (`App.handleApplyReorigin`) reads the model file through the `ensureDir(modelsDir)` grant and rewrites it via new pure module **`src/core/gltfReorigin.ts`** — `applyGltfReorigin(bytes, fileName, delta)` handles `.gltf` (JSON) and `.glb` (chunk-preserving container rewrite, JSON chunk re-padded with spaces, BIN chunks copied verbatim); OBJ is declined in the modal. **The shift is one `__reorigin` wrapper node** parenting the scene's roots, NOT per-root translation edits: animation tracks target the original roots, so a translation written onto an animated root would be stomped the moment a clip plays — and a second re-origin accumulates into the existing wrapper instead of nesting. **Placed copies:** a default-on checkbox moves every placed copy in the loaded world to compensate — `instanceWorldShift` computes R·(S·delta) per instance (XYZ euler degrees, per-axis scale) and a journaled `transaction` subtracts it from each position, so copies stay visually where they were (undoable; the file isn't — and copies in other scenes/projects shift on next load, which the modal says outright). After the write: **`assetManager.evictModel(id)`** (drops the gltf+obj cache entries and bumps a per-model version appended as `?v=n` to the next `loadGLTF` fetch, defeating the browser HTTP cache on the rewritten file), selection is dropped if the selected object uses the asset (the gizmo would be left on a disposed mesh), then new bus event **`asset:model-updated { assetId }`** — `ZoneManager` rebuilds every placed copy by remove+re-add (same path as the prefab assetId-swap rebuild) so meshes, `localAABB`, and auto-fit colliders derive from the fresh geometry. Verified in-browser through real clicks (§9 OPFS picker stub; real `fruit.gltf` verified untouched after): the modal reported fruit's base 0.33 below / off-center X 0.11; Apply wrote wrapper translation `[-0.10733, 0.33171, 0.01304]` (= −center.x, −min.y, −center.z exactly) with `scenes[0].nodes` re-rooted onto it; the placed test copy (yaw 90°, scale 2) moved to precisely `old − R·S·delta` = (4.97392, −0.66342, −5.21466); a tagged mesh was verified replaced on the rebuild event. Acceptance: `test-plans/phase-52-model-reorigin.md`.
- v4.51.4 — **"Uniform scale" checkbox moved beside the Scale label; panel labels brightened** (user: "closer to the scale labels. All the labels should be a little bigger and less gray"). The Scale heading row drops `space-between` for an inline `gap: 12` so the checkbox reads as part of the heading, and the checkbox drops its dimmer `#9090a0` to inherit the row color. The shared `LABEL` style — used panel-wide (Position/Rotation/Scale, FLOOR LEVEL, INTERACTABLE, every section heading built on it) — goes `#646464`/10px → `#c2cadb`/11px, the established readable-secondary tone (no-grey-on-grey rule). Verified with a zoomed panel screenshot on a real selected object.
- v4.51.3 — **Uniform checkbox relabeled "Uniform scale" + breathing room above the Scale row** (user). The checkbox label says what it makes uniform, and the Scale group gets `marginTop: 8` on top of the column's 10px gap so the checkbox row doesn't crowd Rotation.
- v4.51.2 — **Transform fields become real number inputs; uniform scale field sized to one axis field** (user: the uniform input "is super wide", and the fields "have no arrows and accept other letters. probably to prevent the decimal issue? maybe there's another way?"). The `type="text" inputMode="decimal"` markup dated to Phase 2 day one, not the v4.51.0 fix — but the question was right that `type="number"` is the better way: the browser rejects letters natively and gives spinner arrows (↑/↓ step by the group's step: position 0.5, rotation 15°, scale 0.1). The v4.51.0 incomplete-token gate composes cleanly with it — while only `.` is typed, a number input reports `value=""`, the gate skips the write, and React leaves the DOM alone (draft `""` == reported `""`), so the keystroke survives; the rest of the panel's numeric fields were already `type="number"`, so this is also a consistency fix. The uniform field's wrapper gets `width: calc((100% - 8px)/3)` — exactly one axis-field width instead of the full row. Verified in-browser with REAL keystrokes (triple-click select, `.` → 600ms pause → `5`): field showed 0.5, scale {0.5,0.5,0.5}; typing `abc` changed nothing; ArrowUp stepped 0.5→0.6 on all axes; zoomed screenshot confirmed the narrow field + visible spinners; cleanup + autosave snapshot-restored clean, `public/games/**` verified untouched.
- v4.51.1 — **Uniform scale field: `XYZ` prefix removed** (user: it's "shoved in the input and looks like part of the text"). The per-axis fields' colored single-letter prefixes read as labels, but a 3-letter monochrome one at the same 9px reads as typed content. The field is now unlabeled — the row's `Scale` heading + `Uniform` checkbox already say what it is.
- v4.51.0 — **Object transform panel: typing `.5` no longer becomes `5`, + a Uniform scale checkbox.** (1) User: "if I type `.5` instead of `0.5`, it changes it to `5`". Cause in `ObjectGeoView.commit`: every keystroke schedules the debounced write with `toNum` (NaN → 0), so a lone `.` committed **0**; the v4.42.5 value-resync effect then saw the data change and clobbered the in-progress draft to `"0"`, and the next keystroke `5` produced `05` = 5. Fix: incomplete tokens (`.`, `-`, `""`) now update the draft but **skip scheduling the write** (`Number.isFinite(parseFloat(raw))` gate) — nothing commits, nothing resyncs, and `.5` parses as 0.5 on the next keystroke (`parseFloat` always handled bare-dot decimals; the clobber was the bug). Applies to all three groups (position/rotation/scale). (2) **Uniform checkbox** on the Scale row (user ask, checked by default): when on, one `XYZ` field replaces the three axis inputs and writes x=y=z in a single debounced commit (`commitUniformScale`); it initializes per selection — on when the object's scale is already uniform, off when the axes differ (a single field would misrepresent them) — and unchecking reveals the usual per-axis fields. Other panels' drafts (colliders, floors, stairs) share the `toNum` idiom but resync only by id, not value, so they don't exhibit the clobber; left untouched. Verified in-browser through the real DOM input path (spawned object, real click-select, Geometry screen): slow-typed `.` held in the field with data unchanged, then `.5` → scale {0.5,0.5,0.5} on data AND mesh via the one uniform field; unchecked → per-axis Y=2 → {0.5,2,0.5}; slow-typed `.75` in axis Y → 0.75 (the exact reported repro); reselecting the non-uniform object defaults the checkbox off; cleanup + autosave snapshot-restored clean.
- v4.50.0 — **"Save as default for this model" — a placed object's collider set becomes the asset's default** (user: an editor for a default collider on an already-imported model, "if I do custom colliders I could save it to the object to persist too? or is that just a prefab?" — not a prefab: it's library metadata). The engine half already existed since Phase 26: `AssetDef.colliders` + the placement preference `obj.colliders ?? def.colliders ?? auto box`; what was missing was authoring. The Colliders screen's explicit-list view (objects only) gains a green **Save as default for this model** button under `+ Add collider`: edit any placed instance's colliders with the existing tools (shape switch, Move/Rotate/Resize gizmos, Refit, sensors — all saved as-is), then one click runs `App.handleSaveCollidersToAsset` — `ensureDir` models-folder grant, manifest read-modify-write splicing `colliders: structuredClone(set)` into the entry, `assetManager.updateAsset` + `setAssets` + `assets:loaded`, then **clears the source object's override** (`updateObject(colliders: undefined)` in a journaled transaction — the object goes back to tracking the model default; **undo restores the override, not the manifest**, matching every other import-side op) and **rebuilds sibling placements** that never customized (bus-only `object:updated` emits — their data is already `undefined`, they just need `_buildObjectColliders` re-run against the new def). Enabler in `ZoneManager`: the `object:updated` rebuild gate now tests **`"colliders" in changes`** instead of truthiness, since clearing an override back to `undefined` must also rebuild. Copy fixes riding along: the screen summary `auto (N boxes)` → `auto (N presets)` and the implicit-state text no longer claims presets are only "baked from source shapes" (they can now be a saved custom set). Not included by design: a standalone collider editor in the asset browser (a placed instance is the editing surface — it already has every gizmo) and saving from shapes/platforms (asset-backed objects only). Verified in-browser through real clicks (§9 OPFS picker stub; real manifest verified untouched): two spawned closets, A customized box→Sphere (Rapier: 1 ball/135 cuboids), Save → OPFS manifest entry `[{shape:"sphere", r:1.489}]`, A's override cleared (panel flips to the implicit view, chip `auto (1 preset)` on B), **both** closets' physics rebuilt to balls (2 balls/134 cuboids — sibling rebuild proven), sphere wireframe visible on the never-touched sibling, undo restored A's explicit sphere and redo re-cleared it, cleanup + autosave snapshot-restored clean.
- v4.49.0 — **Attribution autofill from existing packs/authors in the model importer** (user: "when importing new models from an existing pack and or author, it might be nice to autofill some fields"). The library already proves the need: 96 of 111 assets carry one of just 3 distinct attributions (all Quaternius packs), each retyped at import time. `AttributionFields` gains an optional **`autofillFrom?: Attribution[]`** prop; when non-empty it renders an **AUTOFILL FROM LIBRARY** select above the AUTHOR field with two optgroups — **Packs** (distinct `sourceName`, first occurrence wins; picking one fills the *complete* attribution: author, pack name/URL, patreon, license) and **Authors** (distinct `author`; picking one fills only the author-level fields — author, `patreonUrl`, `license`/`licenseOther` — deliberately leaving `sourceName`/`sourceUrl` alone, so importing a *new* pack from a known author is one pick + two typed fields). Selection merges over the current value (`{ ...value, ...preset }` — typed fields the preset doesn't carry survive) and the select snaps back to its placeholder (it's pinned `value=""`). `ModelImporterModal` threads a new required `existingAttributions: Attribution[]` prop into the field block, and App passes `assets.flatMap(a => a.attribution ? [a.attribution] : [])` at the render site — same pattern as the Phase 51 `existingTags` prop (the manifest isn't read until the import step, so suggestions must come from the loaded registry). The other `AttributionFields` call sites (audio/skybox/material/graphics importers, `EditMetadataDialog`) omit the optional prop and are pixel-unchanged — extending them is a free follow-up (each would pass its own manifest's attributions). Verified in-browser via the §9 picker stub: the select lists `Ultimate Furniture Pack — Quaternius` / `Ultimate Animated Animal Pack — Quaternius` / `Platformer Game Kit — Quaternius` + author `Quaternius`; picking the pack filled all 5 fields (license → CC0); then with a typed `My New Pack`, picking the author filled author/patreon/license and left the typed pack name and blank URL untouched.
- v4.48.2 — **←/→ editor-camera yaw direction flipped** (user: arrow rotation "seems inverted — right moves left"). One-line sign swap in `EditorCamera.update()`: `ArrowLeft` now does `targetSpherical.theta -= rot` and `ArrowRight` `+= rot`. v4.47.0 picked the signs on the "← turns the view left" reading, but because `theta` is the *camera's* azimuth around the focus point, decreasing it swings the camera left and the scene right — so ← orbited the camera the way a right-drag would, which reads backwards next to RMB-orbit and next to Q/E. The new signs match what the user expects from the key: press → and the scene slides left, i.e. the camera walks right around the subject. Nothing else touched — speed, smoothing lerp, ↑/↓ elevation, and the preview/game arrow bindings in `bindings.ts` are unchanged.
- v4.48.1 — **Asset tag mode made legible (`Categories | Tags` switcher).** User: clicking the `#` "just switches between categories or tags, without really ever showing a tags interface." Correct — v4.48.0 shipped the *mechanism* with no *signal*: tag chips reused `CAT_BTN` verbatim, so switching facets only changed the words in identically-styled pills, and the sole cue was a small `#` button tinting blue. Worse, `TAG_STRIP_COUNT = 4` meant 4 of the library's 15 tags showed and the other 11 hid behind `More ▾`, and counts existed only inside that popout — so the tag vocabulary was never visible. Fixed by naming the facet instead of implying it: the bare `#` button is replaced by a **`Categories | Tags` segmented control** on its own row above the strip (new `SEG_BTN` style, inactive label `#c2cadb` — legible, not grey-on-grey), the `Tags` segment carries the active-tag count, and a right-aligned **`clear`** (shown only when a category or tag filter is live) resets both. Tag chips now render **`#tag count`** (`COUNT_SPAN`, opacity 0.55) in both the strip and the `More ▾` popout, so a tag can't be mistaken for a category pill — categories stay bare, since they are exclusive and have no count. Chips wrap to a second row rather than being truncated. Behaviour is unchanged: still multi-select AND, frequency-ordered, active ones pinned, filters still compose with the category. Verified in-browser: `#prop 44` filters to exactly 44 tiles, `Tags 1` + `clear` appear, `clear` restores 111. Costs one ~22px row in both modes — the v4.48.0 design traded that away for grid height and bought invisibility with it.
- v4.48.1 — **Refit button for primitive colliders** (user: "a kind of refit button like the hull collider has? for sphere/capsule/box"). The collider card's SIZE/RADIUS row gains a **Refit** button (box/sphere/capsule; shown when `defaultColliderFor` yields the model's local AABB — same source as the auto box): box takes the bounds verbatim, sphere r = max dimension / 2, capsule r = max(w,d)/2 + h = bounds height (the `reshapeCollider` derivations), offset = bounds center, rotation reset (axis-aligned fit). One undoable write like any panel edit. Verified in-browser on a real kit tile: a deliberately wrong sphere (offset 5/9/2, r 7) snapped to center ≈ origin, r 1.0 (half the 2m tile) via a real click.
- v4.48.0 — **Phase 51 — Asset tags (models).** Finishes a feature that was half-built from the start: **`AssetDef.tags: string[]` has existed since the original manifest** and `AssetBrowser` already matched search text against it — what was missing was authoring (the importer hardcoded `tags: []`), editing (no field in the metadata dialog) and filtering (search-only). **No type change, no migration** — every manifest entry already had the key; tags are library metadata and are never referenced by placed entities, so zero runtime impact. **Why both axes:** `category` is a single exclusive shelf that was silently doing three jobs at once — *which pack it came from* (`Platform: Objects` 44, `Pieces` 19), *what the thing is* (`Furniture` 20, `Nature` 13, `Animals` 12) and *how it was made* (`Baked` 2). Answering one lost the others: `Door` sits under `Platform: Objects` while `Door1/2/3` sit under `Furniture` purely because they shipped in different packs, and `Rock 2` is filed `Nature` though its pack is `Ultimate Furniture Pack`. Category stays the one shelf; tags carry the rest. **New `src/ui/TagInput.tsx`** — chip editor (Enter or `,` commits, `✕` per chip, Backspace on empty pops the last, `<datalist>` suggestions, `disabled` for bulk gating) + exported `normalizeTag` (trim → lowercase → spaces to `-`), shared by the dialog and the importer. **`EditMetadataDialog`** gains a TAGS block between CATEGORY and ATTRIBUTION, and `EditPatch` gains **two** fields because arrays need explicit merge semantics: `tags` (single edit — full replacement) and `tagsAdd` (bulk — unioned into each item, since a "replace" across a multi-select would silently destroy tags the dialog never showed). The `tagSuggestions` prop is **optional and only passed from the asset call site**, so the sound/skybox/material call sites are untouched and simply don't render the field. **`App.handleConfirmAssetEdit`** resolves tags per-id (`resolveTags`) for both the manifest write and the registry update; the generic `patchEntry` is deliberately **not** widened (it only picks label/category/attribution, so `tagsAdd` can't leak through), and `tagsAdd` is destructured away before `updateAsset` since it isn't an `AssetDef` field. **`ModelImporterModal`** gains one shared batch tags field under "Set all to", mirroring how `AttributionFields` is already batch-shared; suggestions arrive via a new `existingTags` prop because the manifest isn't read until the import step. **`AssetBrowser`** gains a `#` toggle that swaps the pill strip between category mode and tag mode: tags are **multi-select and ANDed**, ordered by frequency (recency doesn't transfer — you accumulate tags), active ones pinned to the strip front, overflow reusing the existing `More ▾` popout (which stays open across clicks in tag mode and shows per-tag counts). Both filters stay live across the toggle — the count rides on the button (`#2`) and, in tag mode, an active category surfaces as a clearable `Nature ✕` pill so a forgotten category can't masquerade as "no matches"; empty results offer **Clear filters**. **Backfill:** one-off `scripts/seed-asset-tags.mjs` unioned derived tags into all 111 existing assets (109 changed) from data already in the manifest — category → `platformer`/`prop`/`tile`/`furniture`/`interior`/`nature`/`outdoor`/`animal`/`creature`/`baked`, plus `animated` from `animations`, and license/pack slugs from `attribution`. Existing tags are never replaced (the 19 tiles keep `platformer`+`tile`) and re-running is a no-op; committed separately from the code so a bad derivation is one revert away. Deliberately **not** included: sounds/skyboxes (they carry the same `tags` field already — a free follow-up), materials (`MaterialDef` has no `tags`), per-entry importer tags, and any tag rename/merge UI. Verified in-browser end-to-end against the user's own project via real clicks, incl. `Nature`+`cc0` → exactly `Rock 2` (1 of 13 Nature, 1 of 96 cc0 — proving the two filters compose) and a 3-item bulk edit where each kept its own distinct tags; the FSA writes ran through the §9 OPFS picker stub and `public/assets/models/manifest.json` was verified byte-unchanged after. Plan: `plans/phase-51-asset-tags.md`; acceptance: `test-plans/phase-51-asset-tags.md`.
- v4.48.0 — **Sphere + capsule colliders get Resize drag-handles** (user on obj_2e766921: no drag gizmo for the sphere collider, and a tall box switched to sphere becomes a huge sphere "with no way to shrink it down again" — the RADIUS panel field existed but was easy to miss). The v4.4.0 "sphere/capsule are wireframe-only (panel edits)" limitation is closed: the panel's **Resize** toggle now appears on every primitive shape (hull/trimesh stay excluded — their shape lives in points). `ColliderEditor._onDragMove` grew per-shape semantics sharing the same axis-constrained plane projection: **sphere** — any of the 6 handles drags the radius with the CENTER pinned (center-symmetric shapes would just wander under face-pinning); **capsule** — ±X/±Z handles drag the radius (center pinned), ±Y handles drag the full height with the opposite CAP END pinned (box idiom; pinned point = center − axis·(cylHalf + r), matching Rapier's capsule halfExtents convention); **box** unchanged. Scale components mirror `colliderWorldTransform` per shape (sphere r ÷ max scale axis, capsule r ÷ max(|sx|,|sz|), height ÷ |sy|). `_positionAll` places capsule ±Y handles at the true cap end (cyl half + radius). Confirmed during testing: **Move always worked on spheres** — the missing piece was Resize (and Rotate is correctly absent: spheres are symmetric). box→sphere conversion still seeds radius = max dimension / 2 (unchanged — now trivially adjustable). Verified in-browser through real clicks/drags: sphere card shows Move+Resize, capsule Move+Rotate+Resize; a 2.5-radius sphere dragged down to 1.0 by its +X handle (0.25 snap, center fixed); capsule +Y drag grew height 2→3.25 with the bottom cap pinned exactly (offset +0.625 = delta/2); cleanup + autosave snapshot-restored byte-identical.
- v4.47.1 — **Arrow-key camera speeds toned down** (user: "arrows is kind of fast too"). ↑/↓ elevation now half the WASD pan speed (`s * 0.5`); ←/→ yaw 0.025→0.015·dt·60 (~1.5→~0.9 rad/s). WASD itself untouched (and was untouched in v4.47.0 — its radius-scaled speed just *feels* faster when zoomed out).
- v4.47.1 — **Sensor checkbox copy pass** (user: a solid collider offering on_player_enter reads as contradictory — you can't enter a solid). Label now leads with what checking does: "Sensor — doesn't block; the player walks through it, and entering / leaving fires on_player_enter / on_player_exit scripts" — making explicit that the checkbox CONVERTS solid→passable rather than adding events to a solid.
- v4.47.0 — **Arrow keys split from WASD in the editor camera** (user ask: arrows duplicated WASD exactly). `EditorCamera.update()`: WASD keeps the XZ focus pan; **↑/↓ now move the focus point up/down the world Y axis** (same radius-scaled speed `s` as the pan, unclamped) and **←/→ rotate the orbit yaw** (`targetSpherical.theta ± 0.025·dt·60` ≈ 1.5 rad/s — ← turns the view left, matching FPS convention; smoothing comes free from the existing 0.12 lerp). No other code touched — preview/game arrows live in `bindings.ts` (`move` + `menuNav`) and are unaffected; the modifier-key guard (v4.2.3) and `_isTypingTarget` gate already cover the new paths since key capture is unchanged.
- v4.47.0 — **Collider face handles become an explicit "Resize" gizmo mode — one collider gizmo on screen at a time** (user: the object's move gizmo and the collider resize cubes both show by default; the panel's Move/Rotate toggle "should also be where I change to the resize-gizmo-cubes… to more overtly only conditionally show the various gizmos"). `collider:move` mode union += `"resize"`; `ColliderEditor._sync` builds the 6 push/pull face handles ONLY for the collider whose panel Resize toggle is active (previously: every visible box collider on selection — 12 cubes stacked on the object gizmo for a 2-collider object); in resize mode the proxy TransformControls detaches (the handles ARE the gizmo), and `_setMove` guards `setMode` to translate/rotate. Panel: collider cards gain a **Resize** button (box only) beside Move/Rotate, same toggle group + object-gizmo suspend. Behavior notes: default selection now shows wireframes only; the implicit auto-box needs Customize before its handles are reachable (materialize-on-drag still works from resize mode); hidden-eye and shape-switch guards unchanged. Verified in-browser through real clicks/drags: selection = 2 wireframes/0 cubes, Resize → exactly 6 handles on that collider, +Y drag resized H 1→2.25 (0.25 snap, opposite face pinned), Move switch cleared handles and attached the translate gizmo, cleanup + autosave snapshot-restored byte-identical.
- v4.46.3 — **Collider rotate-gizmo snap 2.5° → 1°** (user: "smaller"). `ROT_SNAP_DEG` + tooltip. Alt = free unchanged.
- v4.46.2 — **Collider rotate-gizmo snap 5° → 2.5°** (user follow-up to v4.46.1: rotate increments "a little smaller" too). `ColliderEditor`'s `ROT_SNAP_DEG` + Rotate button tooltip. Alt = free unchanged.
- v4.46.1 — **Collider face-handle drag-snap 0.5m → 0.25m** (user: "the increments should be a little smaller"). `ColliderEditor`'s `GRID` const (feeds `snap()` for the push/pull face handles). Alt = free unchanged; rotate snap stays 5° (v4.45.1).
- v4.46.0 — **Phase 50 — Runtime-only InstancedMesh pooling for placed objects (draw-call collapse).** Kit-built levels multiply draw calls linearly (obby level: 126 tiles × ~3 meshes × 2 render passes = 784 calls for 14k tris; baselines are 14–32). New **`src/world/InstancedObjectPool.ts`**, constructed ONLY by `RuntimeApp` and injected into `ObjectPlacer` via a new optional ctor opts (`{ instancing? }`, type-only import — editor bundle verified pool-free) — **ZoneManager/ScriptEngine/App.tsx/types.ts untouched**, editor behavior byte-identical. Eligible objects skip the per-object GLTF clone: `build()` registers a lightweight proxy `Object3D` (`_applyTransform` userData + per-asset `localAABB` + `_instanced` flag) so colliders (`getLocalAABB`), `_removeObject`, `AudioSystem._findEntityMesh` and the interact scan keep working, while the pool renders one `InstancedMesh` per `(assetId, submesh)` sharing the CACHED gltf geometry/materials (`rel = rootInv × child.matrixWorld` strips the gltf root TRS to match build()'s root overwrite; instance matrix = obj TRS × rel). **Eligibility is a static exclusion** (no per-instance mutation machinery): skinned/animated/transparent assets, `.obj`, `autoPlayAnimation`/`material`/`mover`/`sound`/`interactable` objects, and any object or group targeted by a mutating script action — found by a **container-agnostic deep JSON walk** of the level data (covers ScriptDefs at all four homes, `DialogueOption.actions`, `UiMenuOption.actions`, and future containers); over-exclusion just falls back to the clone path, and DEV warns if a pooled object ever receives a runtime mutation. **Lifecycle:** `tryAdd` accumulates during `loadZone`'s sequential builds; finalization happens in the pool's `zone:loaded` handler (exact-count meshes, `computeBoundingSphere`, cast/receive shadow, frozen static-shadow lights get `needsUpdate`); teardown rides `ObjectPlacer.remove` → `release` — pool meshes live in a **scene-root group, NOT the zone group**, because `unloadZone`'s dispose traverse would dispose the shared `_gltfCache` geometry (the trap); `_releaseZone` calls `InstancedMesh.dispose()` only (instanceMatrix buffers). Verified on the committed obby level in the runtime shell: **784 → 68 draw calls (~11×)**, tris unchanged, 35 pools / 130-of-130 objects pooled, colliders identical (135) with the avatar grounded on pooled tiles, shadows include pooled geometry, `router.go` reload ×2 leak-free (colliders 135/135/135, geometries 48/48/48, identical re-render), all 7 eligibility branches probed live, editor smoke clean (0 pools, 130 clone roots). **Known pre-existing** (surfaced, not fixed): `info.memory.textures` grows ~+11 per scene reload — orphaned terrain/material GL textures on the transition path (kit tiles are untextured; the pool allocates none). Editor viewport still draws N copies by design — the discussed follow-up for editor draw calls is prefab consolidation (user: tiles in prefab platforms are never individually edited). Plan: `plans/phase-50-runtime-instancing.md`; acceptance: `test-plans/phase-50-runtime-instancing.md`.
- v4.45.1 — **Collider rotate snap 15° → 5°** (user: "smaller rotation increments please"). New `ROT_SNAP_DEG = 5` const in `ColliderEditor` feeds both `setRotationSnap` sites (init + Alt-keyup restore); Rotate button tooltip updated. Alt = free unchanged.
- v4.45.0 — **Attached colliders rotate on all three axes (gizmo + panel).** User ask: colliders on objects should be rotatable in addition to movable. **Data:** `AttachedCollider.rotation?: Vec3` (deg, local euler XYZ; box/capsule — sphere is symmetric, hull/trimesh orientation lives in `points`); legacy `rotationY` stays as a READ fallback (`rotation` absent → yaw-only), so existing scenes and Phase-26 baked asset presets load unchanged — every writer (panel fields, rotate gizmo) emits `rotation` and clears `rotationY`. **Shared math:** new `colliderLocalQuat(c)` in `attachedColliderMath.ts` (rotation-else-rotationY-else-identity) composed into `colliderWorldTransform` (`objQuat × localQuat`), so Rapier registration, wireframes, face handles and both gizmos all inherit full rotation from the one function; the non-uniform-scale approximation note now covers any collider rotation, not just yaw. `ColliderEditor._onDragMove`'s opposite-face-pinning offset shift generalizes from `applyAxisAngle(Y, rotationY)` to `applyQuaternion(colliderLocalQuat)`. **Gizmo:** `collider:move` payload gains `mode?: "translate"|"rotate"`; the per-collider proxy TC switches modes (`setMode`), rotates in **local space** (`setSpace("local")` — rings align to the collider's own axes; translate keeps world arrows), proxy now wears the collider world quat when idle, and rotate drags snap **15°** (`setRotationSnap`, Alt = free, matching the face-handle Alt idiom); commit converts proxy world quat → object-local euler via `objQuat⁻¹ × proxyQuat` inside the same transaction shape ("rotate collider"). **Panel:** collider cards gain a **Rotate** button beside Move (box/capsule only) and the box-only ROTATION (Y°) field becomes a ROTATION (°) X/Y/Z row for box AND capsule (capsule pitch/roll are now meaningful — tilted capsules); drafts read through the legacy fallback; the card's ✕/hide paths pass the active mode so toggling off works from either mode. Verified in-browser on the user's obby project (fresh tab, snapshot-restored): Rapier collider quats exact to 5dp for both euler XYZ(20,30,0) = (0.16773,0.25489,0.04494,0.95125) and legacy `rotationY:30`; a typed panel X° edit migrated `rotationY:30` → `rotation:{20,30,0}` (rotationY dropped); a real rotate-ring drag stepped Z 45→0 in snapped 15° increments and ONE Cmd+Z restored 45; a face-handle drag on the 45°-rotated box grew H 0.4→1.5 with the offset shifting exactly (−0.389,+0.389) along the rotated axis (opposite face pinned); Move gizmo regression clean (offset.y 1.389→2.497, rotation untouched). Known pre-existing (NOT this change, noted for later): spawning an object and writing `colliders` in the same tick double-registers that object's Rapier colliders — the async `_addObject` build and the `object:updated` rebuild each add a set and the lifecycle map only tracks the last (console-harness pattern; a later rebuild leaves the first set stale until reload).
- v4.44.4 — **Tiled Platform gains a Height (layers) variable** (user: "how to make the platform taller? like extra repeating middle pieces"). The Phase-43 kit's vertical set was already imported but unused; the generator now stacks it: height grows DOWNWARD from the walk surface (top stays at origin.y+1) in 2m bands — top band = `*_tall` edges + the flat `center_tall` grass sheet, middle bands = repeating `*_center_tall` edge pieces with a fully hollow interior (no member emitted), bottom band = `*_bottom_tall` edges + the flat `bottom_tall` underside sheet. Dirt only ships full-height blocks, so its lower bands repeat `dirt_*_tall` (no underside — kit limitation). **[Superseded v4.56.3 — this was wrong. The `grass_*_center_tall`/`*_bottom_tall` pieces are pure `Dirt_1/2/3` with no top face; dirt reuses them for its lower bands. Repeating `dirt_*_tall` stacked a beveled lip at every layer.]** **Height 1 emits the exact legacy output, memberKeys included**, and the top band keeps the legacy `tile_i_j` keys at any height so height edits diff-update tiles in place (assetId swaps ride the v4.44.3 rebuild path). Piece bounds measured from the glTF accessors, not the names (`center_tall` and `bottom_tall` are flat sheets at y=±1; everything else spans y −1..+1). **Schema-staleness fixes that made it reachable:** library defs snapshot `variables` at creation, so pre-height defs would never show the field — `PrefabSection` and `defaultVars` now read the REGISTRY's variable schema for generator prefabs (stored copy is fallback only), and `syncPrefabInstances`' orphan-heal generator match went from exact variable-key-set to **subset** match (old records lack keys generators gain later). Verified in-browser on the user's own 5×3 instance: Height field appeared on the pre-height def, 3 layers expanded 15/12/15 members with the right pieces per band, and undo back to height-1 left zero data-vs-mesh model mismatches with the selection intact.
- v4.44.3 — **Undo/redo of a prefab re-expansion no longer borks the tiles** (user: grew a Tiled Platform dimension, Cmd+Z → "borked it" — melted look + a floating fragment; screenshot matched the v4.42.7 melt). Diagnosis in a fresh tab against the user's own scene state: their DATA was never corrupted (9 flush tiles, clean scales — reload cleans the view). Two causes, two fixes. (1) **ROOT CAUSE — model swap ignored on replay:** the journal collapses a re-expansion's remove+re-add of the same id into ONE `object:updated` (the v4.42.0 selection-keeping collapse) whose payload can change `assetId` (edge tiles flip corner↔side as the grid grows/shrinks) — and neither ObjectPlacer (transform/material patch) nor ZoneManager (collider rebuild) ever swapped the MODEL, so undone edge tiles kept rendering their interior variants while colliders tracked the correct data. `ObjectPlacer.build` now stamps `userData.assetId` on every root (incl. fallback boxes); ZoneManager's `object:updated` does a full `_removeObject`+`_addObject` when the payload's assetId differs from the built mesh. Live edits never change assetId, so nothing else pays the rebuild. (2) **Defense — replay churn with the gizmo attached:** Cmd+Z/redo replays the same member remove/re-add the panel paths guard with `withInstanceReselect` (v4.42.7); `handleUndo`/`handleRedo` now wrap the replay in it whenever a prefab instance is selected (selection context mirrored into `undoInstanceCtxRef` for the memoized handlers) — 3D systems detach first, the member set re-selects ~150ms after the rebuild. Also learned during diagnosis: the multi-select tint on the hollow kit tiles legitimately reads as translucent/striped while selected — cosmetic, not damage. Verified in-browser on the user's scene state: width 3→4 → undo → redo → undo, per-tile data-vs-mesh model comparison clean at every step, selection re-established, final render identical to pre-edit.
- v4.44.1 — **`GUI_GUIDE.md` added (with screenshots) + starter graphics + checker-layer fix.** Human-facing authoring guide in the PREFABS_GUIDE house style covering both new systems: the graphics library / pack import / icon-from-3D flows, item icon picking, every UI element kind with its fields, menu options (conditions/actions/close), show_ui/hide_ui recipes, the visibility-is-game-state persistence model, project-vs-scene scope, a JSON reference, and v1 limits. Five real screenshots in `docs/images/` (graphics-panel, item-icon-pick, icon-stager, ui-tab, gui-ingame). The graphics library now ships a committed **starter set** (`heart`/`coin`/`key`, 64² transparent PNGs generated procedurally — dependency-free Node PNG encoder) so pickers/GUI work out of the box. Fix surfaced by the panel screenshot: `GraphicsBrowser`'s checkerboard tile listed the gradients before the `url()` — CSS paints the FIRST background layer on top, so the checker drew OVER the artwork; image layer moved first (DecalBrowser has the same latent ordering, untouched). Session note: an editor-tab reload mid-session destroyed the in-page `worldeditor_gamesave` snapshot after preview exit had overwritten the key — the save was scrubbed back to a fresh-New-Game shape (`{health:100}`), original values unrecoverable; TESTING.md's snapshot protocol already covers this for future sessions (dump snapshots outside the page).
- v4.44.0 — **Phase 49 — Custom game GUI: script-driven HUD widgets + simple menus.** Activates the long-dead `show_ui` stub (`ui:show` had an emit and zero listeners) into an author-defined UI system. **Types:** `UiElementDef` union — `bar` (stateKey + max/width/height/color/graphicId), `counter` (stateKey incl. `inv.<itemId>`, graphicId/prefix/size), `label`, `image` (graphicId), `menu` (title + `UiMenuOption[] {text, conditions?, actions?, closeOnPick?}`) — each with `id (ui_<uuid8>)`, `label`, `anchor` (6 corners/centers), `offsetX/Y`, `startVisible?`. Registry like items: `WorldConfig.uiElements` (scene, serializes free) + `GameConfig.uiElements` (game.json), merged by `src/scripting/uiElements.ts` `mergeUiElementDefs`/`uiRegistry` (scene wins by id); `WorldState.gameUiElements` session field (gameItems contract), set by App project sites + SceneRouter. **The core decision — visibility lives in gameState, not component state:** `show_ui`/`hide_ui` set reserved key **`__ui.<elementId>`** (ScriptEngine dispatch), the overlay derives `visible = get(__ui.<id>) ?? startVisible`, and everything falls out free: shown elements survive `router.go()` scene transitions, persist into runtime/editor saves and restore on Continue, reset on New Game, and no `ui:update` event is needed — bars/counters re-render on the same `state:changed` subscription that makes them live (`__` prefix hides the keys from the STATE tab). Dead `"ui:show"` bus event REMOVED; new `ui:menu-pick { elementId, optionId }` (overlay → engine: re-checks option conditions via the newly-extracted pure `checkScriptConditions` — one rule set shared by engine/DialogueRunner/overlay — runs `opt.actions` through the real dispatch, then closes via `__ui` unless `closeOnPick:false`) and `ui:menu-shown/-closed` (overlay → ControlSchemeManager `_uiMenuOpen`, the fourth menu-mode gate — confirm/menuNav route to the bus while a GUI menu is up, dialogue precedence handled overlay-side via a dialogueOpen ref). **`src/ui/GameGuiOverlay.tsx`** mounted in BOTH shells (App under `isPreview`, RuntimeApp under `shell==="playing"`): anchored absolutely-positioned widgets (`pointerEvents:none` except menus, zIndex 90 under DialogueOverlay's 100), menus styled on the DialogueOverlay option grammar (▸ highlight, hover select, click pick); v1 limitation: one navigable menu at a time (first visible wins). Graphic srcs resolve via `assetManager.getGraphicDef` + `resolveUrl` (assetsBase-safe); RuntimeApp boots `initGraphics`. **Editor:** SCRIPTS panel gains a 6th **UI** tab — kind select + New, per-kind field rows (state-key inputs use the `wb-state-keys` datalist, colors via `<input type=color>`, GraphicPickerPopover for icons), menu options nest the existing ConditionRow/ActionRow editors; `show_ui`/`hide_ui` ActionFields become element `<select>`s (custom-id preservation); `uiElements` threads the worldItems chain (ScriptEditor/ActionRow/ActionFields/DialogueEditor/NodeCard/OptionRow); `handleUiElementsChange` mirrors items scoping (project → game.json store on Save, else scene transaction; not journaled — items precedent). Verified in-browser (editor preview + runtime pj-fixture): startVisible render, show/hide via `__test.runAction`, bar 50% / counter ×7 live updates, condition-gated option appears the moment its flag is set, pick runs actions + closes through the `__ui` key, dialogue-open confirm doesn't leak into the menu, and a hidden element STAYS hidden across a scene_01→scene_02 router transition then reappears on show. Gotcha for testers: editor preview exit writes `worldeditor_gamesave` even in plain preview mode — snapshot/restore it in automation. Plan: `plans/phase-49-game-gui.md`; acceptance: `test-plans/phase-49-game-gui.md`.
- v4.43.0 — **Phase 48 — 2D graphics assets (icons / UI images).** New **Graphics** asset type for flat images (item icons, HUD art — transparent PNGs, jpg, webp), parallel to models/materials/decals/sounds/skyboxes: `GraphicDef { id, label, category?, path, width?, height?, attribution? }` + `GraphicsManifest` at **`public/assets/graphics/manifest.json`** (empty manifest committed), loaded by `AssetManager.initGraphics()` (initDecals clone: HEAD-filter, `getGraphicDef`/`getGraphicList`). **`AssetManager.resolveUrl(path)`** — the private `_resolve` made public for raw `<img src>` consumers so item icons/GUI graphics respect the runtime `assetsBase` (BagOverlay + ItemRow previews now wrap srcs; `objectFit:"contain"` so transparent icons aren't cropped; DialogueOverlay portraits share the latent gap, out of scope). **UI:** ASSETS flyout gains a Graphics row (`IconGraphic`, `LeftPanelId "graphics"`); `src/ui/GraphicsBrowser.tsx` — checkerboard tiles (DecalBrowser's transparency-visible backdrop), search, category pills, + Import; exported **`GraphicPickerPopover`** (searchable mini-grid) reused by the ITEMS tab (a **Pick** button beside the Icon URL input writes `graphic.path` into `ItemDef.icon` — which stays a bare URL string, zero migration) and Phase 49's UI tab. **`src/ui/GraphicsImporterModal.tsx`** (AudioImporterModal clone): bulk `showOpenFilePicker`, per-entry labels, ONE shared batch Category (packs share one) + shared attribution, copy → dedupe-splice manifest; intrinsic `width/height` decoded via `createImageBitmap(new Blob([copiedBytes]))` — decoding from the source `File` re-reads a snapshot the write can invalidate. **Icon-from-3D:** the shared thumbnail renderer is now `alpha:true` (opaque `scene.background` keeps normal thumbnails pixel-identical) and `ThumbnailStage.render` takes `{ transparent?: boolean }` (background null + clear alpha 0, restored after); ThumbnailStagerModal gains an **"Icon (transparent background)"** checkbox — checkerboard preview, Save Icon → `App.handleSaveIcon` writes `<assetId>_icon.png` + a graphics-manifest entry via the separate `graphicsDir` FSA grant (`ensureDir` pattern; attribution carried from the model). App wiring mirrors the audio trio (`graphics`/`graphicsDir`/importer state, boot `initGraphics`, reload on complete). Verified in-browser (OPFS-stubbed pickers per TESTING.md §9): bulk import copied files + spliced manifest with 8×8 dimensions; icon mode rendered the Fruit model with corner alpha 0 / opaque center (canvas pixel probe) and saved a valid RGBA PNG + manifest entry; existing thumbnail output unchanged. Plan: `plans/phase-48-graphics-assets.md`; acceptance: `test-plans/phase-48-graphics-assets.md`.
- v4.42.5 — **Undo/redo no longer deselects** (user follow-up to v4.42.4: "if I undo a change, while an object is selected, it deselects the object"). Cause: `WorldState._applyChanges` ended every replay with a blanket `object:deselected` — a leftover from the pre-v4.2.0 snapshot era, when undo rebuilt the whole zone and every mesh died. The per-entity diff replay doesn't need it: the emitted `*:removed` events already prune genuinely-deleted entities from the selection (SelectionManager subscriptions), and `*:rebuilt`/`*:updated` re-attach tint/gizmo/panel exactly as they do for live edits. Emit removed. Panel drafts follow: `PlatformGeoView` and `ObjectGeoView` resync their field drafts **by value** (deps include the data record's fields, not just `selected.id` — the RectFloorFields/StairGeoView precedent; Platform's separate rotation-only resync folded into the one effect) so an undone move/resize shows the reverted numbers in the open Geometry screen instead of stale drafts. Verified in-browser (real click-select → panel X edit → toolbar Undo): data reverts, panel stays mounted on the Geometry screen with resynced values, selection tint survives the platform rebuild, zero `object:deselected` events (spy). Undoing an **add** while selected still deselects gracefully via `platform:removed`. Bonus: undoing a remove+re-add-same-id transaction (the prefab re-expansion shape) keeps the selection — the journal's first-touch-wins collapses it to ONE `platform:updated`.
- v4.42.8 — **`PREFABS_GUIDE.md` added** (user: "was there a guide on prefabs?"). Human-facing authoring guide in the DIALOGUES_GUIDE house style: the recipe/instance mental model, generator vs snapshot, create/place/configure/edit flows, the piece-tweaks-reset rule and Unlink, edit-mode behavior, storage (game.json immediate writes + session promotion), orphan healing, the hollow-kit-tile fine print, and a JSON reference (PrefabDef / PrefabInstanceRecord / stamp, memberKey semantics).
- v4.42.7 — **"Reset from prefab" detaches the gizmo/selection systems before rebuilding** (user screenshot: platform rendered as a melted, draped sheet after moving pieces individually and pressing Reset — "It got pretty borked"). Diagnosis had two independent halves. (1) **The transient bug**: with a member gizmo-attached from the drags, the reset's remove+re-add churn fired shrinking `selection:changed` events that made GizmoManager re-track meshes WHILE ZoneManager was disposing/rebuilding them async — leaving stale half-disposed meshes rendering; the user's DATA was verified never corrupted (their reload cleaned the view, and post-reload defs were a perfect flush grid). Fix: `withInstanceReselect` now emits `object:deselected` BEFORE the transaction (SelectionManager tints + GizmoManager tracking + TransformControls tear down cleanly; App's suppressed listeners keep the panel mounted), then re-selects after the rebuild as before. Verified: move tiles → Reset → flush grid, 9-selected retained, no stray meshes; undo×2/redo×2 clean. (2) **The non-bug discovered under it**: the Phase-43 "Single Height" tiles are NOT thin caps — they're full-height (y −1..1) HOLLOW double-sided shells (grass top + three dirt bands, no bottom/interior), and grass_center is a lone flat sheet at y≈+1; the phase-43 plan's "caps" reading came from measuring only the first mesh primitive. Consequence: any displaced tile exposes neighbors' hollow interiors, which legitimately renders like melted cloth — inherent to the kit, invisible while tiles are flush. The slab preset colliders keep the walkable top at origin.y+1 (correct); the visual body extending to origin.y−1 is cosmetic.
- v4.42.6 — **PrefabSection copy pass** (user: "I don't really understand this text… why is the origin called 'instance'?" — they were right that the label drew a distinction only the data model cares about; variables AND origin are all per-instance). Renames: "INSTANCE ORIGIN" → **"POSITION"**; "Re-expand" → **"Reset from prefab"** (tooltip now states what persists: settings + position keep their values, hand-edits to individual pieces are discarded). Footer subtext rewritten to say what actually never resets: *"The settings and position above are saved per placed copy and never reset. Only hand-edits to individual pieces (shift-click) are overwritten when the pieces rebuild — Unlink first if you want to keep those."* No behavior change. The real fix for the remaining caveat — per-member overrides that survive rebuilds — remains the flagged v2 increment (explicitly deferred by the user).
- v4.42.4 — **Prefab variable edits: live debounced commit + the selection survives re-expansion** (user: "only change on blur, and completely deselect the instance which is jarring. is that required?" — it wasn't). (1) `PrefabVarField` numbers now commit on a **600ms debounce** from typing/spinner-stepping (blur/Enter flush immediately) — the delay exists so a half-typed "1" (min-clamped to 2) can become "12" before committing. (2) The deselect's cause: re-expansion removes + re-adds members, and SelectionManager's `*:removed` subscriptions dropped the whole selection mid-commit. New App wrapper **`withInstanceReselect`**: sets `suppressSelRef` (the `object:deselected` / `selection:changed` App listeners early-return, keeping the panel mounted and the focused input alive), runs the transaction, then 150ms later (member meshes rebuild async) re-emits the instance's member set through `selection:set` with the previous primary preserved. Used by the variables/origin/Re-expand handlers. Verified live on the user's 4×3 platform: typed "6" with NO blur → platform regrew to 6×3 at the debounce, "18 selected" re-established, panel never unmounted, **focus still in the width input**.
- v4.42.3 — **Single-object overlay editors detach on multi-selection** (user screenshots: selecting a platform instance showed a mysterious tile-sized cyan wireframe box with small red/blue/green cubes beside the group gizmo). Cause: `ColliderEditor` (cyan collider outline + 6 face drag-handles) subscribes to `object:selected`, and the instance-selection path emits that for the PRIMARY member before `selection:changed` — so the clicked tile got decorated with its own 2×2 collider editor while the whole 8×6m platform was selected. Same latent leak in `TriggerVolumeResizer` / `ShapeResizer` / `StairCutterResizer` for instances whose primary is a volume/shape/stair. All four now subscribe to `selection:changed` and detach when `refs.length > 1` (mid-drag guarded where applicable) — multi-selections show only the group gizmo. Verified in-browser: clicking the user's 4×3 platform shows 12-selected + Prefab section + translate gizmo, zero stray overlays.
- v4.42.2 — **Orphaned GENERATOR instances auto-heal on load** (user report: their platform "has no obvious editable length/width properties" — it was an orphan showing the v4.42.1 ⚠ panel). A generator instance's record already stores everything except *which* generator (`variables {width, depth, tileSet}`) — so the load-time sweep (now extracted to `syncPrefabInstances`, refs-only useCallback) infers the generator by **exact variable-key-set match** against the `GENERATORS` registry and relinks the record to that generator's library def, creating the def (and persisting it immediately) if the library lacks one. Members stay untouched — relink only. Snapshot orphans still can't heal (template genuinely lost) and keep the ⚠ panel. **Boot-ordering guard:** the sweep early-returns while `restoringRef` is set — during boot the autosave-restored world loads BEFORE the project library, and healing against the empty session library would mint duplicate defs; instead the two project-open sites (boot restore + `adoptProject`) re-run the sweep once `world.prefabLibrary` is authoritative. Verified on the user's actual platform in a fresh tab: record relinked to the game.json def, click → full Width/Depth/Tile-set panel restored. Note: the healed record persists via autosave immediately and via the scene file on next save; the heal is idempotent, so unhealed copies elsewhere just re-heal on their next load.
- v4.42.1 — **Prefab defs write game.json IMMEDIATELY + orphaned instances get an honest panel.** User report: selecting their placed platform showed "9 selected … Create Prefab" — nonsense for an instance. Root cause: library edits only reached game.json on Save, so a def created by placing a generator lived solely in that tab's memory; any other/fresh session saw a **dangling prefabId** — grouped selection worked (stamps) but no def meant the plain multi view. Two fixes: (1) `applyPrefabs` in project mode now calls `store.writeGame()` **immediately** (the asset-import manifest.json precedent) — placing/creating/renaming/editing a prefab persists the library on the spot, verified on disk mid-session; (2) `selPrefabInfo` returns `{prefab: null, record}` for a full-instance selection whose def is missing, and `PrefabSection` renders a degraded **"⚠ Prefab instance · definition missing"** view (explanation naming the missing `pfb_` id + Unlink / Delete instance — the def-free operations; variables/re-expand impossible; Create Prefab and the generic Delete hidden). Verified in-browser on the user's actual orphaned platform. Instances placed before this fix whose defs were never saved remain orphans (defs unrecoverable) — Unlink is the recovery path.
- v4.42.0 — **Clicking a prefab-instance member selects the whole instance** (user report: "as soon as I place it, I'm selecting one piece at a time"). SelectionManager `_onClick` (plain select mode, non-additive): picked entity carries a `prefab` stamp → gather every member of the instance and `_setSelection` them (clicked member = primary; **shift/cmd-click still toggles the single member** — the tweak-one-tile escape hatch, stated in the panel hint). The multi-select Properties view now hosts the **PrefabSection** when the selection is exactly one instance's full member set (App's `selPrefabInfo` verifies count + ids; mixed/partial selections keep the plain multi view) — width/depth/tileSet, origin fields, Re-expand/Unlink/**Delete instance** (which replaces the generic Delete there, avoiding orphaned link records), Create Prefab hidden for an existing instance. **GizmoManager `_syncPrefabOrigin`**: a group-translate whose refs are exactly one instance's full member set also shifts the record's `origin` by the drag delta, inside the same move transaction — without this, the next re-expansion would snap the members back to the stale origin (verified: after a real gizmo drag, `record.origin.position.x === centerTile.position.x` exactly). Verified in-browser on the user's own placed platform (selection grouping; read-only) and a test instance (panel + drag + origin sync), test instance purged from world AND the shared autosave before close. Note: an instance whose prefab def isn't in the library (e.g. game.json not yet saved) still groups on click but shows the plain multi view — dangling-prefabId degradation, by design.
- v4.41.3 — **Prefabs panel: duplicate header removed** (user screenshot: "prefabs twice?"). The LeftPanel wrapper already renders the panel title + ✕; PrefabPanel's own header row carried no controls (unlike GroupPanel's, which hosts "+ New") so it was pure stutter — dropped, content starts at the library list.
- v4.41.2 — **Prefabs moved OUT of the ASSETS flyout to their own toolbar button.** User call: prefabs are authored arrangements of world entities (project data in game.json), not imported assets — the flyout's "Import & place assets" identity didn't fit. The `ASSET_ENTRIES` row was removed and a standalone **Prefabs** panel button (48×48, `IconPrefab`, Scripts-button pattern) now renders between Assets and Trigger, toggling the `prefabs` panel; it highlights when the panel is open OR the `prefab` placement tool is armed. Verified in-browser: flyout lists only Models/Materials/Decals/Sounds/Skybox; the new button opens/highlights the panel; autosave untouched.
- v4.41.1 — **Tile thumbnails generated for all 19 Phase-43 platformer-kit assets.** Fully automated without an FSA grant: a throwaway localhost upload endpoint (CORS'd python http.server, only accepting `platform_*` slugs with valid PNG signatures) received PNGs rendered in a live editor tab by the app's own `renderModelThumbnail`/`ThumbnailStage` (modules imported via their Vite URLs from the console; a fresh module instance is fine here since the stage is self-contained). Manifest `thumbnail` fields patched for the whole Tiles category — the AssetBrowser's 3-letter fallback squares are gone. Remaining phase-48 polish items deferred as documented follow-ups: "Bake instance" (bakeShapes.ts reuse for the draw-call escape hatch), real-mesh placement ghost, FpsCounter 10×4×4 stress pass, beforeunload confirm for unsaved prefab edits.
- v4.41.0 — **Phase 47 — Isolated prefab edit mode + propagation (the "linked instances" promise delivered).** `src/prefab/PrefabEditSession.ts` (snapshot prefabs only — a generator's params ARE its interface): **staging = temp zone `__prefab_edit__` in the LIVE WorldState** (not a state swap — every tool/selection/gizmo already operates on the active zone). Enter (Edit button in the Prefabs panel): remember prev zone + camera pose → build the staging ZoneDef from `expandPrefab` at origin with **entity ids = memberKeys** (recapture reads them back; pre-existing members keep keys naturally) → unjournaled `addZone` → unload prev zone / load staging / setActiveZone → `history.clear()` → frame camera. Save (amber `PrefabEditBar`): recapture template from the staging zone's PREFABABLE arrays, `version++`, teardown (unload staging, new unjournaled `WorldState.removeZone`, reload prev zone, clear history, restore pose), persist the def, then **propagate: one undoable transaction re-expanding every open-scene instance** (memberKey diff keeps entity ids stable). Cancel = teardown only. **The contamination guards (the 2026-07-16 incident class) — all verified live:** (a) `writeAutosave` + `handleSave` + preview/new-game/continue + project scene-switch + closeProject all early-return on `editingPrefabRef` (a beforeunload dispatched mid-edit left the autosave byte-identical); (b) belt-and-braces `WorldState.toJSON()` filters the staging zone (`toJSON().zones === ["demo"]` while editing). Verified in-browser end-to-end: capture 2-cube prefab → place 2nd instance → Edit (world vanishes, only members at origin on the empty grid, amber bar) → move a member → Save → def v2, BOTH instances re-expanded at origin+offset (records v2), ONE Cmd+Z reverts the whole propagation (members + record versions; the def itself stays v2 — library is un-journaled by design, and the load-time staleness sweep re-applies it coherently on next load), redo re-applies; Cancel discards edits with template untouched. Flagged tradeoffs: world undo history clears on enter/exit (Unity-like; scoped stacks later), no beforeunload confirm for unsaved prefab edits yet. Plan: `plans/phase-47-prefab-edit-mode.md`; acceptance: `test-plans/phase-47-prefab-edit-mode.md`.
- v4.40.0 — **Phase 46 — Snapshot prefabs: capture-from-selection (the door case).** `captureSnapshotPrefab(world, refs, name)` in `src/prefab/expand.ts`: deep-clones the selection's PREFABABLE members, normalizes to template space (XZ pivot = anchors' centroid, y = 0 at the lowest anchor; stairs contribute start+end), strips any existing `prefab` stamps (capturing another instance's members yields a fresh template), memberKey = the source entity id — which is exactly the key `remapScripts` remaps intra-prefab references through at instantiation. Non-capturable types (node-backed wall/floor/platform) are skipped + console-warned. **UX:** the multi-select Properties screen gains **"⬡ Create Prefab"** (shown when ≥1 selected ref is capturable) → `handleCreatePrefab`: names it `Prefab N` (rename in the panel — the addGroup "New Group" precedent, no user-activation-eating prompt()), adds the def to the library, then in ONE undoable transaction **replaces the originals with the prefab's first linked instance** (`removeEntities` + `instantiatePrefab` joining the same transaction), and opens the Prefabs panel. `TriggerVolumeView` now renders the `PrefabSection` too (via a `prefabSection` node prop — volumes are root-rendered outside the generic isRoot path). Verified in-browser on a real door rig (door1 model + trigger volume whose `on_player_enter` script `move_object`s the door): group-select both → Create Prefab → library def with 2 template members, originals replaced by a stamped instance whose volume script targets the NEW door id; placed a second instance — **each instance's volume targets its own door** (`targetIsOwnDoor` true for both), and in preview, walking into instance 2's trigger moved ONLY door 2 (door 1 untouched) — the per-instance script remap proven end-to-end through the real TriggerSystem. **Documented v1 caveat found in that test:** literal *coordinate payloads* inside captured actions (`move_object.position`, teleport coords) are captured verbatim and NOT re-based per instance — the second door moved to the template's absolute coordinates. Prefab-friendly scripts should prefer relative/targeted actions (`open_door`, `play_animation`, `despawn_object`); action-coordinate re-basing is a possible later increment. Plan: `plans/phase-46-snapshot-prefabs.md`; acceptance: `test-plans/phase-46-snapshot-prefabs.md`.
- v4.39.0 — **Phase 45 — Prefab variables, re-expansion, unlink, instance Properties section.** The live half of the prefab loop. **`src/prefab/expand.ts` grows the diff machinery:** `collectInstanceMembers` (scan the zone's PREFABABLE collections by stamp), `reexpandInstance` — diff-apply by memberKey in one transaction: members present in both **keep their entity id** (remove + re-add under the same id, chosen over a shallow update-merge which couldn't drop stale fields; external script refs / groupIds references / selection survive), new keys add, gone keys remove, record `version` bumps; `unlinkInstance` (strip stamps + drop record); `deleteInstance` (members + record). **PropertiesPanel `PrefabSection`** (root screen, when the selection is a stamped member): variable editors from the prefab's `PrefabVariableDef` schema (number/choice/boolean, blur/Enter commit, clamped), INSTANCE ORIGIN X/Y/Z/ROT° fields (move-the-whole-instance path — origin edit re-expands with stable ids), Re-expand / Unlink / Delete-instance buttons, and the documented v1 semantic: *manual member tweaks reset on re-expansion — unlink first for a one-off*. App handlers wrap record-update + re-expand in ONE transaction and refresh the selection from the live def (`refreshSelectionAfterReexpand`). **Load-time staleness sweep** in `handleLoadFromJSON`: any record with `version < prefab.version` re-expands + marks dirty (missing prefab → warn, keep expanded copies) — this is how other project scenes catch up with prefab edits (full propagation UX lands with isolated edit mode, phase 47). **`pasteClipboard` strips `prefab` stamps** — a pasted member copy is unlinked by design (a duplicate stamp would collide in the memberKey diff). Verified in-browser through real UI: select tile → section shows; width 3→5 re-expands 9→15 members with the selected tile's id stable (`obj_3cda053c` before/after); ONE undo reverts the whole resize (record + members), redo restores; Unlink → 0 records/0 stamps/15 plain tiles, undo restores all 15 stamps + record; Delete instance clears everything + deselects; autosave snapshot-restored. Plan: `plans/phase-45-prefab-instance-ux.md`; acceptance: `test-plans/phase-45-prefab-instance-ux.md`.
- v4.38.0 — **Phase 44 — Prefab core: expansion pipeline, instance records, generator framework + Tiled Platform.** First functional slice of the prefab system (approved plan: generator-prefab DynamicPlatform, library in game.json, isolated edit mode later). **Data model (`src/types.ts`):** `PrefabDef` (`pfb_`, kind `snapshot|generator`, `version`, `variables: PrefabVariableDef[]`, `template?`), per-zone `ZoneDef.prefabInstances?: PrefabInstanceRecord[]` (`pfi_` — keyed `id` so the generic undo journal applies; holds prefabId/version/variables/origin) and a `prefab?: PrefabStamp {prefabId, instanceId, memberKey}` on the five PREFABABLE member types (object/trigger-volume/shape/stair/ladder). **Scenes store fully-EXPANDED entities + this metadata — the runtime renders members as plain entities and needs zero changes; generator code never runs at runtime.** **Expansion pipeline (`src/prefab/expand.ts`):** `expandPrefab` (generator `GENERATORS[id].expand(vars)` | snapshot template clone) → `instantiatePrefab` = one transaction: fresh member ids (memberKey-stable map for later re-expansion), intra-prefab script-ref remap (`remapScripts` rewrites `trigger.targetId`/`actions[].targetId`/`conditions[].npcId` only for ids in the member idMap — external/group ids pass through), prefab-local→world transform (yaw about origin: objects/shapes rotate `rotation.y`, volumes create/merge `rotation`, ladders `rotationY`, stairs rotate start+end), stamps, then `world.addX` per member + `addPrefabInstance`. Undo/redo of a 9-member place is ONE step and round-trips the record (verified: same `pfi_` id back). **WorldState:** add/update/removePrefabInstance (`"prefabInstance"` ChangeKind + `_zoneArr`/`_emitChange` wiring; records are metadata — no ZoneManager listener) + session field `prefabLibrary?` (gameItems precedent). **Library (`src/prefab/library.ts`):** project open → `GameConfig.prefabs` (game.json, written on Save); no project → localStorage `worldeditor_prefabs`; on project open, session prefabs promote by union-by-id (game.json wins) + isDirty. **Generator framework (`src/prefab/generators/`):** `PrefabGenerator {id,label,variables,expand}` registry; first generator **`tiled-platform`** — the old ecctrl `DynamicPlatform` re-derived clean: width×depth (2–32) grass/dirt grid of Phase-43 tiles at 2m pitch, centered, corners rotY 0/90/180/−90 + edge sides per outward face (Corner natural = −X/+Z, Side = +Z), interior centers; walkable top at origin.y+1. **UI:** Toolbar ASSETS flyout gains a **Prefabs** panel row (`LeftPanelId "prefabs"`, `IconPrefab` hexagon+linked-copy); `src/ui/PrefabPanel.tsx` lists library defs (kind badge ƒ/⬡, live instance count, Place/rename/delete — delete disabled while instances exist) + built-in generators (first Place lazily creates the library def); `src/editor/PrefabTool.ts` (ToolId `"prefab"`, ObjectTool template): translucent extents-box ghost, 0.5m snap, R rotates 90°, Esc stops, continuous placement. Instance counts refresh via `prefabinstance:added/removed` bus → `prefabTick`. Verified in-browser through real clicks on the user's project scene: flyout→panel→Place→ground click produced a seamless 3×3 grass platform (skirts all outward), record + 9 stamped members correct in `__world`, preview character stands at y≈1.91 on center AND corner tiles (slab colliders, incl. the flat-plane grass center), one-step undo/redo, Tiles category pill lists all 19 kit assets; test artifacts fully undone + autosave snapshot-restored. Plan: `plans/phase-44-prefab-core.md`; acceptance: `test-plans/phase-44-prefab-core.md`. **Next slices:** variables/re-expand/unlink + instance PropertiesPanel (45), snapshot capture (46), isolated edit mode + propagation (47).
- v4.37.0 — **Phase 43 — Platformer-kit tile assets imported (first slice of the prefab plan).** 19 modular platform tiles from the Quaternius **Ultimate Platformer Pack** (CC0) copied into `public/assets/models/` under new category **"Tiles"** (custom `AssetCategory` string — gets its own pill in AssetBrowser): Single Height grass+dirt Center/Side/Corner (6), Single Cube grass+dirt (2), and the 3D "Tall" stacking family (11). Slugs `platform_<set>_<role>[_tall]`, all self-contained embedded-base64 `.gltf` (the one external-buffer file, `Cube_Dirt_Center_Tall.bin`, was inlined at import). **Collider notes (why several ship preset `colliders[]`):** the kit's tiles are *shells*, not solids — the grass Single Height pieces are thin caps (top plane at y≈1 + skirt to y≈0.53; `Cube_Grass_Center` is a completely flat plane, so an auto-box from its AABB would be zero-height), and four Tall pieces are single flat planes (grass+dirt `Center_Tall` at y=+1, grass `Bottom_Tall` at y=−1, grass `SideCenter_Tall` at z=+1). So: the 3 grass Single Height tiles get an explicit slab box (`{2,0.5,2}`, top at y=1), `platform_grass_single` a full `{2,2,2}` cube (its AABB is skirt-oversized to ±1.117), the 4 flat-plane Tall pieces thin boxes over their faces; the remaining 11 (full-height shells/solids) use the normal auto-box. Tile logical footprint is 2×2×2, walkable top at origin.y+1 — these feed the upcoming tiled-platform generator prefab (phases 44+). Manifest-only change (zero src); thumbnails deferred (3-letter fallback tiles) pending a browser re-stage session. Plan: `plans/phase-43-platformer-tiles.md`.
...
export interface BusEvents {
  ...
  "asset:model-updated": { assetId: string };        // model FILE rewritten in place (re-origin, v4.52.0) — ZoneManager rebuilds placed copies
...
export interface AssetDef {
  ...
  tags:          string[];                // free-form, normalized lowercase-dashed. Authored at
                                          // import (batch-shared), edited via EditMetadataDialog,
                                          // filtered by the AssetBrowser's `#` mode (Phase 51).
                                          // Carries the axes `category` can't hold at once — pack,
                                          // kind, license, `animated`. Library metadata: never
                                          // read at runtime, never referenced by placed entities.
...
export interface TriggerVolume {
  ...
  label:    string;
  position: Vec3;
  size:     Vec3;             // box: width, height, depth; sphere: x = radius; cylinder/capsule: x = radius, y = full height
  shape?:   "box" | "sphere" | "cylinder" | "capsule";   // absent = box (Phase 59); position stays XZ center + Y bottom for every shape
...
├── package.json
├── deno.json                       ← desktop-shell tasks: desktop / desktop:hmr / desktop:dev (shell + vite build --watch, v4.72.0) / compile:{mac-arm64,mac-x64,win-x64} (Phases 54–57)
...
│   ├── serve.ts                    ← /games/* + /assets/* workspace-first with fallthrough to dist (disk in dev, VFS compiled); dist HTML no-store (v4.72.0)
...
│   ├── App.tsx
│   ├── shared/
│   │   └── desktopApi.ts           ← typed client for the shell's HTTP api (+ detectDesktop/desktop/isDesktop/isDesktopDev, uploadAssetFile)
...
│   │   ├── WorldSerializer.ts
│   │   ├── ZoneManager.ts
│   │   ├── volumeShape.ts          ← trigger-volume shape helpers: volumeExtents/volumeGeometry per shape (Phase 59)
│   │   ├── MoverSystem.ts          ← scripted geometry motion: kinematic bodies follow mover defs each frame (Phase 31)
│   │   ├── transformWatchdog.ts    ← dev-only: warns when two systems write one entity's transform in a frame (Phase 62)
...
│   ├── editor/
│   │   ├── EditorCamera.ts
│   │   ├── AiRangeRings.ts        ← Enemy AI screen's SHOW RANGES viewport circles (Phase 63)
...
│   ├── scripting/
│   │   ├── ScriptEngine.ts         ← Runtime: trigger index, condition eval, action dispatch (Phase 60: entity-scoped state, transfer_item; Phase 61: AI triggers)
│   │   ├── entityState.ts          ← per-entity state facade: __ent.<id>.<key> helpers + schema registration (Phase 60)
│   │   └── GameState.ts            ← Generic runtime state store (see GAMEPLAY_STATE.md; replaced GameStateManager)
│   ├── audio/                      ← Phase 36 — audio system
│   │   └── AudioSystem.ts          ← AudioListener + music/ambient/positional playback + 4-bus mixer + per-channel playlist sequencer (Phase 64; both roots)
...
│   ├── preview/
│   │   ├── PreviewController.ts    ← owns the ControlSchemeManager session (Phase 24); playerPosition accessor (Phase 61)
│   │   ├── enemyBrain.ts           ← PURE decision core (no engine imports): tick(mem, params, senses) → intent; Deno-testable (Phase 62)
│   │   ├── EnemyAI.ts              ← enemy AI adapter: senses from rays/player, executes brain intents on the kinematic host (Phase 61/62)
...
**`setGridVisible(v)`** (v4.60.2): toggles the grid helpers + demo ground's
`visible` flag for the editor's global "Grid floor" preference. Meshes stay in
the scene (placement raycasts unaffected — `worldPos` comes from a math plane
anyway), and the helpers' `hideInGame` tag is set to `v` while toggled so
game-mode exit's blanket "restore hideInGame helpers" pass in ZoneManager
can't resurrect a hidden grid.
...
>
> **v4.38.0 (Phase 44):** `addPrefabInstance` / `updatePrefabInstance` /
> `removePrefabInstance` — same optional-array pattern (`zone.prefabInstances ??= []`,
> `"prefabInstance"` ChangeKind, `prefabinstance:added/updated/removed`, `_emitChange`
> case). Records are pure link metadata (no meshes — **ZoneManager has no listener**);
> they're keyed by `id` (`pfi_<uuid8>`) precisely so the generic by-id journal machinery
> applies, and journaling them means undoing an instance delete restores the link.
> Session field `prefabLibrary?: PrefabDef[]` mirrors `gameItems` (set by App from
> game.json or the localStorage session library; never serialized into the scene).
>
> **v4.44.0 (Phase 49):** session field `gameUiElements?: UiElementDef[]` — the
> game.json custom-GUI registry, same non-serialized contract as `gameItems`
> (set by App's project sites and SceneRouter; merged under the scene's
> `WorldConfig.uiElements` by `uiRegistry`).
...
>
> **v4.42.0 (Phase 47.1):** a plain click on a **prefab-instance member** selects the
> WHOLE instance — `_onClick` (plain `select` mode, non-additive) checks the picked
> entity's `prefab` stamp and routes every member ref through `_setSelection`
> (clicked member = primary). Shift/Cmd-click still toggles the single member (the
> tweak-one-tile escape hatch); sub-object modes keep raw behavior. The paired
> GizmoManager change: `_commitGroupTranslate` calls `_syncPrefabOrigin` — when the
> dragged refs are exactly one instance's full member set, the record's origin
> shifts by the same delta inside the same transaction, so a later re-expansion
> (width change, prefab save) doesn't snap the platform back to a stale origin.
...
> **v4.47.0:** face handles moved behind the panel's per-collider **Resize** toggle —
> `collider:move` mode is now `"translate"|"rotate"|"resize"`; resize detaches the TC and
> shows the 6 face handles for that collider only (nothing shows by default).
> **v4.48.0:** Resize covers sphere (radius, center-pinned) and capsule (sides = radius,
> ends = height with the far cap pinned) — hull/trimesh remain wireframe-only.
> **v4.45.0:** ColliderEditor's per-collider proxy TC is dual-mode — `collider:move`
> carries `mode: "translate"|"rotate"`; rotate runs in local space with a 1° snap (v4.46.3)
> (Alt = free) and writes `AttachedCollider.rotation` (local euler, deg).
...
| `ui:menu-shown` / `ui:menu-closed` | GameGuiOverlay → ControlSchemeManager | `{ elementId }` — custom GUI menu up/down (fourth menu-mode gate; Phase 49) |
| `ui:menu-pick` | GameGuiOverlay → ScriptEngine | `{ elementId, optionId }` — picked menu option; engine re-checks conditions, runs its actions, closes via `__ui.<id>` unless `closeOnPick:false` (Phase 49) |
...
| WASD | Pan focus point on XZ plane |
| ↑ / ↓ | Move focus point up / down the world Y axis, half pan speed (v4.47.0/.1) |
| ← / → | Rotate the view left / right (the scene swings the other way), ~0.9 rad/s (v4.47.0/.1, direction flipped v4.48.2) |
...
> **v4.82.0:** `getLivePosition(id)` returns where an object's mesh IS right now (an AI enemy
> walks away from its authored position). ScriptEngine's `launch_player` "away" frame reads it
> through `setLivePositionProvider`.
...
> **v4.56.0 (Phase 53):** attached trigger volumes. `_buildVolumeCollider` looks up
> `_movers.hostFor(vol.attachTo)` and parents the sensor to the host's kinematic
> body (`PhysicsWorld.createSensorColliderOn` — sensor + KINEMATIC_KINEMATIC, the
> combination Phase 31 declared out of scope); missing/mover-less/body-less hosts
> fall back to the classic static sensor. Wireframe + gradient fill ride via
> `_movers.attachMeshes` (still zone-group children — selection/dispose untouched).
> `_reattachVolumes(hostId)` (remove+re-add of attached volumes) runs synchronously
> at the end of `_addPlatform`/`_rebuildPlatform`/`_addShape`/`_rebuildShape`/
> `_buildObjectColliders` and in the platform/shape/object **removed** handlers —
> host rebuilds free the kinematic body (orphaning the sensor) and `register()`
> wipes attached meshes. `_removeSingleVolume`/`_removeTriggerVolumes` guard
> collider removal with `isValid()` (an attached sensor dies WITH its host's body)
> and detach visuals from the mover entry first.
>
> **v4.52.0:** handles `asset:model-updated { assetId }` (a model file was rewritten in
> place by the re-origin flow — same assetId, new geometry): every placed object with
> that assetId in every loaded zone is rebuilt by remove+re-add (`_removeObject` +
> `_addObject` + `_refreshStaticShadows`), the same path the prefab assetId-swap rebuild
> uses, so meshes, `localAABB`, and auto-fit colliders all derive from the fresh file.
>
> **v4.50.0:** the `object:updated` collider-rebuild gate tests `"colliders" in changes`
> (presence, not truthiness) — clearing an override back to `undefined`, which re-tracks
> the asset's default collider set ("Save as default for this model"), must also rebuild.
>
...
> **v4.56.0 (Phase 53):** three additions for attached trigger volumes —
> `hostFor(id)` (the entry's body + rest origin/quat, the missing body-by-entity-id
> lookup), `attachMeshes(id, objs)` (append extra meshes — volume wireframe/fill —
> to the entry so `_applyPose`/`_resetAll` carry them; `register()` still wipes the
> list, ZoneManager's `_reattachVolumes` restores attachments after every host
> rebuild) and `detachMeshes(id | null, objs)` (null = scan all entries; the volume
> def may already be gone from WorldState at teardown).
...
**Object model swap on `object:updated` (v4.44.3)**
Live edits never change an object's `assetId` — but an undo/redo of a prefab
re-expansion does: the journal collapses remove+re-add of the same id into ONE
`object:updated` whose payload can flip the model (a tile turns corner↔side as
the grid grows). The handler compares `changes.assetId` against the built
mesh's `userData.assetId` (stamped by `ObjectPlacer.build`) and, on mismatch,
does a full `_removeObject` + `_addObject` (mesh, colliders, mover, shadows)
instead of the transform/collider patch — which can't re-model.
...
**History restore**
`history:restore` → `unloadZone(activeZoneId)` then `loadZone(activeZoneId)`. Vestigial: nothing emits `history:restore` since the v4.2.0 per-entity-diff undo redesign — `WorldState._applyChanges` replays the normal `*:added/updated/removed` events instead, so ZoneManager rebuilds only the touched meshes (no full zone reload). Since v4.42.5 `_applyChanges` also does NOT clear the selection: undo/redo behaves like the equivalent live edit (`*:removed` prunes deleted entities from the selection via SelectionManager's subscriptions; `*:rebuilt`/`*:updated` re-attach and refresh the rest).
...
**In-flight sharing + preload surface (v4.84.0):** the caches hold only FINISHED
results, so two concurrent calls for one asset used to fetch and decode it twice.
A private `_once(key, load)` map now makes callers for the same key share one
promise: `loadGLTF` (keyed on the versioned URL, and it only writes the cache if
`evictModel` has not bumped the version mid-flight), `_loadOBJ`, `loadTexture`,
`getMaterial`, `loadSound`, `loadSkybox`. This is what lets a level build that
starts while the runtime's `ScenePreloader` is mid-download join those loads
instead of repeating them. Two additions for the preloader: `preloadModel(id)`
(warms the GLTF/OBJ cache without handing out a clone) and
`fileUrls(kind, id)` (resolved URLs an id loads at the CURRENT quality, enabled
map slots only; unknown ids yield `[]`).
...
**Model eviction (v4.52.0):** `evictModel(id)` busts every cache layer for a model whose
FILE was rewritten in place (re-origin) — deletes the `_gltfCache` entries (`id` +
`obj:id`) and bumps a per-model counter in `_modelVersion`; `loadGLTF` appends it as
`?v=n` to the fetch URL so the browser HTTP cache can't serve the stale bytes. The
registry entry is untouched (unlike `removeAssets`).
...
**2D graphics (Phase 48):** `initGraphics(opts?)` fetches
`/assets/graphics/manifest.json` into a `_graphicsRegistry`
(`GraphicDef { id, label, category?, path, width?, height?, attribution? }`;
same HEAD-filter pattern), with `getGraphicDef` / `getGraphicList`, plus
`updateGraphic(id, patch)` / `removeGraphics(ids)` (v4.60.0 — the sound/skybox
registry-mutation pattern, no cache to evict). Graphics are
consumed as `<img src>` (item icons, GameGuiOverlay elements, browser tiles) —
never as scene textures — so the class also exposes **`resolveUrl(path)`**, a
public delegate to `_resolve`, which every raw `<img>` consumer must wrap its
src in so `/assets/**` paths respect the runtime shell's `assetsBase`.
...
if (dir.lengthSq() > 0) dir.multiplyScalar(speed);             // the WANTED velocity; magnitude scales speed
// Phase 70: _velX/_velZ move toward it at a limited rate, then dir = vel x dt (see note below)
...
> **v4.80.0 (Phase 70: character feel):** the constants block "Character feel (Phase 70)"
> is the tuning surface. **Horizontal:** `_velX/_velZ` ramp toward `input x moveSpeed`
> (`GROUND_ACCEL_SEC` / `GROUND_DECEL_SEC` 0.08, `AIR_ACCEL_SEC` 0.2; rate = speed / seconds,
> grounded read = `_coyote > 0`); zeroed on teleport, ladder mount and while climbing.
> **Vertical:** `_jumpArc` is true from a jump takeoff to its landing and selects
> `GRAVITY_RISE` / `GRAVITY_FALL` (both derived from `JUMP_RISE_FRAC` so peak and air time
> match the legacy arc), or `GRAVITY_RISE x JUMP_CUT_MULT` once jump is released while
> rising (unless `variableJump === false`); everything else (launches, walk-off falls)
> uses `LEGACY_GRAVITY`. Takeoff keeps the legacy mapping `peak = jumpHeight x 9.81/20`.
> **Presentation:** `_buildShadow` / `_updateShadow` (one Rapier ray, third person only)
> and `_updatePresentation` (squash spring + lean; owns the root's scale and YXZ
> rotation, base scale in `_modelBaseScale`). **Landing block:** gated on
> `isGrounded && _velY <= 0` (a lip graze while rising is not a landing); it plays the land
> sound, kicks the squash spring with `_lastAirVelY`, and queues `_statsPending`, which is
> emitted as `character:jump-stats` 6 grounded frames later with `drop` read from the
> settled feet (or immediately, using the last grounded feet, if the player is airborne
> again first). A climb or a teleport invalidates the current stretch (`_airPeakY = NaN`).

> **v4.81.0 (Phase 71: run):** `running = (runMultiplier ?? 1) > 1 && actions.run && isMoving`.
> Wanted speed is `moveSpeed x runMultiplier` at full magnitude while running; otherwise
> `moveSpeed x k`, where `k` is the stick magnitude, or `min(1, magnitude / RUN_STICK_THRESHOLD)`
> when run is enabled (the analog walk range ends where run begins). It feeds the same
> `_velX/_velZ` ramp. `_updateAnim(airborne, isMoving, running)` picks the ground clip via
> `_groundClip` (`run` only if the model has one). `actions.run` comes from the input
> sources (`src/input/`): Shift, or the move stick at 85% or more.

> **v4.81.1:** every clip change goes through `_crossfadeTo(next, loop, speed)`. It must
> never crossfade an action with itself: `mixer.clipAction(clip)` is one action per CLIP,
> and two intents can resolve to the same clip (an author's `animClips` override, a script
> naming the playing clip). Same action = keep playing, only re-loop/retime. `_playGround`
> plays idle/walk/run and speeds a walk clip up by `runMultiplier` when run shares it.

> **v4.81.3:** the `jump_land` one-shot plays for STANDING landings only. Landing with a
> direction held goes straight to `_playGround` (`_enterLand(isMoving, running)`), and the
> `"land"` phase exits on `isMoving` as well as on `_animDone()`.
...
enter(mode) {
  ...
  if (input.activeScheme === "kbm") this._lockPointer();   // kbm-only; best-effort, swallows a refused lock (v4.79.83)
}
```
...
- **Tab sub-modes** (window keydown, capture phase, removed on exit):
  ...
  Every request goes through `_lockPointer()`, which catches the promise
  rejection Chrome raises for a refused lock (v4.79.83).
...
>
> **v4.39.0 (Phase 45):** **`PrefabSection`** on the root screen whenever the App passes
> `prefabInfo` (the selection carries a `prefab` stamp): kind badge + prefab name header,
> per-variable editors from `PrefabVariableDef` (number drafts commit on blur/Enter with
> min/max/step clamping; choice select; boolean checkbox), an INSTANCE ORIGIN row
> (X/Y/Z/ROT° — the supported move-whole-instance path), and Re-expand / Unlink /
> Delete-instance buttons. All commits flow through App handlers
> (`handlePrefabVariablesChange` etc.) that wrap `updatePrefabInstance` + `reexpandInstance`
> in ONE transaction, then refresh/drop the selection (`refreshSelectionAfterReexpand` —
> the member def object was replaced under the same id). Values render from the live
> `PrefabInstanceRecord`, so the section survives re-expansion.
>
> **v4.79.58 (Phase 69):** `TriggerVolumeView` restructured into six option-A accordions
> (Geometry / Scripts / State / Attach / Appearance / Groups) via a local `Section`
> component (GroupsAccordion-style header + collapsed-only summary); rotation gains a
> slider + in-field × reset; `EntityStateSection` restyled to typed cards (segmented
> bool/num/text picker, STARTS line, MIN+MAX line for numbers) with its explainer behind
> a ⓘ (`InfoBlurb`), and a `heading?` prop.
...
existing `materialList`/`assets` props), plus three global-preference checkboxes
(v4.60.1/v4.60.2): **perf counter**, **crosshair**, and **grid floor**. Their
state lives in App (`showPerfCounter`/`showCrosshair`/`showGridFloor`,
persisted to localStorage `editorShowPerf`/`editorShowCrosshair`/
`editorShowGrid` the way `editorQuality` is — `'0'` = off, absent = on) and
threads App → PropertiesPanel → ToolView; the perf one gates `<FpsCounter>` in
App's preview overlay stack, the crosshair one is the `showCrosshair` prop on
PreviewHUD, and the grid one calls `SceneManager.setGridVisible` (applied at
boot right after construction, before the first render).
...
- Search input filters by asset name **and by tag**
- **`Categories | Tags` segmented switcher (Phase 51)** above the strip picks
  which facet the pills below show: categories (exclusive, one shelf) or tags
  (multi-select, ANDed). Tags are ordered by frequency, active ones pinned to
  the front, overflow in the shared `More ▾` popout (which stays open across
  clicks in tag mode). Tag chips render as `#tag count`; category pills stay
  bare — **the `#` and the count are what distinguish the two facets**, since
  both use the same pill styling. The `Tags` segment carries the active tag
  count, a right-aligned `clear` resets both filters, and in tag mode an active
  category also renders as a clearable `Category ✕` pill so it can't hide
...
- **Manage mode single-selection actions:** 📷 re-stage thumbnail (v4.3.2) and
  **⌖ re-origin (v4.52.0)** — both enabled only at exactly one checked asset,
  ...
  App `reoriginAsset` state → `ReoriginModal`.

### AudioBrowser.tsx (Phase 36; tag facet v4.65.0)

Its facet specs are the exported **`SOUND_FACETS`** (v4.79.6) so `SoundPickerModal`
filters with exactly the same segments.

The `audio` LeftPanel body (ASSETS flyout → Sounds): search box, filter strip,
"+ Import Sound", a Manage mode (multi-select checkboxes → Edit (n) /
Delete (n) / Done, the pattern GraphicsBrowser later copied), and one row per
`SoundDef` — a ▶/⏸ preview button (a single reused `HTMLAudioElement`, paused on
...
### SoundPickerModal.tsx (v4.79.6)

The modal counterpart to the AudioBrowser's browsing surface, for call-sites where
a flat `<select>` over the whole library doesn't scale — the playlist clip picker
in `AudioSlotEditor` (`+ clip` / clicking a clip row) and, since v4.79.8, every
`SoundPicker` field (the component now renders a label button that opens this modal).
Fixed-overlay modal (same chrome as AudioImporterModal: dim backdrop, click-outside
or ✕ to close): search box, `AssetFilterBar` driven by the **`SOUND_FACETS`** spec
exported from AudioBrowser (one source, so panel and modal filter identically), and
the same row list with ▶/⏸ preview; clicking a row calls `onPick(soundId)` and the
caller closes. Since v4.79.11 there is also a **multi-select mode**: pass
`onPickMulti` instead of `onPick` (used by the playlist `+ clip` flow) and rows
toggle numbered check badges showing pick order; every close path (footer
`ADD N CLIPS` button, ✕, overlay click) commits the checked ids in that order —
unchecking is the cancel. It manages facet state itself with `buildFacets`/`matchesFacets`
(mirroring `useFacetFilters`' self-healing) because the state must outlive the
component: a module-level `savedState { search, mode, sel }` is restored on mount
and written through on change, so closing after adding a clip and reopening lands
on the same filtered view (per-app-run only — deliberately not persisted).
...
The `graphics` LeftPanel body (ASSETS flyout → Graphics): checkerboard image
tiles (transparent PNGs read correctly — the DecalBrowser backdrop), search
input, category pills, "+ Import Graphics" (opens GraphicsImporterModal), and
a **Manage mode** (AudioBrowser pattern: tiles become multi-select checkboxes,
Edit (n) / Delete (n) / Done toolbar → `onEdit(ids)` / `onDeleteGraphics(ids)`;
App wires these to an `EditMetadataDialog` (noun `"graphic"` — label/category/
attribution, no tags: `GraphicDef` has no tags field) and a `DeleteAssetDialog`
...
The custom-GUI renderer, mounted in BOTH shells (App under `isPreview`,
RuntimeApp under `shell === "playing"`). Renders `uiRegistry(world)` (game.json
+ scene defs); everything derives from gameState — visibility from
`__ui.<id>` (`?? startVisible`), bar/counter/icons values from each element's
`stateKey` — so ONE `state:changed` subscription keeps the whole overlay live
and shown elements survive scene transitions/saves with zero extra wiring.
Widgets are anchored absolute divs (`pointerEvents: "none"`, zIndex 90 —
under DialogueOverlay's 100); menus reuse the dialogue option grammar
(▸ highlight, `menu:nav`/`action:confirm` while a menu is up and no dialogue
is open, mouse hover/click) and emit `ui:menu-shown/-closed` (ControlSchemeManager
menu mode) + `ui:menu-pick` (ScriptEngine dispatches the option). Options are
condition-filtered per render via the shared `checkScriptConditions`. One
navigable menu at a time (first visible wins).

`icons` kind (v4.61.0): a repeated-icon meter (GTA stars / Zelda hearts) —
`count` icons (default 3), each worth `max/count` state units (`max` defaults
to `count`), drawn from `fullGraphicId` / optional `halfGraphicId` /
optional `emptyGraphicId` (fallback: full at 0.25 opacity). Values round to
the nearest half-icon when a half graphic exists, else the nearest whole.
DOM hooks for tests: `data-ui-icons=<id>` on the row, `data-ui-slot`
(`full|half|empty`) per img.
...
- Centered crosshair: two 1px lines, 18px, rgba(255,255,255,0.75) — **hidden in
  ...
  Also gated by the optional `showCrosshair?: boolean` prop (default `true` so
  RuntimeApp needs no edit) — the editor's global toggle (v4.60.1)
...
ObjectGeoView's transform fields additionally gate the scheduled write on `Number.isFinite(parseFloat(raw))` — incomplete tokens (`.`, `-`, empty) stay in the draft without committing, because a NaN→0 write triggers the by-value draft resync (v4.42.5) and clobbers the field mid-keystroke (typing `.5` used to become `5`; v4.51.0). The Scale row also has a **Uniform** checkbox (per-selection default: on when x=y=z, off otherwise): checked, a single unlabeled field writes all three axes (an `XYZ` prefix was tried and removed — it read as part of the typed value; v4.51.1); unchecked, the standard per-axis fields show.
...
- Search input filters by label and id, live (and by tag)
- Category tabs: All, Furniture, Props, Structures, Lights, Characters, Vegetation, Other — derived from manifest, not hardcoded
- `Categories | Tags` segmented switcher swaps the same strip to **tag chips** (`#tag count`, multi-select, ANDed, frequency-ordered) — see the AssetBrowser.tsx section above
...
**Re-origin (v4.52.0):** a ⌖ button beside 📷 (same single-selection gating) opens `ReoriginModal` — moves a model's pivot to its **Base** (bottom-center) or **Center** by rewriting the `.gltf`/`.glb` in place with a `__reorigin` wrapper-node translation (`src/core/gltfReorigin.ts`), optionally shifting placed copies to compensate. v4.52.1 added an orbitable marker preview (amber = origin now, green = Base target, blue = Center target, line = the move; markers staged WITH the model so the framing includes an origin outside the geometry). See the v4.52.x changelog entries for the full flow (cache eviction, `asset:model-updated` rebuild, animation-safety rationale).
...
Categories are open strings (`AssetCategory` ends in `(string & {})`); besides the
built-in pills, the library currently uses **"Baked"** (Phase 26 CSG bakes) and
**"Tiles"** (Phase 43 — 19 Quaternius platformer-kit modular platform tiles,
`platform_<set>_<role>[_tall]`, 2×2×2 logical footprint, walkable top at origin.y+1).
Several Tiles entries carry preset `colliders[]` because the kit meshes are shells —
flat caps/planes whose auto-box from the AABB would be zero-thickness (see the
v4.37.0 changelog entry for the exact list).

**Category vs tags (Phase 51).** `category` is the one exclusive shelf; everything
else an asset could be filed under lives in `tags`. The live library shows why both
are needed: the categories in use mix *which pack it came from* (`Platform: Objects`,
`Pieces`), *what the thing is* (`Furniture`, `Nature`, `Animals`) and *how it was
made* (`Baked`), so any single-select answer loses the other axes — `Door` is under
`Platform: Objects` while `Door1/2/3` are under `Furniture` purely by pack, and
`Rock 2` is `Nature` despite shipping in the Ultimate Furniture Pack. All 111 assets
were backfilled by `scripts/seed-asset-tags.mjs` from data already in the manifest.
...
- Tags — **one `TagInput` shared by the whole batch** (Phase 51), sitting under
  "Set all to" alongside the batch-shared `AttributionFields`. Not per-entry:
  packs share tags, same reasoning as attribution
- Attribution — batch-shared `AttributionFields`, fed `existingAttributions`
  (the loaded registry's attributions, via App) so its **AUTOFILL FROM LIBRARY**
  select can fill the fields from an existing pack (full attribution) or author
...
**If-blocks (Phase 65, v4.79.47).** `ScriptDef.blocks` + `ScriptAction.block` tags; `selectBlockActions(script, ownerId)` (exported, pure) picks each block's branch once — first passing branch, else the `else` branch, else nothing — and `_runActions` dispatches the filtered list (after the trigger delay, before per-action delays). `_ownedScript` resolves "self" inside block conditions. Legacy per-action `conditions` still evaluate in `_dispatch`.
...
| `show_ui` / `hide_ui` | `gameState.set('__ui.<elementId>', true/false)` — GameGuiOverlay derives visibility from the key via `state:changed` (Phase 49; the old dead `ui:show` emit is gone) |
...
"ui:menu-pick":             { elementId: string; optionId: string };  // Phase 49 — GUI menu option picked (visibility itself lives at gameState `__ui.<id>`, not on the bus)
"ui:menu-shown":            { elementId: string };  // Phase 49 — ControlSchemeManager menu-mode gate
...
> **v4.46.0 (Phase 50):** in the RUNTIME shell, `ObjectPlacer` takes an optional `{ instancing?: InstancedObjectPool }` ctor opts; eligible static objects register an instance placement and `build()` returns a proxy `Object3D` (userData + `localAABB`, `userData._instanced`) instead of a clone — the pool renders them as shared-geometry `InstancedMesh`es. The editor never passes the opts, so this path is dead in the editor shell.
...
- Wired to the Phase 10.7 animation mixer/clip system owned by **`ObjectPlacer`**: emits `object:play-animation { id, clipName, loop?, hold?, blend? }`; ObjectPlacer plays the named clip on that object's `AnimationMixer`. Target `"player"` routes to `character:play-animation` instead (v4.63.0), and clip `"__auto__"` routes to `stopPreview` (v4.61.3).
- **CLIP dropdown source (v4.63.2):** the list comes from the *manifest* — `assets.find(a => a.id === targetObj?.assetId)?.animations` — not from the glTF. A clip baked into a `.gltf` is unpickable until its name is added to that asset's `animations` array in `public/assets/models/manifest.json`. The target is resolved first: `"self"` → `owner?.id`, `"player"` → `playerModelAssetId`. An **empty CLIP dropdown means the panel could not resolve the target**, not that the asset lacks clips (the recurring v4.61.2 / v4.61.3 / v4.63.2 symptom).
- **`hold` freezes the clip's LAST frame, whatever pose that is.** It is not "hold the pose the animation builds toward". A stock clip that plays a full round trip (rise *and* retract, e.g. Quaternius' original `SpikeTrap_Activate`) ends where it started, so `hold` looks like it does nothing. Holding a mid-clip pose requires **splitting the clip** so the pose is a final frame — see v4.63.2 and the `gltf-clip-authoring` skill. Author state changes as *pairs of one-way clips*, not one round-trip clip.
...
- **Not the one you want for a hit/damage tint (v4.64.0).** `overlay:fade-in` animates to *full* opacity and makes both `InputManager` and `ControlSchemeManager` suppress player input — it exists for respawns and scene transitions, where the screen must hold opaque while the world changes underneath. For damage feedback use **`flash_player`** → `overlay:flash` → `FlashOverlay`, which peaks at 0.32 as a vignette, releases itself, and never touches input.
...
> **v4.45.0:** `colliderWorldTransform` composes the collider's full local rotation via
> `colliderLocalQuat` (`rotation` euler-XYZ, legacy `rotationY` yaw fallback) — box/capsule
> colliders can be oriented on all three axes; sphere stays identity.
...
**Current shape (v4.36.3):** a 64px column with a 9-entry `TOOLS` list (Select, Floor, Wall, Platform, Stair, Shape, Groups, Spawn, Light — several are menu-first variant groups), followed directly by the **Assets** flyout button (Models / Materials / Decals / Sounds / Skybox / Graphics (v4.43.0) — `ASSET_ENTRIES`; "tool" rows call `onToolSelect("object"|"decal")` and rely on App's tool→panel coupling, "panel" rows call `onPanelToggle`), then the standalone **Prefabs** panel button (v4.41.2 — briefly a flyout row in v4.38.0–4.41.1, moved out because prefabs are authored project data, not imported assets; toggles the `prefabs` panel, highlights when it's open or the `prefab` tool is armed, and the panel's Place buttons arm the tool), then the **Trigger** tool (`TRIGGER_TOOL`, rendered via the hoisted `renderTool` — moved beside Scripts in v4.36.3 since trigger volumes host their scripts there), then the amber **Scripts** panel button (`IconScript`, a bulleted list — no longer sharing the Trigger tool's icon, v4.36.3) — panel buttons 48×48 with tool-style labels (v4.36.2). A `flex:1` spacer below them keeps only the divider + Play split-button row anchored at the bar's bottom. The Object and Decal tools live *only* in the Assets flyout (moved from the tool list in Phase 42, along with the removal of the separate MATS/SOUNDS/SKYBOX buttons). The flyout anchors `bottom: 0` and shares the variant-popover close plumbing via the `openMenu` sentinel `"assets-menu"`. Assets button active-state: any of its six panels/tools active; its icon swaps to the active entry's.
...
| `ScenePreloader.ts` | Level preloading (v4.84.0). `warm(sceneId)` = MENU tier: fetch the scene JSON, `collectAssetRefs` it, then download AND decode every referenced model/material/skybox/decal/graphic/sound into AssetManager's caches (6 at a time) so Start/Continue find the level in memory. `sceneEntered(sceneId, file)` (called by the router after `onPlaying`) = PLAYING tier: marks the scene's assets as in-memory, then after 3 s downloads, never decodes, what the scene's `load_scene` targets need (`AssetRefs.scenes`, max 3 targets, 2 at a time, `fetch` `priority: "low"`, body streamed and dropped) so the next transition reads from the HTTP cache. `stop()` (router calls it at the top of `go()`) halts queued work; in-flight items finish and are shared with the level build. Skips: assets already warmed/fetched, `navigator.connection.saveData`, and hosts that answer `cache-control: no-store` (the dev shell), where a download-only prefetch would just be fetched again. Every failure is swallowed; `stats` is on `__runtime.preloader` under dev globals. |
...
## Desktop Shell (Deno) — Phases 54–58 (v4.67.0–v4.71.0)

The editor is now a **Deno desktop application**: `deno desktop` (Deno ≥ 2.9)
compiles our code, the Deno runtime, and a bundled-Chromium (**CEF**) webview
into one distributable binary per platform (`deno.json` tasks:
`desktop` / `desktop:hmr` for the dev loop, `compile:mac-arm64` /
`compile:mac-x64` / `compile:win-x64`; the spike-measured binary is ~421 MB —
≈300 MB fixed CEF floor + the embedded `dist/`). **Plain-browser editing is
retired** (user decision, phase 55): the vite dev server still renders the
editor, but every persistence path — projects, autosave, asset management,
export — needs the shell. The point of the conversion: the entire persistence
layer was a stack of browser-sandbox workarounds (FSA handles persisted in
IndexedDB, six session-only folder handles, re-grant banners, user-activation
ordering, an HTTP probe in Play, truncate-on-write saves that destroyed a
level — v4.29.4), and the shell deletes the whole pile while unlocking a real
export/deploy pipeline.

Plans: `plans/phase-54-deno-desktop-spike.md` … `phase-57-desktop-export-bundle.md`;
acceptance: `test-plans/phase-54…57-*.md`. All four phases live on branch
`deno-desktop`. (Phase 58 — the compile matrix proper plus the final FSA
cleanup deleting `src/fsa.d.ts`/`src/lib/fileHandleStore.ts` and TopBar's dead
reopen banner — follows in `plans/phase-58-desktop-packaging.md`.)

### Transport: HTTP, not the bindings bridge

ALL frontend↔backend traffic is plain HTTP against the shell's own
`Deno.serve`:

- **`POST /api/<method>`** with a JSON **array of args** as the body — one
  route, dispatched through the `apiMethods` table in `desktop/main.ts`;
  errors come back as `{ error }` + HTTP 500. (`GET /api/getAppInfo` also
  works — it doubles as the desktop-detection probe.)
- **`POST /api-file/<kind>/<rel…>`** with raw bytes as the body — binary
  asset uploads (GLBs, PNGs, OGGs). The JSON api can't carry bytes.

The in-process webview **bindings bridge is deliberately NOT used**. The
phase-54 spike measured it fast for JSON (1 MB string ≈ 28 ms) but found
`Uint8Array` args broken outright (laufey 0.6.1 `TypeError`; one run froze the
bridge), and phase 55 then hit the killer twice: the bridge **intermittently
deadlocks at launch** — the first call never resolves, which hung editor boot
on `readAutosave` and produced a blank window. HTTP has been solid in every
run, so all app traffic moved to it; the bindings (`spikePing`,
`spikeEchoBytes`, `spikeEchoJson`, `spikeReport`) remain registered on the
editor window **as spike diagnostics only** (`/spike.html` is kept as a
shell-regression harness).

### Serving model (desktop/serve.ts) — the workspace/VFS overlay

`makeHandler(distDir, ws)`:

- **`/games/*` and `/assets/*`** are served from the **workspace content dir
  first** (`cache-control: no-store` — the editor re-reads what it just
  wrote), **falling through to the built `dist/` on 404** (disk in dev, the
  embedded in-memory VFS in the compiled app).
- Everything else serves from `dist/`, with **HTML responses `no-store`**
  (v4.72.0): hashed chunk filenames are immutable and stay cacheable, but a
  heuristically-cached `index.html` pins a stale bundle hash across rebuilds —
  the dev-loop "reload shows old code" failure mode.

The fallthrough is **load-bearing**, twice over: (1) Vite emits the app's OWN
hashed js/css chunks under `dist/assets/` (`build.assetsDir` default) — the
same URL prefix as the asset library — so routing all of `/assets/*` to the
workspace 404'd `main-*.js` and produced a perfectly blank window with no
console error (the phase-55 incident; lesson recorded: re-probe page boot
after ANY serve-layer change). (2) It is what makes the **overlay asset
model** work at all — see the asset-library section below.

### Workspace (desktop/workspace.ts)

| Mode | Content dir (`games/`, `assets/`) | State dir |
|---|---|---|
| Dev (repo checkout — detected by `<cwd>/public/games` existing) | `<repo>/public` | `<repo>/.worldbuilder` (gitignored) |
| Packaged app | `~/WorldBuilder` | `~/WorldBuilder/.state` |
| `WORLDBUILDER_WORKSPACE=<dir>` | `<dir>` | `<dir>/.state` |

Dev mode writes into the same `public/` tree vite serves, so the
commit-`public/games/**`-around-sessions safety rule (CLAUDE.md §5) continues
unchanged. The state dir holds `settings.json` (`lastSession` +
string-keyed `prefs`), `autosave/` (`latest.json` + `latest.meta.json`),
`backups/` (rotating, **10 per key**, timestamp+uuid filenames so same-ms
saves can't collide), `trash/` (timestamped moves), and `exports/`
(single-file saves + game bundles).

**Data safety — the FSA truncate-on-write failure mode is gone by
construction:** every write is atomic (tmp file in the same dir + rename);
every JSON write is `JSON.parse`-guarded server-side (refuses to write
non-JSON); every overwrite of an existing scene/game/manifest/asset-manifest
first copies the old file into `backups/<key>/`; deletes (scenes, imported
assets) move files to `trash/` instead of unlinking. Path safety:
`assertSafeId` on every id/segment (no traversal), `assertSafeRel` for
subdir'd asset rels.

### Backend API (desktop/main.ts + projects.ts + assets.ts)

`desktop/projects.ts` is **primitives only** — manifest/game-config semantics
stay in the frontend `ProjectStore`: `listProjects` (scans `games/*/manifest.json`,
`manifestVersion === 1`, newest-first), `createProject` (mkdir + duplicate
refusal), `saveScene` (backup + atomic write, returns the backup path),
`deleteScene` (→ trash), `writeGameFile`, `writeProjectManifest`, the
autosave trio (`writeAutosave`/`readAutosave`/`clearAutosave` —
`latest.json` + meta `{projectId, sceneId, savedAt}`), and `writeExportFile`
(single-file saves into `exports/`). `desktop/workspace.ts` adds
`get/setLastSession` + `get/setPref` (settings.json).

`desktop/assets.ts`: `writeAssetManifest` (kind-checked, atomic + rotating
backup — kills the manifest-truncation failure mode), `writeAssetFile`
(the `/api-file` handler target; subdir rels support texture quality tiers),
`deleteAssetFiles` (workspace files → trash, counted; names that only exist
in the embedded VFS are **silently skipped** — removing the manifest entry IS
their deletion).

`desktop/main.ts` shell ops: `getAppInfo` (version/platform/dirs/serveOrigin/dev),
**`openRuntimeWindow({ manifestUrl, title? })`** — navigates a reusable second
native `Deno.BrowserWindow` to `runtime.html?manifest=…` (`window.open`
returns null in the webview; the window is recreated after close),
`revealPath` (Finder/Explorer/xdg-open), `exportGameBundle` (below). Dev
diagnostics: `WORLDBUILDER_BOOT=spike|probe55` serves that harness page at `/`
(the implicit CEF window auto-navigates to the serve root — whatever should
show first must be served there, a phase-54 gotcha).

### Frontend seam (src/shared/desktopApi.ts)

A typed `DesktopApi` interface + a `Proxy` client that turns any method call
into `POST /api/<method>`; `uploadAssetFile(kind, rel, data)` wraps
`/api-file`. **`detectDesktop()`** (call once, early in boot — App awaits it
before any storage code runs) probes `GET /api/getAppInfo` and caches the
parsed body; `desktop()` returns the client or `null`, `isDesktop()` the
cached boolean, and **`isDesktopDev()`** (v4.72.0) is true when the shell
reported `dev: true` — App/RuntimeApp use it to install the `window.__*` dev
globals + `__test` harness in shell-served pages, where the production dist
compiled out the `import.meta.env.DEV` path (packaged apps report `dev: false`,
so end users never get them). Before detection resolves, `desktop()` answers
`null` and callers fall back to their browser path — the safe default.

### ProjectStore.ts rewritten (phase 55)

Same public API as the Phase 33 original (`create`/`open`/`loadScene`/
`saveScene`/`addScene`/`removeScene`/`writeManifest`/`writeGame`,
`slugifyId`/`uniqueSceneId`) — App's call sites survived nearly untouched —
but **no `FileSystemDirectoryHandle` anywhere**:

- **Reads are fetches** against the shell's server (`fetch('/games/<id>/…',
  { cache: "no-store" })`) — the same URL shapes the runtime uses.
- **Writes go through the api** (`saveScene`/`writeGameFile`/
  `writeProjectManifest`/`createProject`/`deleteScene`), which means atomic +
  backed-up + trash-not-delete on the Deno side for free.
- **Session persistence is a plain `{projectId, sceneId}`** in workspace
  settings (`persistLastProject`/`restoreLastProject`/`clearLastProject` →
  `set/getLastSession`). The whole permission apparatus — `fileHandleStore`
  (IDB handle persistence), `queryPermission`/`requestPermission`, the amber
  REOPEN banner, user-activation ordering — is **deleted**; boot restores the
  last project by id silently.
- `publishTo` is gone (removed with the PROJ-menu Publish… in phase 55;
  replaced by Phase 57's export). ▶ Play calls `openRuntimeWindow` — the
  old HTTP probe + "is this folder under public/games?" guesswork deleted.
- `NewProjectModal` is **name-only** (the workspace owns the location —
  `games/<slug>/`); the new `OpenProjectModal` lists workspace projects via
  `listProjects` (name, `games/<id>/`, scene count, last-saved date).

Autosave (App.tsx): `storeAutosave`/`readStoredAutosave`/`clearStoredAutosave`
route to the workspace autosave api when `desktop()` answers; localStorage
(`worldeditor_autosave`) only as the plain-browser fallback. No-project FILE
Save writes `stateDir/exports/<slug>.json` and reveals it; Load keeps the
TopBar `<input type="file">`.

### Asset library (phase 56): overlay, not seeding — src/assets/assetLibrary.ts

The original plan seeded `~/WorldBuilder/assets` with the 132 MB stock library
on first run; the serve overlay made that unnecessary:

- **Stock assets ship inside the binary** (dist/VFS) and are never copied out.
- **Imports/edits write real files** into `workspace/assets/<kind>/…`,
  shadowing the stock copy at the same URL.
- **Deleting a stock asset = removing its manifest entry** (the VFS file
  becomes unreachable); deleting an imported asset moves the file to trash.
- `~/WorldBuilder` stays lean: user content only, no seed-version bookkeeping.

`src/assets/assetLibrary.ts` is the **single owner of asset-library
mutations** (absorbing `src/core/assetLibraryWriter.ts`, deleted, and the
manifest-splice logic previously duplicated across modals and App handlers):
`readManifest` (cache-bypassing fetch, optional fallback), `writeManifest`,
`writeAssetFile` (→ `uploadAssetFile`), `removeAssetFiles`,
`removeEntries`/`updateEntries`/`upsertEntry` (read-modify-write by id, keyed
per kind via `LIST_KEY` — `assets`/`materials`/`sounds`/`skyboxes`/`graphics`/
`decals`), and `writeAssetToLibrary` (the Phase 26 bake path). In a plain
browser every mutation throws ("Asset management needs the desktop app").

The **five importer modals** (models, audio, skyboxes, graphics, materials)
select files with plain `<input type="file">` elements (`webkitdirectory` for
the material source-folder scan) — destination pickers and the folder-grant UI
are deleted, as are App's six dir-handle states, `ensureDir`, and the
`needsFolderGrant`/`folderHint` props threaded through the delete dialogs.
**Zero `showDirectoryPicker`/`showSaveFilePicker`/`showOpenFilePicker` call
sites remain in `src/`** (net −481 lines; the leftover `src/fsa.d.ts` types
and `src/lib/fileHandleStore.ts` were deleted by phase 58's final cleanup).

### Game export (phase 57): src/export/assetRefs.ts + desktop/export.ts

Replaces both the manual PUBLISHING_GUIDE recipe (host all 151 MB of dist)
and the deleted FSA Publish (which copied JSON only — the sandbox couldn't
walk assets).

**`src/export/assetRefs.ts`** — plain TS (no DOM), imported by the frontend
AND the Deno backend. `collectAssetRefs(scenes, game)` walks every reference
class: object `assetId`s + per-object material overrides, platform/shape/
stair/ladder/floor/wall surface materials (incl. per-face brush materials),
attached sounds, script actions at all four homes (sound/music/material,
dialogue portraits), zone decals, skybox (`"sky"` = built-in procedural, never
collected), scene music/ambient, item icons + dialogue portraits (raw
`/assets/**` `<img src>` paths, matched back to graphics entries by path at
resolve time), uiElement graphicIds (bar/counter/icons/image/menu-options),
and playerSettings (avatar model + locomotion sounds). The prefab library
(`game.prefabs`) is deliberately NOT walked (v4.83.3): placed instances are
real entities in the scenes, the runtime never reads templates, so an unplaced
prefab must not drag its assets into the bundle. `resolveAssetFiles(refs, manifests)` maps
ids → concrete files: `{quality}` texture paths expand to all three tiers,
every material map slot ships (per-instance overrides can re-enable disabled
maps), model/skybox thumbnails are **excluded** (editor-only; the field is
dropped from the pruned entries), decals ship albedo + optional normal/
roughness. Returns the file list, **per-kind manifests pruned to the
referenced entries**, and a `missing` list (reported, never fatal).

**`desktop/export.ts` `exportGameBundle({ projectId })`** writes
`stateDir/exports/<projectId>-bundle/`: `runtime.html` with its absolute
`"/assets/…"` script refs rebased to `"./assets/…"` (works from any
static-host subpath), Vite's hashed chunks (**only the depth-1 files of
`dist/assets` reachable from `runtime.html`**, found by crawling hashed file
names through the html and each reachable js/css chunk; the editor's
`main-*` entry chunk and the dev-only `testHelpers-*` chunk never ship
(v4.83.2); the subdirs there are the full copied asset library, which is
exactly what's being pruned), a redirecting `index.html`, the game manifest
with `assetsBase: "./"`, `game.json` + scenes verbatim, pruned per-kind
manifests, and the referenced files copied **workspace-first with dist/VFS
fallback** — the same overlay as serving. Unmanifested model ids still ship
`<id>.glb` (AssetManager's `loadGLTF` fallback). Measured on `platfrom-obby`:
**68 files / 10.85 MB vs 151 MB dist**; the bundle played from a plain
`python3 -m http.server` with zero failed asset requests.

**`desktop/deploy.ts`** — a `DeployProvider { name, deploy(bundleDir) }`
interface + empty registry. Deliberately unwired (bundle-first decision);
provider uploads (static hosts, itch.io, zip) are a later increment.

UI: **"Export game…"** in the PROJ ▾ ⋯ menu (desktop only) → saves first (the
export reads from disk), then alerts file count / size / missing refs and
reveals the bundle folder.

### What phase 54 (the spike) established

CEF-backend verdicts recorded in `test-plans/phase-54-deno-desktop-spike.md`:
WebGL2 (ANGLE Metal), WASM/Rapier, Gamepad + pointer-lock APIs, Web Audio,
and localStorage-across-relaunch all pass (existing localStorage keys —
prefabs, session prefs, bindings — keep working unchanged); `window.open`
returns null (hence `openRuntimeWindow`); the real editor boots and renders
in the shell. Embedding: plain `--include dist` + in-memory VFS (~150 MB/s
serve throughput, no `--self-extracting`, `--exclude node_modules
--exclude-unused-npm` required). Compiled-mode paths must fall back from
`Deno.cwd()` to `import.meta.url`; `Object.keys(bindings)` is empty (proxy) —
feature-detect by calling, not enumerating.

### Phase 58 — packaging + cleanup (v4.71.0)

The shipping phase. `deno.json` carries the compile matrix
(`compile:mac-arm64` / `compile:mac-x64` / `compile:win-x64` / `compile:all`,
outputs under gitignored `build/`) — all cross-compiled from one machine with
the CEF backend, `--include dist` (the stock asset library ships inside the
binary; the overlay serves it), and `--exclude node_modules
--exclude-unused-npm`. Measured artifacts: 421 MB mac-arm64 `.app`, 458 MB
mac-x64 `.app`, 273 MB win-x64 `.msi`; ~300 MB of each is the fixed CEF
framework. The last FSA remnants left the tree here: `src/fsa.d.ts`,
`src/lib/fileHandleStore.ts`, TopBar's `onLoadFSA` branch, the reopen banner,
and the Publish prop/block. Follow-ups tracked in
`plans/phase-58-desktop-packaging.md`: signing/notarization,
`Deno.autoUpdate()`, deploy providers, an app icon, real-hardware smoke for
the Intel and Windows builds.
