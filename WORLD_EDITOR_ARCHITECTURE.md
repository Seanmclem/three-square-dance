...
### Lights list (v4.29.1, always-visible v4.29.2)

**LIGHTS IN THIS ZONE (n)** section in the nothing-selected ToolView
(`LightListSection`): shown under **every** tool whenever the zone has lights
(v4.29.2); with zero lights it appears only under the Light tool (empty-state
placement hint — other tools don't carry a permanent empty section). One row per
`zone.lights` entry — glowing color swatch, label-or-id, kind glyph, `☑︎sh` when
shadow-casting. Clicking a row selects that light (App `handleSelectLight`:
switches to the Select tool so the next canvas click doesn't place, then emits
`object:selected`) — panel shows its LightView, gizmo attaches. WORLD LIGHT stays
Light-tool-only. Backing state: App `zoneLights`, synced like `checkpoints`
(`light:added/updated/removed` + `zone:activated` + `world:loaded`).
