...
**Version 4.20.1** — last updated 2026-07-08
...
- v4.20.1 — **Runtime menu backdrop: vantage camera on exit-to-menu.** Nothing drives the game-mode default camera, so after exiting to the menu it sat at the origin *inside* the still-loaded level (close-up brick / trigger-volume fills, floor-coplanar view). `RuntimeApp`'s exit-to-menu path now frames it at `spawn + (9, 8, 9)` looking at the spawn — the level renders as a diorama behind the menu card. First boot (no world loaded) keeps the plain sky.
