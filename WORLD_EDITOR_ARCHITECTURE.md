...
**LIGHTS IN THIS ZONE (n)** section above WORLD LIGHT in the Light tool's ToolView
(`LightListSection`): one row per `zone.lights` entry — glowing color swatch,
label-or-id, kind glyph, `☑︎sh` when shadow-casting. Clicking a row selects that
light (App `handleSelectLight`: switches to the Select tool so the next canvas
click doesn't place, then emits `object:selected`) — panel shows its LightView,
gizmo attaches. Backing state: App `zoneLights`, synced like `checkpoints`
