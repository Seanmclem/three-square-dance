...
**Version 4.57.0** — last updated 2026-08-01
...
same HEAD-filter pattern), with `getGraphicDef` / `getGraphicList`. Graphics are
...
input, category pills, "+ Import Graphics" (opens GraphicsImporterModal). No
manage/delete mode in v1. Also exports **`GraphicPickerPopover`** — a
searchable checkerboard mini-grid popover used by the ITEMS tab's icon Pick
button and the UI tab's graphicId fields; all tile/preview srcs go through
`assetManager.resolveUrl`.
