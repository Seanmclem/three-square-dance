...
| 9f | (?) help icon (v4.34.3) | Click the (?) in the ← header row | VERIFIED 2026-07-20: icon rgb(221,227,240), popover opens below, right-anchored within panel, correct DIALOGUE text, popover text rgb(194,202,219) |
| 9g | Picked-response dimming (v4.34.4) | In preview, loop a dialogue back to an answered page | VERIFIED 2026-07-20: picked row renders "Ask around.  ✓" at rgb(90,100,116) / opacity 0.75 when not highlighted (highlight overrides, still selectable); fresh conversation shows no ✓ |
| 9h | + Add scroll/focus (v4.34.4) | + Add on a node with a nested subtree | VERIFIED 2026-07-20: focus lands in the new response's empty text input, well in view (`wb-dlgopt-*` anchor); nested pages directly below their response, no ↳ label |
| 9i | Slight inset (v4.34.4) | Measure n1→n3 chain | VERIFIED 2026-07-20: lefts 76/93/97 — +17 level 1, +4/level after (breakout −14) |
| 9j | Accordion self-collapse fix (v4.34.4) | Type into a fresh response's text | VERIFIED 2026-07-20: stays open. Root cause found live: the open-default was DERIVED ("empty ⇒ open"), so the first typed character flipped it shut — this was the "phantom collapse" seen in earlier sessions. Fixed with a per-option sticky seed (`seedOpen` ref, evaluated once per option id) |
