...
interface DeleteAssetDialogProps {
  labels:    string[];                  // labels of the assets to delete
  usage:     { count: number; zones: string[] };  // placed-object usage across zones
  needsFolderGrant: boolean;            // true if the models folder isn't granted yet this session
  onCancel:  () => void;
...
export function DeleteAssetDialog({ labels, usage, needsFolderGrant, onCancel, onConfirm }: DeleteAssetDialogProps) {
  ...
  return (
    <div style={S.overlay} onClick={e => { if (e.target === e.currentTarget) onCancel(); }}>
      <div style={S.card}>
        <div style={S.title}>DELETE {many ? `${labels.length} MODELS` : "MODEL"}</div>
        <div style={S.body}>
          Remove {many ? "these models" : "this model"} from the library?
          <ul style={S.list}>
        ...
        {usage.count > 0 && (
          <div style={S.warn}>
            ⚠ {usage.count} placed object{usage.count !== 1 ? "s" : ""} across{" "}
            {usage.zones.length} group{usage.zones.length !== 1 ? "s" : ""} use{usage.count === 1 ? "s" : ""}{" "}
            {many ? "these models" : "this model"}. They will show as placeholder boxes until reassigned or removed.
          </div>
        )}

        <label style={S.check}>
          <input type="checkbox" checked={deleteFiles} onChange={e => setDeleteFiles(e.target.checked)} />
          Also delete the model file{many ? "s" : ""} from disk (irreversible)
        </label>

        {needsFolderGrant && (
          <div style={S.hint}>
            Next, the browser will ask for a folder — select{" "}
            <strong style={{ color: "#90a4c0" }}>public/assets/models</strong> and allow editing.
          </div>
        )}
