...
interface AssetBrowserProps {
  ...
  onReorigin:      (id: string) => void;
}

export function AssetBrowser({ assets, selectedAssetId, onSelect, onImport, onDeleteAssets, onEdit, onRestage, onReorigin }: AssetBrowserProps) {
        ...
        {!manage ? (
          ...
          <>
            ...
            <button
              ...
              disabled={checked.size !== 1}
              title="Re-origin — move the model's pivot to its base or center (select exactly one)"
              style={{
                flexShrink: 0, padding: "5px 8px",
                background: checked.size === 1 ? "rgba(80,140,255,0.12)" : "rgba(255,255,255,0.03)",
                border: `1px solid ${checked.size === 1 ? "rgba(80,140,255,0.3)" : "rgba(255,255,255,0.07)"}`,
