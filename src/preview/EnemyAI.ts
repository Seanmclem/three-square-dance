...
import { tick, MAX_VERTICAL_GAP, OVER_TOP_SLACK, type BrainMem, type BrainSenses } from "./enemyBrain";
 ...
 *  see BrainMem) plus the physical fields only the adapter needs. */
interface AiRec extends BrainMem {
  id:    string;
  def:   EnemyAIDef;
...
export class EnemyAI {
  ...
  update(dt: number): void {
    ...
    for (const rec of this._recs.values()) {
      ...
      if (!entry) { this._stopWalkSound(rec); continue; }   // entity deleted mid-preview — don't strand the loop
      ...
      const senses: BrainSenses = {
        ...
        dyOk:     Math.abs(player.y - rec.pos.y) <= MAX_VERTICAL_GAP,
        overTop:  (player.y - rec.pos.y) > (p.heightY ?? 1) + OVER_TOP_SLACK,
        fromPost: Math.hypot(rec.pos.x - rec.post.x, rec.pos.z - rec.post.z),
        homeDist: Math.hypot(rec.post.x - rec.pos.x, rec.post.z - rec.pos.z),
        clock:    this._clock,
      ...
      let moved = false;
      if (intent.move === "approach") {
        this._dir.set(dx, 0, dz).normalize();
        if (intent.orbitBias) this._dir.applyAxisAngle(this._up, intent.orbitBias);
        ...
        this._dir.set(rec.post.x - rec.pos.x, 0, rec.post.z - rec.pos.z).normalize();
        moved = this._step(rec, this._dir, p.moveSpeed, dt, entry);
        this._turnToward(rec, Math.atan2(this._dir.x, this._dir.z), dt);
      } else if (intent.facePlayer) {
        this._turnToward(rec, Math.atan2(dx, dz), dt);
      }
